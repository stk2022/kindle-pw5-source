...
static unsigned long log_goal_buf = 0;
static u32 log_goal_buf_size = 0;
...
#ifdef CONFIG_LAB126_PRINTK_BUFFER
	if (log_goal_idx) {
		/* log message with timestamp to log_goal buffer */
		timestamp_len = print_time(msg->ts_nsec, NULL);
		if ((msg->text_len + timestamp_len) >= (log_goal_last - log_goal_idx)) {
			/* clear left buffer */
			memset(log_goal_idx, 0, log_goal_last - log_goal_idx);
			log_goal_idx = log_goal_first;
		}
		/* no else here */
		if ((msg->text_len + timestamp_len) < (log_goal_last - log_goal_idx)) {
			timestamp_len = print_time(msg->ts_nsec, log_goal_idx);
			log_goal_idx += timestamp_len;
...
#ifdef CONFIG_LAB126_PRINTK_BUFFER
#define PRINTK_BUFFER_PROC "printkbuf"

#ifdef CONFIG_MTK_RAM_CONSOLE
void setup_printk_proc(void) {
	proc_symlink(PRINTK_BUFFER_PROC, NULL, "last_kmsg");
}

#else

int reserve_memory_printk_buf_fn(struct reserved_mem *rmem)
{
	log_goal_buf_size = rmem->size / 2;
	log_goal_buf = rmem->base;
	return 0;
}

RESERVEDMEM_OF_DECLARE(reserve_memory_printk_buf, "amazon,printk-buffer-reserve-memory",
		reserve_memory_printk_buf_fn);

static ssize_t lastk_buffer_read(struct file *file, char __user *buf, size_t count, loff_t *off) {
	if (!last_log_buffer) {
		pr_warn("Printk backup buffer is unavailable\n");
		return 0;
	}

	return simple_read_from_buffer(buf, count, off, last_log_buffer, log_goal_buf_size);
}
...
void setup_printk_proc(void) {
	...
	backup_proc_entry = proc_create(PRINTK_BUFFER_PROC, S_IRUGO, NULL, &printkbuf_fops);

	if (!backup_proc_entry) {
		pr_err("Failed to initialize printk recovery proc entry\n");
	}
}
#endif
...
#ifdef CONFIG_LAB126_PRINTK_BUFFER
	if (!early && log_goal_buf) {
		log_goal_idx = log_goal_first = __va(log_goal_buf);
		log_goal_last = log_goal_first + log_goal_buf_size - 1;
		last_log_buffer = log_goal_first + log_goal_buf_size;
		memcpy(last_log_buffer, log_goal_first, log_goal_buf_size);
		pr_info("Prink buffer had been relocated to physical address: 0x%08lx, size: 0x%x\n",
				log_goal_buf, log_goal_buf_size);
		/*
		 * Now that it is saved, clear out last time's prink to prevent log framents from
		 * recusrively showing up in printk logs.
		 */
		memset(log_goal_first, '\0', log_goal_buf_size);
	}
#endif /* CONFIG_LAB126_PRINTK_BUFFER */
