...
#ifdef CONFIG_FALCON
static void __falcon_clr_preload_bytes(phys_addr_t paddr, size_t size)
{
	phys_addr_t start = 0;
	phys_addr_t end = 0;
	int num = 0;

	start = PAGE_ALIGN(paddr);
	end = paddr + size - ((paddr + size) % PAGE_SIZE);

	if (start <= end)
		num = (end - start) / PAGE_SIZE;
	if (num)
		falcon_clr_preload(start, num);
}
#endif /* CONFIG_FALCON */
	     ...
	     unsigned long attrs)
{
...
#ifdef CONFIG_FALCON
	{
		...
		falcon_set_preload_bytes(paddr, size);
	}
#endif /* CONFIG_FALCON */
		...
		size_t size, enum dma_data_direction dir, unsigned long attrs)
{
#ifdef CONFIG_FALCON
	{
		...
		phys_addr_t paddr = page_to_phys(page) + (handle & ~PAGE_MASK);

		__falcon_clr_preload_bytes(paddr, size);
	}
#endif /* CONFIG_FALCON */
	if ((attrs & DMA_ATTR_SKIP_CPU_SYNC) == 0)
				       ...
				       gfp_t gfp, int coherent_flag)
{
	...
	if (!page)
		return NULL;
#ifdef CONFIG_FALCON
	falcon_set_preload(page_to_phys(page), (1 << order));
#endif /* CONFIG_FALCON */
...
#else /* CONFIG_FALCON */
	for (p = page + (size >> PAGE_SHIFT), e = page + (1 << order); p < e; p++) {
		falcon_clr_preload(page_to_phys(p), 1);
		__free_page(p);
	}
#endif /* CONFIG_FALCON */
...
static void __dma_free_buffer(struct page *page, size_t size)
{
	...
	while (page < e) {
#ifdef CONFIG_FALCON
		falcon_clr_preload(page_to_phys(page), 1);
#endif /* CONFIG_FALCON */
		__free_page(page);
		page++;
	}
			 ...
			 unsigned long attrs, const void *caller)
{
	...
	if (!buf)
		return NULL;
#ifdef CONFIG_FALCON
	falcon_set_preload_bytes(virt_to_phys(buf), sizeof(*buf));
#endif /* CONFIG_FALCON */
...
#ifdef CONFIG_FALCON
		__falcon_clr_preload_bytes(virt_to_phys(buf), sizeof(*buf));
#endif /* CONFIG_FALCON */
		kfree(buf);
	}
		...
		enum dma_data_direction dir, unsigned long attrs)
{
	...
	for_each_sg(sg, s, nents, i) {
		...
		if (dma_mapping_error(dev, s->dma_address))
			goto bad_mapping;
#ifdef CONFIG_FALCON
		falcon_set_preload_bytes(s->dma_address, s->length);
#endif
	}
	return nents;
...
#ifdef CONFIG_FALCON
	falcon_set_preload_bytes(virt_to_phys(pages), array_size);
#endif /* CONFIG_FALCON */
...
#ifdef CONFIG_FALCON
		falcon_set_preload(page_to_phys(pages[i]), (1 << order));
#endif /* CONFIG_FALCON */
		i += 1 << order;
		count -= 1 << order;
	}
...
error:
	...
	while (i--) {
		if (pages[i]) {
			falcon_clr_preload(page_to_phys(pages[i]), 1);
			__free_pages(pages[i], 0);
		}
	}
	__falcon_clr_preload_bytes(virt_to_phys(pages), array_size);
#endif /* CONFIG_FALCON */
	kvfree(pages);
	return NULL;
}
...
#ifdef CONFIG_FALCON
	__falcon_clr_preload_bytes(virt_to_phys(pages),
...
arm_iommu_create_mapping(struct bus_type *bus, dma_addr_t base, u64 size)
{
	...
	if (!mapping)
		goto err;
#ifdef CONFIG_FALCON
	falcon_set_preload_bytes(virt_to_phys(mapping),
	...
	if (!mapping->bitmaps)
		goto err2;
#ifdef CONFIG_FALCON
	falcon_set_preload_bytes(virt_to_phys(mapping->bitmaps),
				 extensions * sizeof(unsigned long *));
#endif /* CONFIG_FALCON */
	...
	if (!mapping->bitmaps[0])
		goto err3;
#ifdef CONFIG_FALCON
	falcon_set_preload_bytes(virt_to_phys(mapping->bitmaps[0]), bitmap_size);
#endif /* CONFIG_FALCON */
...
err4:
#ifdef CONFIG_FALCON
	__falcon_clr_preload_bytes(virt_to_phys(mapping->bitmaps[0]),
				   bitmap_size);
#endif /* CONFIG_FALCON */
...
err3:
#ifdef CONFIG_FALCON
	__falcon_clr_preload_bytes(virt_to_phys(mapping->bitmaps),
				   extensions * sizeof(unsigned long *));
#endif /* CONFIG_FALCON */
	kfree(mapping->bitmaps);
err2:
#ifdef CONFIG_FALCON
	__falcon_clr_preload_bytes(virt_to_phys(mapping),
				   sizeof(struct dma_iommu_mapping));
#endif /* CONFIG_FALCON */
	kfree(mapping);
err:
...
static void release_iommu_mapping(struct kref *kref)
{
	...
	__falcon_clr_preload_bytes(virt_to_phys(mapping->bitmaps),
				   mapping->extensions * sizeof(unsigned long *));
	kfree(mapping->bitmaps);
...
static int extend_iommu_mapping(struct dma_iommu_mapping *mapping)
{
	...
	if (!mapping->bitmaps[next_bitmap])
		return -ENOMEM;
#ifdef CONFIG_FALCON
	falcon_set_preload_bytes(virt_to_phys(mapping->bitmaps[next_bitmap]),
				 mapping->bitmap_size);
#endif /* CONFIG_FALCON */
			...
			const struct iommu_ops *iommu, bool coherent)
{
	 ...
	 */
	/* v4.9 don't has dev->dev_ops. fix this for build@YongWu */
	if (get_dma_ops(dev)) /* dev->dma_ops */
		return;
