...
int falcon_alloc_work(void);
void falcon_free_work(void);
...
extern void (*falcon_pm_idle)(void);
void (**pm_idle_func)(void) = &falcon_pm_idle;
#else
void (**pm_idle_func)(void) = &pm_idle;
#endif
EXPORT_SYMBOL(pm_idle_func);
...
static void add_write_perm(u32 addr)
{
	...
	modify_pte(addr, L_PTE_RDONLY | (1 << 7), 0);
}
EXPORT_SYMBOL(add_write_perm);
...
static void restore_perm(u32 addr)
{
	...
	modify_pte(addr, 0, L_PTE_RDONLY | (1 << 7));
}
EXPORT_SYMBOL(restore_perm);
...
static int __init falcon_init(void)
{
	...
	falcon_dev = kzalloc(sizeof(struct device), GFP_KERNEL);
	falcon_dev->coherent_dma_mask = 0xffffffff;
	arch_setup_dma_ops(falcon_dev, 0, 0, NULL, 0);
#endif

	falcon_set_preload_bytes(virt_to_phys(&falcon_error), sizeof(int));
...
int falcon_io_patch = 0;
EXPORT_SYMBOL(falcon_io_patch);
				     ...
				     int depth, void *data)
{
	...
	reg = of_get_flat_dt_prop(node, "io-patch", &l);
	if (reg)
		falcon_io_patch = dt_mem_next_cell(1, &reg);
...
#if 1
        // reserve 1 page before bios
        idle_buff = (void *)(storage_bios_addr - PAGE_SIZE);
        memblock_reserve(__virt_to_phys(storage_bios_addr - PAGE_SIZE), PAGE_SIZE);
#endif
...
int add_revert_process(struct mm_struct *mm)
{
	struct falcon_revert_entry *revert = (struct falcon_revert_entry *)kmalloc(sizeof(struct falcon_revert_entry), GFP_KERNEL);
	if (!revert) {
		return -ENOMEM;
	}

	revert->mm = mm;
	list_add_tail(&revert->head, &falcon_revert_list);
	return 0;
}
EXPORT_SYMBOL(add_revert_process);

int prepare_revert_mmu(void)
{
	int ret = 0;
	...
	bios_svc(0x34, init_mm.pgd, 0, 0, 0, 0);

#if defined(CONFIG_ARM) && LINUX_VERSION_CODE >= KERNEL_VERSION(3, 3, 0)
	bios_svc(0x34, idmap_pgd, 0, 0, 0, 0);
#endif

	if (atomic_read(&current->active_mm->mm_users) < 1) {
		bios_svc(0x34, current->active_mm->pgd, 0, 0, 0, 0);
	}

	spin_lock(&falcon_revert_lock);
	for_each_process_thread(p, t) {
		if(t->mm) {
			ret = add_revert_process(t->mm);
			if (ret)
				return ret;
		}
	}
	...
	for_each_possible_cpu(cpu) {
		...
		t = (&per_cpu(cpu_data, cpu))->idle;
#else
		t = get_cpu_idle_thread(cpu);
#endif
		if (t != NULL && t->mm) {
			ret = add_revert_process(t->mm);
			if (ret)
...
EXPORT_SYMBOL(prepare_revert_mmu);

#ifdef CONFIG_FALCON_CMA
#include <linux/dma-mapping.h>

dma_addr_t falcon_workmem_dmaaddr;
EXPORT_SYMBOL(falcon_workmem_dmaaddr);

void falcon_dma_free(void)
{
	printk(KERN_DEBUG "%s(%d) falcon_workmem_addr = %x falcon_workmem_dmaaddr = %x size = %x", __func__, __LINE__, falcon_workmem_addr, falcon_workmem_dmaaddr, falcon_workmem_size);
	dma_free_coherent(falcon_dev, falcon_workmem_size,
			  falcon_workmem_addr,
			  falcon_workmem_dmaaddr);
}
EXPORT_SYMBOL(falcon_dma_free);
#endif
...
#endif
{
	...
	ret = bios_svc(0x21, 0, 0, 0, 0, 0);

	if (idle_buff)
		if (memcmp(idle_buff + PAGE_SIZE - 4, &buf, 4))
			memcpy(idle_buff + PAGE_SIZE - 4, &buf, 4);

	if (!need_revert)
		return ret ? -EINVAL : 0;

	falcon_revert_process();

	ret = prepare_revert_mmu();
	if (ret)
		falcon_error = ret;

	while (!list_empty(&falcon_revert_list)) {
		struct falcon_revert_entry *revert;
		revert = list_first_entry(&falcon_revert_list, struct falcon_revert_entry, head);

		if (revert->mm){
			bios_svc(0x34, revert->mm->pgd, 0, 0, 0, 0);
			list_del(&revert->head);
			kfree(revert);
		}
	}
	need_revert = 0;

	bios_svc(0x33, pm_idle_func, 0, 0, 0, 0);
	falcon_free_work();
	return ret ? -EINVAL : 0;
}
...
EXPORT_SYMBOL(falcon_alloc_work);

void falcon_free_work(void)
{
}
EXPORT_SYMBOL(falcon_free_work);
