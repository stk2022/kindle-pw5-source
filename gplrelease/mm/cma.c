...
#ifdef CONFIG_FALCON
void falcon_get_cont_info(unsigned long *base_pfn, unsigned long *count)
{
	int i;

	for (i = 0; i < MAX_CMA_AREAS; i++) {
		base_pfn[i] = cma_areas[i].base_pfn;
		count[i] = cma_areas[i].count;
	}
}
EXPORT_SYMBOL(falcon_get_cont_info);
#endif