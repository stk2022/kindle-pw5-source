	...
	struct snd_soc_dai *dai)
{
	...
	dev_dbg(afe->dev, "%s [%d] input_fs 0x%x, output_fs 0x%x\n",
		__func__, dai->id, input_fs, output_fs);
	dev_dbg(afe->dev, "%s [%d] input_rate %d, output_rate %d\n",
		__func__, dai->id, input_rate, output_rate);
	dev_dbg(afe->dev, "%s [%d] gasrc_op_freq_45m %d\n",
		__func__, dai->id, *gasrc_op_freq_45m);
	...
	afe_priv->gasrc_data[gasrc_id].cali_tx = false;
	afe_priv->gasrc_data[gasrc_id].cali_rx = false;
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (mt8512_afe_gasrc_is_tx_tracking(output_fs))
			afe_priv->gasrc_data[gasrc_id].cali_tx = true;
		else
			afe_priv->gasrc_data[gasrc_id].cali_tx = false;
	} else if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		if (mt8512_afe_gasrc_is_rx_tracking(input_fs))
			afe_priv->gasrc_data[gasrc_id].cali_rx = true;
		else
			afe_priv->gasrc_data[gasrc_id].cali_rx = false;
	...
	dev_dbg(afe->dev, "%s [%d] cali_tx %d, cali_rx %d\n",
		__func__, dai->id,
		afe_priv->gasrc_data[gasrc_id].cali_tx,
		afe_priv->gasrc_data[gasrc_id].cali_rx);

	switch (mux) {
	case MUX_GASRC_8CH:
	case MUX_GASRC_6CH:
	case MUX_GASRC_4CH:
		afe_priv->gasrc_data[gasrc_id].one_heart = true;
		break;
	case MUX_GASRC_2CH:
		afe_priv->gasrc_data[gasrc_id].one_heart = false;
		break;
	default:
		afe_priv->gasrc_data[gasrc_id].one_heart = false;
		break;
	...
	if (afe_priv->gasrc_data[gasrc_id].one_heart &&
	...
	if (stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (afe_priv->gasrc_data[gasrc_id].cali_tx) {
		...
		mt8512_afe_gasrc_set_rx_mode_fs(afe,
			dai, input_rate, output_rate);
		if (afe_priv->gasrc_data[gasrc_id].cali_rx) {
			mt8512_afe_gasrc_sel_cali_clk(afe,
				dai, *gasrc_op_freq_45m);
...
static int mt8512_afe_enable_gasrc(struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8512_afe_private *afe_priv = afe->platform_priv;
	const int gasrc_id = mt8512_dai_num_to_gasrc(dai->id);
...
static int mt8512_afe_disable_gasrc(struct snd_soc_dai *dai)
{
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
	struct mt8512_afe_private *afe_priv = afe->platform_priv;
	const int gasrc_id = mt8512_dai_num_to_gasrc(dai->id);
	...
	struct snd_soc_dai *dai)
{
	...
	afe_priv->gasrc_data[gasrc_id].op_freq_45m = false;
	afe_priv->gasrc_data[gasrc_id].cali_cycles = 64;

	if (dai->capture_active && dai->playback_active)
		afe_priv->gasrc_data[gasrc_id].duplex = true;

	dev_info(afe->dev, "%s [%d] duplex %d\n", __func__,
		dai->id, afe_priv->gasrc_data[gasrc_id].duplex);

	if (afe_priv->gasrc_data[gasrc_id].duplex)
		mt8512_afe_disable_gasrc(dai);
	...
	int cmd, struct snd_soc_dai *dai)
{
	...
	dev_dbg(dai->dev, "%s %d cmd %d\n", __func__, dai->id, cmd);

	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		ret = mt8512_afe_enable_gasrc(dai);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		ret = mt8512_afe_disable_gasrc(dai);
		break;
	default:
		break;
	}
