...
static int es7243_read(u8 reg, u8 *rt_value,
...
static int es7243_update_bits(u8 reg, u8 mask,
	u8 value, struct i2c_client *client)
{
	u8 val_old, val_new;
...
static void es7243_init_codec(struct snd_soc_codec *codec, u8 i)
{
	...
	//dsp for tdm mode, DSP-A, 32BIT, bclk invert
	//es7243_write(ES7243_SDPFMT_REG01, 0xd3, i2c_clt[i]);
	//dsp for tdm mode, DSP-A, 16BIT, bclk invert
	es7243_write(ES7243_SDPFMT_REG01, 0xCF, i2c_clt[i]);
	...
	es7243_write(ES7243_ANACTL1_REG08, 0x43, i2c_clt[i]);
...
static void pcm_pop_work1_events(struct work_struct *work)
{
	...
	pr_info("es8316--------pcm_pop_work1_events\n");
...
static void pcm_pop_work_events(struct work_struct *work)
{
	...
	pr_info("es8316--------pcm_pop_work_events\n");
	...
	struct snd_soc_dai *dai)
{
	...
	pr_info("<es7243>rate=%d,format=%d\n",
		params_rate(params),  params_format(params));
	for (i = 0; i < ARRAY_SIZE(coeff_div); i++) {
		if (coeff_div[i].sr_rate == params_rate(params)
				&& es7243->sysclk == coeff_div[i].mclk) {
			for (i = 0; i < (ES7243_CHANNELS_MAX)/2; i++)
				es7243_update_bits(ES7243_MODECFG_REG00,
					0x60, coeff_div[i].speedmode << 5,
					i2c_clt[i]);
			}
	...
	if (es7243_init_reg == 0) {
#if 0
		pr_info(">>>>>>>>es8316_pcm_startup es8316_init_reg=0\n");
		//schedule_delayed_work(&es7243->pcm_pop_work,
			msecs_to_jiffies(100));
		schedule_delayed_work(&es7243->pcm_pop_work, 0);
		schedule_delayed_work(&es7243->pcm_pop_work1,
			msecs_to_jiffies(100));
#else
		pr_info("es7243 i2c init\n");
		for (i = 0; i < (ES7243_CHANNELS_MAX)/2; i++)
			es7243_init_codec(tron_codec[i], i);
		...
		for (i = 0; i < (ES7243_CHANNELS_MAX)/2; i++)
			es7243_unmute(
				tron_codec[(ES7243_CHANNELS_MAX/2) - 1 - i],
				(ES7243_CHANNELS_MAX/2) - 1 - i);
	...
	for (i = 0; i < (ES7243_CHANNELS_MAX)/2; i++) {
		//enter into hardware mode
		es7243_read(ES7243_MODECFG_REG00, &val, i2c_clt[i]);
		pr_info("%d -- Reg 00 ----> 0x%x!\n", __LINE__, val);
		//soft reset codec
		es7243_read(ES7243_STATECTL_REG06, &val, i2c_clt[i]);
		pr_info("%d -- Reg 06 ----> 0x%x!\n", __LINE__, val);
		//dsp for tdm mode, DSP-A, 16BIT
		es7243_read(ES7243_SDPFMT_REG01, &val, i2c_clt[i]);
		pr_info("%d -- Reg 01 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_LRCDIV_REG02, &val, i2c_clt[i]);
		pr_info("%d -- Reg 02 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_BCKDIV_REG03, &val, i2c_clt[i]);
		pr_info("%d -- Reg 03 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_CLKDIV_REG04, &val, i2c_clt[i]);
		pr_info("%d -- Reg 04 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_MUTECTL_REG05, &val, i2c_clt[i]);
		pr_info("%d -- Reg 05 ----> 0x%x!\n", __LINE__, val);
		//enable microphone input and pga gain for 27db
		es7243_read(ES7243_ANACTL1_REG08, &val, i2c_clt[i]);
		pr_info("%d -- Reg 08 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_ANACTL2_REG09, &val, i2c_clt[i]);
		pr_info("%d -- Reg 09 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_STATECTL_REG06, &val, i2c_clt[i]);
		pr_info("%d -- Reg 06 ----> 0x%x!\n", __LINE__, val);
		//power up adc and analog input
		es7243_read(ES7243_ANACTL0_REG07, &val, i2c_clt[i]);
		pr_info("%d -- Reg 07 ----> 0x%x!\n", __LINE__, val);
		es7243_read(ES7243_MUTECTL_REG05, &val, i2c_clt[i]);
		pr_info("%d -- Reg 05 ----> 0x%x!\n", __LINE__, val);
	}
	...
	for (i = 0; i < (ES7243_CHANNELS_MAX)/2; i++) {
		...
		case SNDRV_PCM_FORMAT_S16_LE:
			pr_info("es7243,16bit\n");
			es7243_update_bits(ES7243_SDPFMT_REG01, 0x1c, 0x0c,
				i2c_clt[i]);
		break;
		case SNDRV_PCM_FORMAT_S20_3LE:
			es7243_update_bits(ES7243_SDPFMT_REG01, 0x1c, 0x04,
				i2c_clt[i]);
			break;
		case SNDRV_PCM_FORMAT_S24_LE:
			es7243_update_bits(ES7243_SDPFMT_REG01, 0x1c, 0x00,
				i2c_clt[i]);
			break;
		case SNDRV_PCM_FORMAT_S32_LE:
			pr_info("es7243,32bit\n");
			es7243_update_bits(ES7243_SDPFMT_REG01, 0x1c, 0x10,
				i2c_clt[i]);
			break;
		}
...
#endif
	if (ret < 0) {
		dev_info(codec->dev, "Failed to set cache I/O: %d\n", ret);
		return ret;
	}
	pr_info("begin->>>>>>>>>>%s!\n", __func__);
...
{
	//pr_info("adc1 ucontrol->value.integer.value[0] = %ld",
...
static ssize_t es7243_store(struct device *dev,
...
static ssize_t es7243_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	pr_info("echo flag|reg|val > es7243\n");
	pr_info("eg read star address=0x06,count 0x10:echo 0610 >es7243\n");
	pr_info("eg write star address=0x90,value=0x3c,count=4:echo 4903c >es7243\n");
