...
#ifdef CONFIG_FALCON
	falcon_set_preload_range(runtime->buffer, runtime->buffer_size);
#endif
	return 0;
}
...
#ifdef CONFIG_FALCON
	falcon_clr_preload_range(runtime->buffer, runtime->buffer_size);
#endif
...
#ifdef CONFIG_FALCON
		falcon_clr_preload_range(runtime->buffer, runtime->buffer_size);
		falcon_set_preload_range(newbuf, params->buffer_size);
#endif
		spin_lock_irqsave(&runtime->lock, flags);
