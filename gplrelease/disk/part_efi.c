...
static int fix_gpt_tables(struct blk_desc *dev_desc, gpt_header *pgpt_head, gpt_entry *pgpt_pte)
{
	gpt_entry *gpt_e = pgpt_pte;
	int ret;
	int i;
	int last_part = -1;
	u64 ending_lba, last_ending_lba = 0;

	printf("GPT: Fixing Partition Table\n");
	/* make sure using full size of emmc in primary gpt */
	pgpt_head->my_lba = cpu_to_le64(1);
	pgpt_head->alternate_lba = cpu_to_le64(dev_desc->lba - 1);
	pgpt_head->first_usable_lba = cpu_to_le64(34);
	pgpt_head->last_usable_lba = cpu_to_le64(dev_desc->lba - 34);
	pgpt_head->partition_entry_lba = cpu_to_le64(2);
	pgpt_head->header_crc32 = 0;

	/* read gpt entry from disk if not provided */
	if (pgpt_pte == NULL) {
		gpt_e = alloc_read_gpt_entries(dev_desc, pgpt_head);
		if (gpt_e == NULL) {
			printf("GPT: Failed to get valid Partition Table Entries\n");
			return -1;
		}
	}
	...
	for (i = 0; i < le32_to_cpu(pgpt_head->num_partition_entries); i++) {
		if (!is_pte_valid(&gpt_e[i]))
			continue;
		ending_lba = le64_to_cpu(gpt_e[i].ending_lba);
		if (ending_lba > last_ending_lba) {
			last_ending_lba = ending_lba;
			last_part = i;
		}
	}
	...
	/* extend the last partition to maximuim */
	if (last_part < 0) {
		printf("GPT: Cannot found last partition\n");
		return -1;
	}
	printf("GPT: Extend last partition from 0x%llx to 0x%llx\n",
			last_ending_lba, pgpt_head->last_usable_lba);
	gpt_e[last_part].ending_lba = pgpt_head->last_usable_lba;

	ret = write_gpt_table(dev_desc, pgpt_head, gpt_e);
	if (pgpt_pte == NULL) free(gpt_e);
	return ret;
}
...
int write_mbr_and_gpt_partitions(struct blk_desc *dev_desc, void *buf)
{
	...
	if (fix_gpt_tables(dev_desc, gpt_h, gpt_e)) {
		printf("%s: failed writing GPT table\n", __func__);
		return 1;
	}
			...
			gpt_header *pgpt_head, gpt_entry **pgpt_pte)
{
	...
	/* XXX: generated gpt table have some issues, correct it at runtime */
	/* Check and fix GPT header and extend last partition */
	if (lba == GPT_PRIMARY_PARTITION_TABLE_LBA &&
			(pgpt_head->alternate_lba != cpu_to_le64(dev_desc->lba - 1) || pgpt_head->partition_entry_lba != 2)) {
		fix_gpt_tables(dev_desc, pgpt_head, NULL);
	}
