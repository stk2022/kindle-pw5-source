...
#ifdef CONFIG_FALCON
	if (lvl == 1)
		falcon_set_preload(virt_to_phys(table), 1 << get_order(size));
#endif
...
#ifdef CONFIG_FALCON
	if (lvl == 1)
		falcon_clr_preload(virt_to_phys(table), 1 << get_order(size));
#endif
...
#ifdef CONFIG_FALCON
static void falcon_ctor(void *p)
{
	falcon_set_preload(virt_to_phys(p), 1);
}
#endif
