...
static int hall_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_IDME
	if (idme_hwid_value >= 2 && idme_hwid_value <=4) {
		pr_info("eink_hall not supported by HW build:%d\n",idme_hwid_value);
		return -EFAULT;
	}
#endif

	pr_info("%s probed\n", __func__);
	ddata = devm_kzalloc(&pdev->dev, sizeof(struct hall_drvdata), GFP_KERNEL);
	if (!ddata) {
	...
	np = of_find_compatible_node(NULL, NULL, "amzn,hall");
	if (!np) {
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: get hall_int_n GPIO failed err=%d\n", __func__, ret);
		ret = -EINVAL;
		goto err_free_ddata;
	}
...
static struct platform_driver eink_hall = {
	.driver		= {
...
static int __init hall_init(void)
{
	...
	platform_driver_register(&eink_hall);
	return 0;
}

static void __exit hall_exit(void)
{
	platform_driver_unregister(&eink_hall);
}
