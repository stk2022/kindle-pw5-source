...
struct mtk_i2c {
	...
	bool trans_addr_only;           /* I2C transfer slave address only */
...
static inline u16 dma_readw(struct mtk_i2c *i2c, u8 offset)
{
	return readw(i2c->pdmabase + offset);
}
...
static void i2c_dump_info(struct mtk_i2c *i2c)
{
	...
	dev_err(i2c->dev,
		...
		(i2c_readw(i2c, OFFSET_FIFO_STAT1)),
	...
	dev_err(i2c->dev,
		"I2C register:\nRSV_DEBUG %x\nSOFTRESET %x\n",
		(i2c_readw(i2c, OFFSET_RSV_DEBUG)),
	...
	dev_err(i2c->dev,
		"I2C register:\nFIFO_THRESH %x\nFIFO_ADDR_CLR %x\nDCM_EN %x\n",
		(i2c_readw(i2c, OFFSET_FIFO_THRESH)),
		(i2c_readw(i2c, OFFSET_FIFO_ADDR_CLR)),
		(i2c_readw(i2c, OFFSET_DCM_EN)));
	dev_err(i2c->dev,
		"I2C register:\nDEBUGCTRL %x\nTRANSFER_LEN_AUX %x\nCLOCK_DIV %x\n",
		(i2c_readw(i2c, OFFSET_DEBUGCTRL)),
		(i2c_readw(i2c, OFFSET_TRANSFER_LEN_AUX)),
		(i2c_readw(i2c, OFFSET_CLOCK_DIV)));

	dev_err(i2c->dev,
		"DMA register:\nINT_FLAG %x\nINT_EN %x\nCON %x\n",
		(dma_readw(i2c, OFFSET_INT_FLAG)),
		(dma_readw(i2c, OFFSET_INT_EN)),
		(dma_readw(i2c, OFFSET_CON)));
	dev_err(i2c->dev,
		"DMA register:\nTX_MEM_ADDR  %x\nRX_MEM_ADDR %x\nTX_LEN %x\n RX_LEN %x\n",
		(dma_readw(i2c, OFFSET_TX_MEM_ADDR)),
		(dma_readw(i2c, OFFSET_RX_MEM_ADDR)),
		(dma_readw(i2c, OFFSET_TX_LEN)),
        (dma_readw(i2c, OFFSET_RX_LEN)));
}
			       ...
			       int num, int left_num)
{
	...
	if(i2c->trans_addr_only){
		writew(addr_reg | 0x100, i2c->base + OFFSET_SLAVE_ADDR);
	...
	if (!i2c->trans_addr_only) {
		...
		if (i2c->op == I2C_MASTER_RD) {
			msg_buf_r = kzalloc(msgs->len, GFP_KERNEL);
			if (msg_buf_r == NULL)
				return -ENOMEM;

			writel(I2C_DMA_INT_FLAG_NONE, i2c->pdmabase + OFFSET_INT_FLAG);
			writel(I2C_DMA_CON_RX, i2c->pdmabase + OFFSET_CON);
			rpaddr = dma_map_single(i2c->dev, msg_buf_r,
			...
			msg_buf_w = kzalloc(msgs->len, GFP_KERNEL);
			if (msg_buf_w == NULL)
				return -ENOMEM;
			memcpy(msg_buf_w, msgs->buf, msgs->len);

			writel(I2C_DMA_INT_FLAG_NONE, i2c->pdmabase + OFFSET_INT_FLAG);
			writel(I2C_DMA_CON_TX, i2c->pdmabase + OFFSET_CON);
			wpaddr = dma_map_single(i2c->dev, msg_buf_w,
			...
			if (msg_buf_w == NULL)
				return -ENOMEM;
			msg_buf_r = kzalloc((msgs+1)->len, GFP_KERNEL);
			if (msg_buf_r == NULL) {
				kfree(msg_buf_w);
				return -ENOMEM;
			}
	...
	if (!i2c->trans_addr_only) {
		if (i2c->op == I2C_MASTER_WR) {
			...
			dma_unmap_single(i2c->dev, rpaddr,
					 msgs->len, DMA_FROM_DEVICE);
			memcpy(msgs->buf, msg_buf_r, msgs->len);
			kfree(msg_buf_r);
		} else {
			...
			dma_unmap_single(i2c->dev, rpaddr, (msgs + 1)->len,
					 DMA_FROM_DEVICE);
			memcpy((msgs+1)->buf, msg_buf_r, (msgs+1)->len);
			kfree(msg_buf_w);
			kfree(msg_buf_r);
		}
	}
...
static const struct dev_pm_ops mt_i2c_ops = {
	SET_NOIRQ_SYSTEM_SLEEP_PM_OPS(mt_i2c_suspend_noirq,
				      mt_i2c_resume_noirq)
};
