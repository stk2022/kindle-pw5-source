...
cmdqBackupSlotHandle h_backup_slot = 0;

void debug_cmdq_slot_test(int argc, char *argv[])
{
	struct cmdqRecStruct *handle;
	//u32 gce_reg_event_id = 0x10238000 + 0x60;
	//u32 gce_reg_event_value = 0x10238000 + 0x64;

	if (h_backup_slot == 0) {
		TCON_ERR("alloc backup slot");
		cmdqBackupAllocateSlot(&h_backup_slot, 10);
	} else {
		TCON_ERR("already started");
		return;
	}
	...
	cmdqRecCreate(CMDQ_SCENARIO_HWTCON_LUT_END_LOOP,
		&handle);
	cmdqRecReset(handle);
	cmdqRecClearEventToken(handle, CMDQ_EVENT_DPI_UPDATE_DONE);
	cmdqRecClearEventToken(handle, CMDQ_EVENT_LUT_FRAME_DONE);		// slot 0
	cmdqRecClearEventToken(handle, CMDQ_EVENT_TCON_END);	// slot 1

	cmdqRecWait(handle, CMDQ_EVENT_LUT_FRAME_DONE);
	cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 0, 0x14006040);

	cmdqRecWait(handle, CMDQ_EVENT_TCON_END);
	cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 1, 0x14006040);


	#if 0
	pp_write_mask(handle, 0x1400700C, 0x2 << 20, GENMASK(23, 20));

	pp_write_mask(handle, 0x1400D004, 5 << 16, GENMASK(19, 16));
	//value0 = pp_read(hwtcon_device_info()->mmsys_reg_va + 0xD0C0);
	cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 0, hwtcon_device_info()->mmsys_reg_pa + 0xD0C0);

	pp_write_mask(handle, 0x1400D004, 1 << 16, GENMASK(19, 16));
	//value1 = pp_read(hwtcon_device_info()->mmsys_reg_va + 0xD0C0);
	cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 1, hwtcon_device_info()->mmsys_reg_pa + 0xD0C0);
	#endif

	#if 0
	cmdqRecWrite(handle, gce_reg_event_id, CMDQ_EVENT_LUT_FRAME_DONE, 0xFFFFFFFF);
	cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 0, gce_reg_event_value);

	cmdqRecWrite(handle, gce_reg_event_id, CMDQ_EVENT_WF_LUT_FRAME_DONE, 0xFFFFFFFF);
	cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 1, gce_reg_event_value);
	#else
	//cmdqRecWrite(handle, gce_reg_event_id, 19, 0xFFFFFFFF);
	//cmdqRecBackupRegisterToSlot(handle, h_backup_slot, 0, gce_reg_event_value);

	#endif
	
	cmdqRecStartLoop(handle);
}
...
void debug_dump_power_status(int argc, char *argv[])
{
	...
	if (hwtcon_core_get_task_count(&hwtcon_fb_info()->collision_task_list.list) != 0) {
		struct hwtcon_task *task, *tmp;

		list_for_each_entry_safe(task, tmp,
			&hwtcon_fb_info()->collision_task_list.list, list) {
			TCON_ERR("region:[%d %d %d %d] lut_dependency:0x%016llx",
...
{
	g_debug_info.enable_dump_next_buffer = true;

	TCON_LOG("enable dump buffer");
}
...
void debug_dump_image_buffer(int argc, char *argv[])
{
	...
	if (enable && !hwtcon_fb_info()->debug_img_buffer_available) {
		if (hwtcon_fb_alloc_debug_img_buffer(hwtcon_fb_info())) {
			TCON_WARN("fail to allocate debug image buffer");
			g_debug_info.enable_dump_image_buffer = false;
		}
	}
...
void debug_save_image_buffer_to_file(int argc, char *argv[])
{
	...
	if (!hwtcon_fb_info()->debug_img_buffer_available) {
		TCON_WARN("debug image buffer not available");
		return;
	}
	for (i = 0; i < MAX_DEBUG_IMAGE_BUFFER_COUNT; i++) {
		if (hwtcon_fb_info()->debug_img_buffer_name[i][0] != 0)
			hwtcon_file_save_buffer(hwtcon_fb_info()->debug_img_buffer_va[i],
...
{
	TCON_ERR("fb buf[%p 0x%08x] img buf[%p 0x%08x] temp img buf[%p 0x%08x] wb buf[%p 0x%08x] wf_file[%p 0x%08x]",
	...
	TCON_ERR("dump wf_file info ready[%d] file_name[%s] mode_version[0x%x] wf_type[0x%x] wfm_rev[%d]",
...
void debug_enable_mitigation(int argc, char *argv[])
{
	int value = 0;
	...
	g_debug_info.sw_mitigation = value;
}
...
static struct proc_write_info g_proc_write[] = {
		...
		{"sw_mitigation", debug_enable_mitigation},
};
...
int hwtcon_debug_read(struct inode *inode, struct file *file)
{
	...
	if (strncmp(g_proc_read[i].proc_node_name, "temp",
		...
		if (hwtcon_device_info()->reserved_buf_ready)
			hwtcon_debug_reserved_buffer_info_printf("0x%08x 0x%08x",
				hwtcon_device_info()->reserved_buf_pa,
				hwtcon_device_info()->reserved_buf_pa + WAVEFORM_SIZE);
	}
