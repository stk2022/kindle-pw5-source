...
{
	struct tcon_pin_swap_para global_config = {0};

	global_config.sdec = 0x8;
	global_config.sdoe = 0xA;
	global_config.gdclk = 0xB;
	//global_config.sdle = SDLE_SOURCE;
	global_config.gdoe = 0x9;

	pp_write_mask(pkt, PAPER_TCTOP_PIN_SWAP3,
		global_config.sdec << 6,
		GENMASK(11, 6));
	pp_write_mask(pkt, PAPER_TCTOP_PIN_SWAP3,
		global_config.sdoe << 18,
		GENMASK(23, 18));
	pp_write_mask(pkt, PAPER_TCTOP_PIN_SWAP3,
		global_config.gdclk << 24,
		GENMASK(29, 24));
	pp_write_mask(pkt, PAPER_TCTOP_PIN_SWAP3,
		global_config.sdle << 12,
		GENMASK(17, 12));
...
void tcon_setting(struct cmdqRecStruct *pkt)
{
	...
	//tcon_config_timing4(pkt, &time4_para);
