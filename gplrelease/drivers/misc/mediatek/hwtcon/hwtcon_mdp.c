...
/* check whether this task need to use mdp to do dither & etc. */
...
int hwtcon_mdp_convert(struct hwtcon_task *task)
{
	...
	TCON_LOG("MDP convert, dither:%d invert:%d rotate:%d degree gamma:0x%08x",
		dither_enable, invert_enable, rotate, gamma_flag);
	if (rotate) {
		TCON_LOG("src width:%d height:%d region:[%d %d %d %d]",
			src_buffer_width, src_buffer_height,
			src_region.x,
			src_region.y,
			src_region.width,
			src_region.height);
		TCON_LOG("dst width:%d height:%d region:[%d %d %d %d]",
			dst_buffer_pitch, dst_buffer_height,
			dst_region.x,
			dst_region.y,
			dst_region.width,
