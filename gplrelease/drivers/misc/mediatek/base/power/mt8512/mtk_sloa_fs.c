...
/* PWR_STATUS_2ND */
#define PWR_STA_DISP			BIT(15)
#define PWR_STA_IMAGE			BIT(16)
...
enum pwr_ctrl_enum {
	...
	PW_INFRA4_IDLE_MASK,
	PW_MMSYS0_IDLE_MASK,
	PW_MMSYS_OTH_IDLE_MASK,
	PW_IMAGE0_IDLE_MASK,
	PW_IMAGE1_IDLE_MASK,
	PW_IMAGE_OTH_IDLE_MASK,
	PW_INFRA1_IDLE_RG,
	PW_INFRA0_IDLE_RG,
	PW_INFRA2_IDLE_RG,
	PW_INFRA3_IDLE_RG,
	PW_INFRA4_IDLE_RG,
	PW_MMSYS0_IDLE_RG,
	PW_MMSYS_OTH_IDLE_RG,
	PW_IMAGE0_IDLE_RG,
	PW_IMAGE1_IDLE_RG,
	PW_IMAGE_OTH_IDLE_RG,
	PW_LOG_EN,
...
static void spm_output_wake_reason(const struct wake_status *wakesta)
{
	...
	if (wakesta->assert_pc != 0) {
		pr_info("PCM ASSERT AT %u, timeout = %u, r13 = 0x%x\n",
		       wakesta->assert_pc, wakesta->timer_out, wakesta->r13);
		goto spm_debug_flags;
	}

	for (i = 0; i <= 31; i++) {
		if (wakesta->r12 & (1U << i)) {
			pr_notice("wake up by %s, timeout = %u, r13 = 0x%x\n",
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	idle_masks[CG_MMSYS_OTH] = SMC_CALL(GET_PWR_CTRL_ARGS,
				SPM_PWR_CTRL_DPIDLE, PW_MMSYS_OTH_IDLE_MASK, 0);
	idle_masks[CG_IMAGE0] = SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				       PW_IMAGE0_IDLE_MASK, 0);
	idle_masks[CG_IMAGE1] = SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				       PW_IMAGE1_IDLE_MASK, 0);
	idle_masks[CG_IMAGE_OTH] = SMC_CALL(GET_PWR_CTRL_ARGS,
				SPM_PWR_CTRL_DPIDLE, PW_IMAGE_OTH_IDLE_MASK, 0);
	...
	if (readl(PWR_STATUS_2ND) & PWR_STA_DISP) {
		clks[CG_MMSYS0] = ~SMC_CALL(GET_PWR_CTRL_ARGS,
					    SPM_PWR_CTRL_DPIDLE,
					    PW_MMSYS0_IDLE_RG, 0);
		clks[CG_MMSYS_OTH] = ~SMC_CALL(GET_PWR_CTRL_ARGS,
					    SPM_PWR_CTRL_DPIDLE,
					    PW_MMSYS_OTH_IDLE_RG, 0);
	}

	if (readl(PWR_STATUS_2ND) & PWR_STA_IMAGE) {
		clks[CG_IMAGE0] = ~SMC_CALL(GET_PWR_CTRL_ARGS,
				SPM_PWR_CTRL_DPIDLE, PW_IMAGE0_IDLE_RG, 0);
		clks[CG_IMAGE1] = ~SMC_CALL(GET_PWR_CTRL_ARGS,
				SPM_PWR_CTRL_DPIDLE, PW_IMAGE1_IDLE_RG, 0);
		clks[CG_IMAGE_OTH] = ~SMC_CALL(GET_PWR_CTRL_ARGS,
				SPM_PWR_CTRL_DPIDLE, PW_IMAGE_OTH_IDLE_RG, 0);
	}
				 ...
				 size_t count, loff_t *f_pos)
{
	...
	clks[CG_IMAGE0] = ~SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				  PW_IMAGE0_IDLE_RG, 0);
	clks[CG_IMAGE1] = ~SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				  PW_IMAGE1_IDLE_RG, 0);
	clks[CG_IMAGE_OTH] = ~SMC_CALL(GET_PWR_CTRL_ARGS, SPM_PWR_CTRL_DPIDLE,
				   PW_IMAGE_OTH_IDLE_RG, 0);
...
int sloa_fs_init(void)
{
	...
	const u8 *compatible_node = "mediatek,mt8512-scpsys";
	static const char *d = "/sys/kernel/debug/cpuidle";
	int ret;
