...
static unsigned long claim_mtk_svs_lock(void)
{
	unsigned long flags;
...
static void release_mtk_svs_lock(unsigned long flags)
{
...
static void svs_get_vops_v1(struct mtk_svs *svs)
{
	struct svs_bank *svsb = svs->bank;
	u32 temp;

	temp = svs_readl(svs, VOP30);
	svsb->volts[3] = (temp >> 24) & 0xff;
	svsb->volts[2] = (temp >> 16) & 0xff;
	svsb->volts[1] = (temp >> 8)  & 0xff;
	svsb->volts[0] = (temp & 0xff);

	temp = svs_readl(svs, VOP74);
	svsb->volts[7] = (temp >> 24) & 0xff;
	svsb->volts[6] = (temp >> 16) & 0xff;
	svsb->volts[5] = (temp >> 8)  & 0xff;
	svsb->volts[4] = (temp & 0xff);
}

static void svs_set_freqs_pct_v1(struct mtk_svs *svs)
{
	struct svs_bank *svsb = svs->bank;

	svs_writel(svs,
		   ((svsb->freqs_pct[3] << 24) & 0xff000000) |
		   ((svsb->freqs_pct[2] << 16) & 0xff0000) |
		   ((svsb->freqs_pct[1] << 8) & 0xff00) |
		   (svsb->freqs_pct[0] & 0xff),
		   FREQPCT30);
	svs_writel(svs,
		   ((svsb->freqs_pct[7] << 24) & 0xff000000) |
		   ((svsb->freqs_pct[6] << 16) & 0xff0000) |
		   ((svsb->freqs_pct[5] << 8) & 0xff00) |
		   ((svsb->freqs_pct[4]) & 0xff),
		   FREQPCT74);
}
...
static int svs_mt8512_efuse_parsing(struct mtk_svs *svs)
{
	const struct svs_platform *svsp = svs->platform;
	struct svs_bank *svsb;
	u32 idx, i, ft_pgm;
	...
	if (svsp->fake_efuse) {
		...
		svs->efuse[0] = 0x0000ff00;
		svs->efuse[1] = 0x51c65116;
		svs->efuse[2] = 0x20f70000;
	...
	for (idx = 0; idx < svsp->bank_num; idx++) {
		...
		switch (svsb->sw_id) {
		case SVS_CPU_LITTLE:
			svsb->bdes = (svs->efuse[1] >> 16) & 0xff;
			svsb->mdes = (svs->efuse[1] >> 24) & 0xff;
			svsb->dcbdet = (svs->efuse[2] >> 16) & 0xff;
			svsb->dcmdet = (svs->efuse[2] >> 24) & 0xff;
			svsb->mtdes  = svs->efuse[1] & 0xff;
			break;
		default:
			pr_err("unknown sw_id = %u?\n", svsb->sw_id);
			return -EINVAL;
		}
	}
...
struct svs_bank_ops svs_mt8512_banks_ops = {
	.set_freqs_pct	= svs_set_freqs_pct_v2,
	.get_vops	= svs_get_vops_v2,
};

struct svs_bank svs_mt8512_banks[1] = {
	{
		.of_compatible		= "mediatek,mt8512-svs-cpu-little",
		.sw_id			= SVS_CPU_LITTLE,
		.hw_id			= 0,
		.ops			= &svs_mt8512_banks_ops,
		.zone_name		= "tzts1",
		.buck_name		= "proc",
...
static const struct svs_platform svs_mt8512_platform = {
	.name		= "mt8512-svs",
	.banks		= svs_mt8512_banks,
	.efuse_parsing	= svs_mt8512_efuse_parsing,
	.regs		= svs_regs_v2,
	.fake_efuse	= false,
	.bank_num	= 1,
	.efuse_num	= 8,
...
struct svs_bank_ops svs_mt8110_banks_ops = {
	.set_freqs_pct	= svs_set_freqs_pct_v1,
	.get_vops	= svs_get_vops_v1,
};

struct svs_bank svs_mt8110_banks[1] = {
	{
		.of_compatible		= "mediatek,mt8110-svs-cpu-little",
		.sw_id			= SVS_CPU_LITTLE,
		.hw_id			= 0,
		.ops			= &svs_mt8110_banks_ops,
		.zone_name		= "tzts1",
		.buck_name		= "proc",
		.mtcmos_request		= false,
		.init01_volt_flag	= SVS_INIT01_VOLT_INC_ONLY,
...
static const struct svs_platform svs_mt8110_platform = {
	.name		= "mt8110-svs",
	.banks		= svs_mt8110_banks,
	.efuse_parsing	= svs_mt8512_efuse_parsing,
	.regs		= svs_regs_v2,
	.fake_efuse	= false,
	.bank_num	= 1,
...
static const struct of_device_id mtk_svs_of_match[] = {
	{
		...
		.data = &svs_mt8512_platform,
	}, {
		/* sentinel */
		.compatible = "mediatek,mt8110-svs",
		.data = &svs_mt8110_platform,
	},
};
