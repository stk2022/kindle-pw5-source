				...
				struct mdp_job_mapping *mapping_job)
{
	...
	if (mapping_job->handle_count >= MAX_HANDLE_NUM) {
		CMDQ_ERR("%s mapping_job->handle_count >= MAX_HANDLE_NUM\n", __func__);
		return 0;
	}
			...
			struct cmdqRecStruct *handle)
{
	...
	case CMDQ_MOP_WRITE_SEC:
	{
		...
		#else
		CMDQ_ERR("Secure path is not support\n");
		#endif
		break;
	}
	case CMDQ_MOP_NOP:
			...
			struct cmdqRecStruct *handle)
{
	...
	for (i = 0; i < user_job->meta_count; i++) {
		...
		if (unlikely(status < 0)) {
			...
			CMDQ_ERR("op:%d event:%d\n", metas[i].op, metas[i].event);
			break;
		}
	}
#ifdef TRACK_PERF_MON
	...
	unsigned long param)
{
	...
	default:
		...
		CMDQ_ERR("err cmd:0x%08x dir:0x%x type:0x%x nr:0x%x size:0x%x",
			code,
			_IOC_DIR(code),
			_IOC_TYPE(code),
			_IOC_NR(code),
			_IOC_SIZE(code));
		CMDQ_ERR("submit cmd:0x%08x dir:0x%x type:0x%x nr:0x%x size:0x%x",
			CMDQ_IOCTL_ASYNC_EXEC,
			_IOC_DIR(CMDQ_IOCTL_ASYNC_EXEC),
			_IOC_TYPE(CMDQ_IOCTL_ASYNC_EXEC),
			_IOC_NR(CMDQ_IOCTL_ASYNC_EXEC),
			_IOC_SIZE(CMDQ_IOCTL_ASYNC_EXEC));
		return -ENOIOCTLCMD;
	}
