...
#define KTCH_BOOST_DELAY_TIME 2000
...
static int ktch_mgr_time_delay = KTCH_BOOST_DELAY_TIME;
static struct pm_qos_request emi_request;
static struct pm_qos_request idle_request;
static struct delayed_work disable_work;
/*--------------------FUNCTION----------------*/
...
void set_freq(int enable, int core, int freq)
{
	...
	if (enable == 1) {
		...
		pm_qos_update_request(&emi_request, VCORE_OPP_0);
		pm_qos_update_request(&idle_request, 10);
		cancel_delayed_work_sync(&disable_work);
	} else if(enable == 0) {
		freq_to_set[targetclu].min = freq;
		mod_delayed_work(system_wq, &disable_work, msecs_to_jiffies(ktch_mgr_time_delay));
	} else if(enable == -1) {
		pm_qos_update_request(&emi_request, VCORE_OPP_UNREQ);
		pm_qos_update_request(&idle_request, 1900);
	}
...
static int ktchboost_thread(void *ptr)
{
	...
	while (!kthread_should_stop()) {
		...
		event = (ktchboost.touch_event > 0) ? 1: 0;
		core = ktch_mgr_core;
		freq = ktch_mgr_freq;
		...
		unsigned int code, int value)
{
	...
	if ((type == EV_ABS) && (code == ABS_MT_TRACKING_ID)) {
		pr_debug("input cb, type:%d, code:%d, value:%d\n",
				type, code, value);
		spin_lock_irqsave(&ktchboost.touch_lock, flags);
		ktchboost.touch_event += (value != -1)? 1 : -1;
		spin_unlock_irqrestore(&ktchboost.touch_lock, flags);

		atomic_inc(&ktchboost.event);
		wake_up_interruptible(&ktchboost.wq);
	}
}
...
static void ktch_disable_work(struct work_struct *work)
{
	set_freq(-1, 0, 0);
}

static ssize_t perfmgr_tb_duration_write(struct file *filp, const char *ubuf,
		size_t cnt, loff_t *data)
{
	char buf[64];
	unsigned long val;
	...
	ktch_mgr_time_delay = val;

	return cnt;
}

static int perfmgr_tb_duration_show(struct seq_file *m, void *v)
{
	if (m)
		seq_printf(m, "delay boost: %d ms\n", ktch_mgr_time_delay);
	return 0;
}

static int perfmgr_tb_duration_open(struct inode *inode, struct file *file)
{
	return single_open(file, perfmgr_tb_duration_show, inode->i_private);
}

static const struct file_operations perfmgr_tb_duration_fops = {
	.open =	perfmgr_tb_duration_open,
	.write = perfmgr_tb_duration_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
int init_ktch(struct proc_dir_entry *parent)
{
	...
	struct proc_dir_entry *tbe_dir, *tbc_dir, *tbf_dir, *tbclstr_dir, *tbduration_dir;
	int handle;
	...
	pm_qos_add_request(&emi_request, PM_QOS_DDR_OPP, PM_QOS_VCORE_OPP_DEFAULT_VALUE);
	pm_qos_add_request(&idle_request, PM_QOS_CPU_DMA_LATENCY, 1900);

	INIT_DELAYED_WORK(&disable_work, ktch_disable_work);
	...
	tbduration_dir = proc_create("tb_duration", 0644, ktch_root,
			&perfmgr_tb_duration_fops);
	if (!tbduration_dir)
		pr_debug("tbduration_dir not create\n");
