...
struct pwm_spec_config irtx_pwm_config = {
	.pwm_no = PWM3, /* using pwm2 as irtx output*/
...
void switch_irtx_gpio(int mode)
{
	...
	if (IS_ERR(pins_irtx)) {
		pr_notice("[IRTX][PinC]%s pinctrl_lockup(%p, %s) fail!!\n",
			__func__, ppinctrl_irtx, irtx_gpio_cfg[mode]);
		pr_notice("[IRTX][PinC]ppinctrl:%p, err:%ld\n",
			pins_irtx, PTR_ERR(pins_irtx));
		return;
	}
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	ret = put_ir_device();
	if (ret) {
		pr_notice("[IRTX] device close without open\n");
		goto exit;
	}
	...
	unsigned long arg)
{
	...
	case IRTX_IOC_SET_IRTX_LED_EN:
		if (copy_from_user(&para, (void __user *)arg,
			sizeof(unsigned int))) {
			pr_notice("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: ");
			...
			en = (para & 0xF);
			pr_info("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: ");
			pr_info("0x%x, gpio_id:%ul, en:%ul\n",
				para, gpio_id, en);
			if (en)
		...
		break;
	default:
		pr_notice("[IRTX] unknown ioctl cmd 0x%x\n", cmd);
		ret = -ENOTTY;
		break;
	}
	return ret;
	...
	const char __user *buf, size_t count, loff_t *ppos)
{
	...
	wave_vir = dma_alloc_coherent(&mt_irtx_dev.plat_dev->dev,
		count, &wave_phy, GFP_KERNEL | GFP_DMA);
	if (!wave_vir) {
		pr_notice("[IRTX] alloc memory fail\n");
		return -ENOMEM;
	}
	ret = copy_from_user(wave_vir, buf, count);
	if (ret) {
		pr_notice("[IRTX] write, copy from user fail %d\n", ret);
		goto exit;
	}
	/* invert bit */
	...
	irtx_pwm_config.PWM_MODE_MEMORY_REGS.BUF0_SIZE =
...
exit:
	...
	dma_free_coherent(&mt_irtx_dev.plat_dev->dev, count,
		wave_vir, wave_phy);
...
static int irtx_probe(struct platform_device *plat_dev)
{
	...
	if (plat_dev->dev.of_node == NULL) {
		pr_notice("[IRTX] irtx OF node is NULL\n");
		return -1;
	}
	...
	if (IS_ERR(mt_irtx_dev.ppinctrl_irtx)) {
		pr_notice("[IRTX][PinC]cannot find pinctrl. ptr_err:%ld\n",
	...
	if (!major) {
		...
		if (ret) {
			pr_notice("[IRTX] alloc_chrdev_region fail ret=%d\n",
				ret);
			goto exit;
		} else {
		...
		if (ret) {
			pr_notice("[IRTX] register_chrdev_region fail ret=%d\n",
				ret);
			goto exit;
		}
	}
	...
	if (ret) {
		pr_notice("[IRTX] cdev_add fail ret=%d\n", ret);
		goto exit;
	}
	...
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
		pr_notice("[IRTX] device_create fail ret=%d\n", ret);
		goto exit;
	}
...
static int __init irtx_init(void)
{
	...
	if (ret) {
		pr_notice("[IRTX] irtx platform driver register fail %d\n",
			ret);
		goto exit;
	}
