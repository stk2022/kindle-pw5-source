...
#else
	bufp = ioremap(CONFIG_MTK_RAM_CONSOLE_ADDR,
...
{
	static struct kedump_reserved_buffer *buffer = NULL;
...
#ifdef CONFIG_MTK_AEE_SAVE_DEBUGINFO_RESERVED_BUFFER
		entry = proc_create("last_debug_info", 0444, NULL, &kedump_console_file_ops);
		if (!entry) {
