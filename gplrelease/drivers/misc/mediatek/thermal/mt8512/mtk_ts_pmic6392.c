...
static s32 thermal_cal_exec(u32 ret)
{
	...
	mtktspmic_dprintk("[thermal_cal_exec] ret=%d, y_curr=%d\n", ret,
			  y_curr);
	mtktspmic_dprintk(
		"[thermal_cal_exec] data =%d, slope_data=%d, VBE_T=%d\n",
...
static int mtktspmic_get_hw_temp(void)
{
	...
	mtktspmic_dprintk(
		"[mtktspmic_get_hw_temp] PMIC_IMM_GetOneChannel 4=%d, T=%d\n",
		temp, temp1);
...
int mtktspmic_register_cooler(void)
{
	cl_dev_sysrst = mtk_thermal_cooling_device_register(
		"mtktspmic-sysrst", NULL, &mtktspmic_cooling_sysrst_ops);
	return 0;
}
...
static ssize_t mtktspmic_write(struct file *file, const char __user *buffer,
...
static void mtktspmic_shutdown(struct platform_device *pdev)
{
	mtktspmic_remove(pdev);
}

static struct platform_driver mtk_thermal_pmic_driver = {
	.probe = mtktspmic_probe,
	.remove = mtktspmic_remove,
	.shutdown = mtktspmic_shutdown,
	.driver = {
