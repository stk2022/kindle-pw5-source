 ...
 *  Users can set 4 different SMA len for a thermal zone and assign a high
 *threshold for each.
...
struct mtk_thermal_cooler_data {
	...
	void *devdata;
	int trip;
	char conditions[MTK_THERMAL_MONITOR_COOLER_MAX_EXTRA_CONDITIONS]
		       [THERMAL_NAME_LENGTH];
...
static struct proc_dir_entry
	*proc_cooler_dir_entry; /* lock by MTM_COOLER_PROC_DIR_LOCK */
static struct proc_dir_entry
	*proc_tz_dir_entry; /* lock by MTK_TZ_PROC_DIR_LOCK */
static struct proc_dir_entry *proc_drv_therm_dir_entry;
/**
 *  write to nBattCurrentCnsmpt, nCPU0_usage, and nCPU1_usage are locked by
 * MTM_SYSINFO_LOCK
 */
...
/* For enabling time based thermal protection under phone call+AP suspend
 * scenario.
 */
...
static int mtk_thermal_monitor_suspend(struct platform_device *dev,
				       pm_message_t state)
{
	...
	if (g_mtm_phone_call_ongoing) {
		...
		if (timeout > 0) {
			...
			if (gpt_remaining_cnt > 0 &&
			    gpt_remaining_cnt <= (timeout * 13000000))
		...
		if (g_mtm_phone_call_ongoing) {
			...
			if (!wake_lock_active(&mtm_wake_lock) &&
				...
				THRML_ERROR_LOG(
					...
					__func__, gpt_counting, GPT5_cmp,
					GPT5_cnt);
...
static int mtk_thermal_get_tz_idx(char *type)
{
	...
	else if (strncmp(type, "mtktspcb3", 9) == 0)
		return MTK_THERMAL_SENSOR_PCB3;
...
recoveryClientCooler(struct thermal_cooling_device *cdev,
...
static struct thermal_zone_device_ops *
getClientZoneOps(struct thermal_zone_device *zdev)
{
...
static void _mtm_update_sysinfo(struct work_struct *work)
{
	if (true == enable_ThermalMonitor)
		...
		mask |= (g_check_cpu_info_flag == 0ULL) ? 0
							: THERMAL_SYS_INFO_CPU;
		mask |= (g_check_batt_info_flag == 0ULL)
				? 0
				: THERMAL_SYS_INFO_BATT;
		mask |= (g_check_wifi_info_flag == 0ULL)
				? 0
				: THERMAL_SYS_INFO_WIFI;
		mask |= (g_check_mobile_info_flag == 0ULL)
				? 0
				: THERMAL_SYS_INFO_MD;
...
/* Write */
static ssize_t mtkthermal_write(struct file *file, const char __user *buffer,
...
static int
_mtkthermal_check_cooler_conditions(struct mtk_thermal_cooler_data *cldata)
{
	int ret = 0;
		...
		for (; i < MTK_THERMAL_MONITOR_COOLER_MAX_EXTRA_CONDITIONS;
			...
			if (cldata->condition_last_value[i] == NULL) {
				if (cldata->conditions[i][0] == 0x0)
					ret++;
				else if (strncmp(cldata->conditions[i], "EXIT",
						 4) == 0)
					ret++;
			} else {
#if 1 /* [FIX ME] Special case for "condifion of MOBILE"  */
				if (strncmp(cldata->conditions[i], "MOBILE",
					    5) == 0) {
...
static void
_mtkthermal_clear_cooler_conditions(struct mtk_thermal_cooler_data *cldata)
{
	int i = 0;
...
static ssize_t _mtkthermal_cooler_write(struct file *file,
					...
					loff_t *data)
{
	...
	/**
	 * sscanf format <condition_1> <threshold_1> ... <condition_n>
	 * <threshold_n>
...
/* WARNING: Modify here if MTK_THERMAL_MONITOR_COOLER_MAX_EXTRA_CONDITIONS is
 * changed to other than 3
 */
#if (MTK_THERMAL_MONITOR_COOLER_MAX_EXTRA_CONDITIONS == 3)
	_mtkthermal_clear_cooler_conditions(mcdata);

	if (sscanf(desc, "%19s %d %19s %d %19s %d", &mcdata->conditions[0][0],
		   &mcdata->threshold[0], &mcdata->conditions[1][0],
		   &mcdata->threshold[1], &mcdata->conditions[2][0],
		   &mcdata->threshold[2]) >= 2) {
		int i = 0;
		...
		for (; i < MTK_THERMAL_MONITOR_COOLER_MAX_EXTRA_CONDITIONS;
			...
			if (strncmp(mcdata->conditions[i], "EXIT", 4) == 0) {
			...
			} else if (strncmp(mcdata->conditions[i], "CPU0", 4) ==
				   0) {
				mcdata->condition_last_value[i] =
					&nCPU_loading_sum;
				g_check_cpu_info_flag |= (1ULL << mcdata->id);
				THRML_LOG("%s cpu flag: %016llx, id=%d\n",
					  __func__, g_check_cpu_info_flag,
					  mcdata->id);
			} else if (strncmp(mcdata->conditions[i], "BATCC", 5) ==
				   0) {
				mcdata->condition_last_value[i] =
					&nBattCurrentCnsmpt;
				g_check_batt_info_flag |= (1ULL << mcdata->id);
				THRML_LOG("%s batt flag: %016llx, id=%d\n",
					  __func__, g_check_batt_info_flag,
					  mcdata->id);
				...
				THRML_LOG("%s wifi flag: %016llx, id=%d\n",
					  __func__, g_check_wifi_info_flag,
					  mcdata->id);
			} else if (strncmp(mcdata->conditions[i], "MOBILE",
					   5) == 0) {
				mcdata->condition_last_value[i] =
					&nMobile_throughput;
				...
				THRML_LOG("%s mobile flag: %016llx, id=%d\n",
					  __func__, g_check_mobile_info_flag,
					  mcdata->id);
			} else {
...
#else
#error "Change correspondent part when changing !"
#endif
...
static ssize_t _mtkthermal_tz_write(struct file *file,
				    const char __user *buffer, size_t count,
				    loff_t *data)
{
	int len = 0;
	...
	if (sscanf(desc, "%31s %d %127s", arg_name, &arg_val, trailing) >= 2) {
		if ((strncmp(arg_name, "ma_len", 6) == 0) && (arg_val >= 1) &&
		    (arg_val <= 60)) {
			struct mtk_thermal_tz_data *tzdata = NULL;
...
static long _mtkthermal_update_and_get_sma(struct mtk_thermal_tz_data *tzdata,
					   long latest_val)
{
	long ret = 0;
	...
	if (tzdata->ma_len == 1) {
		...
		tzdata->ma[(tzdata->ma_counter) % (tzdata->ma_len)] =
			latest_val;
		tzdata->ma_counter++;
	 ...
	 *      b. if (curr_idx_sma_len != i) {ma_counter = 0; ma_len =
	 * sma_len[curr_idx_sma_len = i]; }
	 *  3. Check if need to change region if ma_counter > 1
	 *      a. if SMA >= high_threshold[curr_idx_sma_len]
	 *      { Move upward: ma_counter = 0; ma_len =
	 * sma_len[++curr_idx_sma_len]; }
	 *      b. else if curr_idx_sma_len >0 && SMA <
	 * high_threshold[curr_idx_sma_len-1]
	 *      { Move downward: ma_counter =0; ma_len =
	 * sma_len[--curr_idx_sma_len]; }
	 */
	if (tzdata->ma_counter == 1) {
		...
		if (tzdata->curr_idx_ma_len != i) {
			...
			THRML_LOG("%s 2b ma_len: %d curr_idx_ma_len: %d\n",
				  __func__, tzdata->ma_len,
				  tzdata->curr_idx_ma_len);
		}
	} else {
...
/* Write */
static ssize_t _mtkthermal_indicator_write(struct file *file,
...
/* Write */
static ssize_t _mtm_scen_call_write(struct file *file,
				    const char __user *buffer, size_t count,
				    loff_t *data)
{
...
static int __init mtkthermal_init(void)
{
	...
	entry = proc_create("mtm_monitor", 0664, dir_entry, &mtkthermal_fops);
	if (!entry)
		THRML_ERROR_LOG("%s Can not create mtm_monitor\n", __func__);
	else
		proc_set_user(entry, uid, gid);

	entry = proc_create("mtm_indicator", 0644, dir_entry,
			    &_mtkthermal_indicator_fops);
	if (!entry)
		THRML_ERROR_LOG("%s Can not create mtm_indicator\n", __func__);

	entry = proc_create("mtm_scen_call", 0664, dir_entry,
			    &_mtm_scen_call_fops);
	if (!entry)
	...
	/* WARNING! This is not gauranteed to be invoked before mtk_ts_cpu's
	 * functions...
	 */
	proc_cooler_dir_entry = (proc_cooler_dir_entry == NULL)
					? proc_mkdir("mtkcooler", NULL)
	 ...
	 * functions...
	 */
	proc_tz_dir_entry = (proc_tz_dir_entry == NULL)
				    ? proc_mkdir("mtktz", NULL)
				    : proc_tz_dir_entry;
	if (proc_tz_dir_entry == NULL)
		THRML_ERROR_LOG("%s mkdir /proc/mtktz failed\n", __func__);
#if defined(CONFIG_MTK_THERMAL_TIME_BASE_PROTECTION)
 ...
 */
static int mtk_thermal_wrapper_bind(struct thermal_zone_device *thermal,
 ...
 */
static int mtk_thermal_wrapper_unbind(struct thermal_zone_device *thermal,
				      struct thermal_cooling_device *cdev)
{
	int ret = 0;
	struct thermal_zone_device_ops *ops;
 ...
 */
static int mtk_thermal_wrapper_get_temp(struct thermal_zone_device *thermal,
					int *temperature)
{
	int ret = 0;
	struct thermal_zone_device_ops *ops;
	...
	if ((tz_last_values[MTK_THERMAL_SENSOR_BATTERY] !=
	     NULL) && /* batt TZ is registered */
	    (&(thermal->temperature) ==
	     tz_last_values[MTK_THERMAL_SENSOR_BATTERY])) {
	...
	if (ret == 0) {
		*temperature = _mtkthermal_update_and_get_sma(thermal->devdata,
							      raw_temp);
		/* No strong type cast... */
	} else {
	...
	THRML_STORAGE_LOG(THRML_LOGGER_MSG_ZONE_TEMP, get_temp, thermal->type,
			  (int)*temperature);
	THRML_LOG("[.get_temp] tz: %s raw: %d sma: %ld\n", thermal->type,
		  nTemperature, (long)*temperature);
 ...
 */
static int mtk_thermal_wrapper_get_mode(struct thermal_zone_device *thermal,
 ...
 */
static int mtk_thermal_wrapper_set_mode(struct thermal_zone_device *thermal,
					enum thermal_device_mode mode)
{
	int ret = 0;
	struct thermal_zone_device_ops *ops;
 ...
 */
static int
mtk_thermal_wrapper_get_trip_type(struct thermal_zone_device *thermal, int trip,
...
/*
 * .get_trip_temp wrapper: get the temperature above which the certain trip
 * point
 *  will be fired.
 */
static int
mtk_thermal_wrapper_get_trip_temp(struct thermal_zone_device *thermal, int trip,
				  int *temperature)
{
	int ret = 0;
	struct thermal_zone_device_ops *ops;
 ...
 */
static int
mtk_thermal_wrapper_get_crit_temp(struct thermal_zone_device *thermal,
				  int *temperature)
{
	int ret = 0;
	struct thermal_zone_device_ops *ops;
...
static int mtk_thermal_wrapper_notify(struct thermal_zone_device *thermal,
...
struct thermal_zone_device *mtk_thermal_zone_device_register_wrapper(
	...
	const struct thermal_zone_device_ops *ops, int tc1, int tc2,
	int passive_delay, int polling_delay)
{
	...
	tz = thermal_zone_device_register(
		type, trips, /* /< total number of trip points */
		0,	   /* /< mask */
		/* (void*)ops,                  ///< invoker's ops pass to
		 * devdata
		 */
		(void *)tzdata,
	...
	struct thermal_cooling_device *cdev)
{
	...
	ret = thermal_zone_bind_cooling_device(
		thermal, trip, cdev, THERMAL_NO_LIMIT, THERMAL_NO_LIMIT,
	...
	if (ret) {
		...
		/* TODO: think of a way don't do this here...Or cannot rollback
		 * devdata in bind ops...
		 */
	...
	THRML_STORAGE_LOG(THRML_LOGGER_MSG_BIND, bind, thermal->type, trip,
			  cdev->type);
...
/* .get_max_state */
static int
mtk_cooling_wrapper_get_max_state(struct thermal_cooling_device *cdev,
				  unsigned long *state)
{
	int ret = 0;
	struct thermal_cooling_device_ops *ops;
...
/* .get_cur_state */
static int
mtk_cooling_wrapper_get_cur_state(struct thermal_cooling_device *cdev,
				  unsigned long *state)
{
	int ret = 0;
	struct thermal_cooling_device_ops *ops;
...
/* set_cur_state */
static int
mtk_cooling_wrapper_set_cur_state(struct thermal_cooling_device *cdev,
				  unsigned long state)
{
	struct thermal_cooling_device_ops *ops;
	struct thermal_cooling_device_ops_extra *ops_ext;
...
/* check conditions */
	...
	if (state != 0) {
		...
		if (MTK_THERMAL_MONITOR_COOLER_MAX_EXTRA_CONDITIONS ==
			...
			THRML_LOG(
				...
				mcdata->tz->type, cdev->type, mcdata->trip,
				state);
			state = 0;
		}
	}
	...
	if (state == 0) {
		...
		if ((mcdata->exit_threshold > 0) && (mcdata->tz != NULL)) {
			/* if exit point is set and if this cooler is still
			 * bound...
			 */
	...
	THRML_LOG(
		"[.set_cur_state] tz_type:%s cdev_type:%s trip:%d state:%lu\n",
	...
	char *type, void *devdata, const struct thermal_cooling_device_ops *ops)
{
	...
	if (_get_proc_cooler_dir_entry() != NULL) {
		...
		entry = proc_create_data((const char *)type, 0664,
					 proc_cooler_dir_entry,
					 &_mtkthermal_cooler_fops, mcdata);
		if (!entry) {
			THRML_ERROR_LOG("%s proc file not created: %p\n",
					__func__, mcdata);
		} else {
...
int mtk_thermal_zone_bind_trigger_trip(struct thermal_zone_device *tz, int trip,
				       int mode)
{
	THRML_LOG("%s trip %d\n", __func__, trip);
