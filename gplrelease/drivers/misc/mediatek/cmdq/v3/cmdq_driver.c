...
s32 cmdq_driver_ioctl_async_job_wait_and_close(unsigned long param)
{
	...
	struct cmdqRecStruct *handle = {0};
	u32 *userRegValue = NULL;
	/* backup value after task release */
	s32 status;
	u64 exec_cost = sched_clock();

	memset(&jobResult, 0x0, sizeof(jobResult));
...
long cmdq_ioctl(struct file *pf, unsigned int code,
	unsigned long param)
{
	s32 status = 0;
	...
	case CMDQ_IOCTL_READ_READBACK_SLOTS:
		...
		break;
	default:
		CMDQ_ERR("driver unrecognized ioctl 0x%08x\n", code);
		return -ENOIOCTLCMD;
	}
...
#ifdef CONFIG_COMPAT
long cmdq_ioctl_compat(struct file *pFile, unsigned int code,
	unsigned long param)
{
	switch (code) {
	case CMDQ_IOCTL_QUERY_USAGE:
