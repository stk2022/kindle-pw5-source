	...
	struct cmdqRecStruct *handle, u32 index)
{
	...
	cmd_va= cmdq_pkt_get_va_by_offset(handle->pkt, index * CMDQ_INST_SIZE);
	if (cmd_va == NULL) {
		CMDQ_ERR("failed to alloc cmdq_stack_node\n");
		return ;
	}
	...
	inst = *cmd_va;
	logic_inst_temp = cmdq_pkt_get_va_by_offset(handle->pkt, (index - 1) *
		CMDQ_INST_SIZE);
	if (logic_inst_temp == NULL) {
		CMDQ_ERR("failed to get logic_inst_temp\n");
		return;
	}
	logic_inst = *logic_inst_temp;
	CMDQ_MSG(
...
s32 cmdq_op_acquire(struct cmdqRecStruct *handle, enum cmdq_event event)
{
	...
	if (arg_a < 0 || arg_a >= CMDQ_MAX_EVENT || !handle)
		return -EINVAL;
...
{
	return cmdqCoreAllocWriteAddress(count, paStart);
}

s32 cmdq_free_write_addr(dma_addr_t paStart, u32 clt)
{
	return cmdqCoreFreeWriteAddress(paStart);
}
...
s32 cmdq_op_write_mem_with_cpr(struct cmdqRecStruct *handle,
	CMDQ_VARIABLE var_mem_addr, u32 value)
{
	s32 status;
...
EXPORT_SYMBOL(cmdq_op_write_mem_with_cpr);
	...
	void *prop_addr, u32 prop_size)
{
	...
	memset(pprop_addr, 0x0, prop_size);
	if (!pprop_addr)
		return -ENOMEM;
...
{
	u32 op = 0, op_arg_type = 0, op_jump = 0;
	u32 *va_jump = NULL, *va_cond = NULL;
	...
	enum CMDQ_CONDITION_ENUM arg_condition, CMDQ_VARIABLE arg_c)
{
	...
	do {
		...
		status = cmdq_append_jump_c_command(handle, arg_b,
			arg_condition, arg_c);
		if (status < 0) {
			CMDQ_ERR("call jump_c_command fail: argb:0x%016llx cond:%d argc:0x%016llx",
				arg_b, arg_condition, arg_c);
			break;
		}
...
s32 cmdq_op_read_mem_with_cpr(struct cmdqRecStruct *handle, CMDQ_VARIABLE addr,
	CMDQ_VARIABLE *arg_out, u32 mask)
{
	/* buffer address is store in CPR:addr */
	s32 status = 0;
...
EXPORT_SYMBOL(cmdq_op_read_mem_with_cpr);
