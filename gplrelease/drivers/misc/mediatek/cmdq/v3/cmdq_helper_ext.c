	...
	const u32 op, const u32 arg_a, const u32 arg_b)
{
	...
	case CMDQ_CODE_MOVE:
		if (1 & (arg_a >> 23)) {
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 1\n");
				return -1;
			}
		} else {
			reqLen = snprintf(textBuf, bufLen,
				"Set MASK:0x%08x\n", arg_b);
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 2\n");
				return -1;
			}
		}
		break;
	...
	case CMDQ_CODE_WRITE_S_W_MASK:
		...
		if (reqLen < 0 || reqLen >= bufLen) {
			CMDQ_ERR("[ERR] write textBuf with bufLen wrong 3\n");
			return -1;
		}
		bufLen -= reqLen;
		textBuf += reqLen;
		...
		if (arg_value_type != 0) {
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 4\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			...
			reqLen = snprintf(textBuf, bufLen, "0x%08x, ",
				arg_value);
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 5\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
		...
		if (arg_addr_type != 0) {
			...
			reqLen = snprintf(textBuf, bufLen, "Reg%d, ", reg_id);
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 6\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 7\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
		...
		if (reqLen < 0 || reqLen >= bufLen) {
			CMDQ_ERR("[ERR] write textBuf with bufLen wrong 8\n");
			return -1;
		}
		bufLen -= reqLen;
		textBuf += reqLen;
		...
		if (arg_a) {
			if (arg_a & (1 << 22)) {
				...
				reqLen = snprintf(textBuf, bufLen,
					"JUMP(register): Reg%d\n", arg_b);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 9\n");
					return -1;
				}
				...
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 10\n");
					return -1;
				}
			...
			if ((s32) arg_b >= 0) {
				reqLen = snprintf(textBuf, bufLen,
					"JUMP(relative):+%d\n", (s32) arg_b);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 11\n");
					return -1;
				}
				...
				reqLen = snprintf(textBuf, bufLen,
					"JUMP(relative):%d\n", (s32) arg_b);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 12\n");
					return -1;
				}
			}
		}
	...
	case CMDQ_CODE_WFE:
		if (arg_b == 0x80008001) {
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 13\n");
				return -1;
			}
			...
			reqLen = snprintf(textBuf, bufLen, "Clear Event:%s\n",
				cmdq_core_get_event_name(arg_a));
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 14\n");
				return -1;
			}
			...
			reqLen = snprintf(textBuf, bufLen, "Set Event:%s\n",
				cmdq_core_get_event_name(arg_a));
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 15\n");
				return -1;
			}
			...
			reqLen = snprintf(textBuf, bufLen,
				...
				cmdq_core_get_event_name(arg_a));
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 16\n");
				return -1;
			}
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 17\n");
				return -1;
			}
	...
	case CMDQ_CODE_EOC:
		if (arg_a == 0 && arg_b == 0x00000001) {
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 18\n");
				return -1;
			}
		} else {
			reqLen = snprintf(textBuf, bufLen, "MARKER:");
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 19\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			if (arg_b == 0x00100000) {
				...
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 20\n");
					return -1;
				}
			} else if (arg_b == 0x00130000) {
				reqLen = snprintf(textBuf, bufLen, " Enable");
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 21\n");
					return -1;
				}
			} else {
				reqLen = snprintf(textBuf, bufLen,
					"no_suspnd:%d no_inc:%d m:%d m_en:%d prefetch:%d irq:%d\n",
						...
						(arg_b & (1 << 0)) > 0);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 22\n");
					return -1;
				}
			}
		}
	...
	case CMDQ_CODE_LOGIC:
		{
			...
			reqLen = snprintf(textBuf, bufLen, "%s: ",
				cmdq_core_parse_op(op));
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 23\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			...
			reqLen = snprintf(textBuf, bufLen, "Reg%d = ",
				(arg_a & 0xFFFF));
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 24\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			...
			if (s_op == CMDQ_LOGIC_ASSIGN) {
				reqLen = snprintf(textBuf, bufLen, "0x%08x\n",
					arg_b);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 25\n");
					return -1;
				}
				...
				if (arg_b_type != 0)
					reqLen = snprintf(textBuf, bufLen,
						"~Reg%d\n", arg_b_i);
					if (reqLen < 0 || reqLen >= bufLen) {
						CMDQ_ERR("[ERR] write textBuf with bufLen wrong 26\n");
						return -1;
					}
				else
					reqLen = snprintf(textBuf, bufLen,
						"~%d\n", arg_b_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 27\n");
					return -1;
				}
				bufLen -= reqLen;
				textBuf += reqLen;
				...
				if (arg_b_type != 0) {
					reqLen = snprintf(textBuf, bufLen,
						"Reg%d ", arg_b_i);
					if (reqLen < 0 || reqLen >= bufLen) {
						CMDQ_ERR("[ERR] write textBuf with bufLen wrong 28\n");
						return -1;
					}
					bufLen -= reqLen;
					textBuf += reqLen;
					...
					reqLen = snprintf(textBuf, bufLen,
						"%d ", arg_b_i);
					if (reqLen < 0 || reqLen >= bufLen) {
						CMDQ_ERR("[ERR] write textBuf with bufLen wrong 29\n");
						return -1;
					}
					bufLen -= reqLen;
					textBuf += reqLen;
				...
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 30\n");
					return -1;
				}
				bufLen -= reqLen;
				textBuf += reqLen;
				...
				if (arg_c_type != 0) {
					reqLen = snprintf(textBuf, bufLen,
						"Reg%d\n", arg_c_i);
					if (reqLen < 0 || reqLen >= bufLen) {
						CMDQ_ERR("[ERR] write textBuf with bufLen wrong 31\n");
						return -1;
					}
					bufLen -= reqLen;
					textBuf += reqLen;
					...
					reqLen = snprintf(textBuf, bufLen,
						"%d\n", arg_c_i);
					if (reqLen < 0 || reqLen >= bufLen) {
						CMDQ_ERR("[ERR] write textBuf with bufLen wrong 32\n");
						return -1;
					}
					bufLen -= reqLen;
					textBuf += reqLen;
	...
	case CMDQ_CODE_JUMP_C_ABSOLUTE:
		{
			...
			reqLen = snprintf(textBuf, bufLen, "%s: if (",
				cmdq_core_parse_op(op));
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 33\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			...
			if (arg_b_type != 0) {
				reqLen = snprintf(textBuf, bufLen, "Reg%d ",
					arg_b_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 34\n");
					return -1;
				}
				...
				reqLen = snprintf(textBuf, bufLen, "%d ",
					arg_b_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 35\n");
					return -1;
				}
				bufLen -= reqLen;
				textBuf += reqLen;
			...
			if (reqLen < 0 || reqLen >= bufLen) {
				CMDQ_ERR("[ERR] write textBuf with bufLen wrong 36\n");
				return -1;
			}
			bufLen -= reqLen;
			textBuf += reqLen;
			...
			if (arg_c_type != 0) {
				reqLen = snprintf(textBuf, bufLen,
					"Reg%d) jump ", arg_c_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 37\n");
					return -1;
				}
				bufLen -= reqLen;
				textBuf += reqLen;
				...
				reqLen = snprintf(textBuf, bufLen,
					"%d) jump ", arg_c_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 38\n");
					return -1;
				}
				bufLen -= reqLen;
				textBuf += reqLen;
			...
			if (arg_a_type != 0) {
				reqLen = snprintf(textBuf, bufLen,
					"Reg%d\n", arg_a_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 39\n");
					return -1;
				}
				...
				reqLen = snprintf(textBuf, bufLen,
					"+%d\n", arg_a_i);
				if (reqLen < 0 || reqLen >= bufLen) {
					CMDQ_ERR("[ERR] write textBuf with bufLen wrong 40\n");
					return -1;
				}
				bufLen -= reqLen;
				textBuf += reqLen;
	...
	default:
		reqLen = snprintf(textBuf, bufLen,
			...
			op, arg_a, arg_b);
		if (reqLen < 0 || reqLen >= bufLen) {
			CMDQ_ERR("[ERR] write textBuf with bufLen wrong 41\n");
			return -1;
		}
		break;
	}
...
int cmdq_core_print_record_title(char *_buf, int bufLen)
{
	...
	if (length < 0 || length >= bufLen) {
		CMDQ_ERR("[ERR] write Buf with bufLen wrong 1\n");
		return -1;
	}
	bufLen -= length;
	buf += length;
	...
	if (length < 0 || length >= bufLen) {
		CMDQ_ERR("[ERR] write Buf with bufLen wrong 2\n");
		return -1;
	}
	...
	int index, char *_buf, int bufLen)
{
	...
	if (length < 0 || length >= bufLen) {
		CMDQ_ERR("[ERR] write Buf with bufLen wrong 3\n");
		return -1;
	}
	bufLen -= length;
	buf += length;
	...
	if (length < 0 || length >= bufLen) {
		CMDQ_ERR("[ERR] write Buf with bufLen wrong 4\n");
		return -1;
	}
	bufLen -= length;
	buf += length;
	...
	for (i = 0; i < profileMarkerCount; i++) {
		...
		if (length < 0 || length >= bufLen) {
			CMDQ_ERR("[ERR] write Buf with bufLen wrong 5\n");
			return -1;
		}
	...
	length = snprintf(buf, bufLen, "\n");
	if (length < 0 || length >= bufLen) {
		CMDQ_ERR("[ERR] write Buf with bufLen wrong 6\n");
		return -1;
	}
	bufLen -= length;
	buf += length;
	...
	s32 thread, bool dump_irq, bool aee_irq, const char *tag)
{
	...
	if (dump_irq && value[0] == value[1] && value[2] == 1) {
		#if 0
		/* harry add for early porting build pass. */
...
void cmdq_core_turnon_first_dump(const struct cmdqRecStruct *task)
{
	...
	ret = snprintf(cmdq_first_err.callerName, TASK_COMM_LEN, "%s",
		task->caller_name);
	if (ret < 0 || ret >= TASK_COMM_LEN) {
		CMDQ_ERR("[ERR] write (cmdq_first_err.callerName wrong \n");
		return;
	}
...
s32 cmdq_core_save_first_dump(const char *string, ...)
{
	...
	logLen = vsnprintf(buffer, cmdq_first_err.cmdqMaxSize, string,
		argptr);
	if (logLen < 0 || logLen >= cmdq_first_err.cmdqMaxSize) {
		CMDQ_ERR("[ERR] write buffer with cmdq_first_err.cmdqMaxSize wrong\n");
		return -1;
	}

	if (ret < 0 || ret >= cmdq_first_err.cmdqMaxSize) {
	CMDQ_ERR("read cmdq_first_err error.");
	return -1;
	}
	...
	int rowsize, int groupsize, const void *buf, size_t len)
{
	...
	unsigned char linebuf[32 * 3 + 2 + 32 + 1] = {0};
	int logLen = 0;
	char *pBuffer = NULL;
	...
	for (i = 0; i < len; i += rowsize) {
		...
		if (logLen < 0 || logLen >= cmdq_first_err.cmdqMaxSize) {
		CMDQ_ERR("[ERR] write pBuffer with cmdq_first_err.cmdqMaxSize wrong\n");
		return;
		}
		cmdq_first_err.cmdqMaxSize -= logLen;
		cmdq_first_err.cmdqCount += logLen;
...
EXPORT_SYMBOL(cmdqCoreClearEvent);
...
EXPORT_SYMBOL(cmdqCoreGetEvent);
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < cmdq_ctx.errNum && i < CMDQ_MAX_ERROR_COUNT;
		...
		ret = snprintf(buf + length,
			PAGE_SIZE - length, "[%5lu.%06lu] ",
			(unsigned long)ts, rem_nsec / 1000);
		if (ret < 0 || ret >= (PAGE_SIZE - length)) {
		CMDQ_ERR("[ERR] write cmdq_core_print_error buf  with TASK_COMM_LEN wrong\n");
		return 0;
		}
		length += ret;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (buf)
		len = sprintf(buf, "%d\n", cmdq_ctx.logLevel);
	if (len < 0) {
		CMDQ_ERR("print log level error\n");
		len = -1;
	}
	return len;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (len < 0) {
		CMDQ_ERR("[ERR] printf cmdq_ctx.enableProfile wrong\n");
		return 0;
	}
	return len;
}
	...
	const struct cmdqRecStruct *handle)
{
	...
	ret = snprintf(cmdq_first_err.callerName, TASK_COMM_LEN, "%s",
		handle->caller_name);
	if (ret < 0 || ret >= TASK_COMM_LEN) {
	CMDQ_ERR("[ERR] write cmdq_first_err.callerName with TASK_COMM_LEN wrong\n");
	return;
	}
	cmdq_first_err.savetime = sched_clock();
	do_gettimeofday(&cmdq_first_err.savetv);
	...
	u32 index)
{
	...
	if (NULL == p_cmd_assign) {
		CMDQ_ERR("GET p_cmd_assign FAIL\n");
		return;
	}
...
s32 cmdq_pkt_mdp_finalize(struct cmdqRecStruct *handle)
{
    int err;
    ...
    err = cmdq_pkt_append_command(handle, (CMDQ_CODE_EOC << CMDQ_OP_CODE_SHIFT), 1);
    if (err < 0)
        return err;

    /* JUMP to end */
    err = cmdq_pkt_append_command(handle, 0x10000000, 8);
    if (err < 0)
        return err;
...
static void cmdq_pkt_flush_handler(struct cmdq_cb_data data)
{
	...
	if (handle->loop) {
		...
		if (handle->loop_cb) {
			...
			CMDQ_MSG("loop callback handle:0x%p\n", handle);
			loop_ret = handle->loop_cb(handle->loop_user_data);
			CMDQ_MSG("loop callback done\n");
		}
...
s32 cmdq_pkt_wait_flush_ex_result(struct cmdqRecStruct *handle)
{
	...
	do {
		if (handle->pkt->timeout) {
			...
			u32 wait_time_ms = (handle->scenario == CMDQ_SCENARIO_WF_LUT) ? CMDQ_HWTCON_PREDUMP_TIMEOUT_MS : CMDQ_PREDUMP_TIMEOUT_MS;
