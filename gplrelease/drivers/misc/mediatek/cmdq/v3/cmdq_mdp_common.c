...
{ \
	ACTION(CMDQ_ENG_IMG_RDMA, img_rdma) \
	ACTION(CMDQ_ENG_WB_RDMA, wb_rdma) \
	ACTION(CMDQ_ENG_WB_WDMA, wb_wdma) \
	ACTION(CMDQ_ENG_PIPELINE, pipeline) \
	ACTION(CMDQ_ENG_REGAL, regal) \
	ACTION(CMDQ_ENG_PAPER_TOP, paper_top) \
	ACTION(CMDQ_ENG_WF_LUT, wf_lut) \
	ACTION(CMDQ_ENG_DISP_RDMA, wf_disp_rdma) \
	ACTION(CMDQ_ENG_TCON, tcon) \
	ACTION(CMDQ_ENG_DISP_DPI, dpi) \
	ACTION(CMDQ_ENG_MDP_RDMA0, mdp_rdma) \
	ACTION(CMDQ_ENG_MDP_GAMMA, mdp_gamma) \
	ACTION(CMDQ_ENG_MDP_RSZ0, mdp_rsz) \
	ACTION(CMDQ_ENG_MDP_TDSHP0, mdp_sharp) \
	ACTION(CMDQ_ENG_MDP_WROT0, mdp_wrot) \
	ACTION(CMDQ_ENG_DISP_OVL_2L, disp_ovl_2l) \
	ACTION(CMDQ_ENG_DISP_WDMA, disp_wdma) \
	ACTION(CMDQ_ENG_IMGRESZ, imgresz) \
	ACTION(CMDQ_ENG_PNG_DEC, png_dec) \
	ACTION(CMDQ_ENG_JPEG_DEC, jpeg_dec) \
}
...
{
	command->scenario = CMDQ_SCENARIO_MDP;
	#if 0
...
s32 cmdq_mdp_handle_create(struct cmdqRecStruct **handle_out)
{
	...
	status = cmdq_task_create(CMDQ_SCENARIO_MDP, &handle);
	if (status < 0) {
...
static void cmdq_mdp_init_pmqos(void)
{
	...
	for (i = 0; i < MDP_TOTAL_THREAD; i++) {
		...
		ret = snprintf(mdp_bw_qos_request[i].owner,
		  sizeof(mdp_bw_qos_request[i].owner) - 1, "mdp_bw_%d", i);
		if (ret < 0 || ret >= (sizeof(mdp_bw_qos_request[i].owner) - 1)) {
		CMDQ_ERR("[ERR] write owner  wrong1\n");
		return;
		}
		ret = snprintf(mdp_clk_qos_request[i].owner,
		  sizeof(mdp_clk_qos_request[i].owner) - 1, "mdp_clk_%d", i);
		if (ret < 0 || ret >= (sizeof(mdp_clk_qos_request[i].owner) - 1)) {
		CMDQ_ERR("[ERR] write owner  wrong2\n");
		return;
		}
		ret = snprintf(isp_bw_qos_request[i].owner,
		  sizeof(isp_bw_qos_request[i].owner) - 1, "isp_bw_%d", i);
		if (ret < 0 || ret >= (sizeof(isp_bw_qos_request[i].owner) - 1)) {
		CMDQ_ERR("[ERR] write owner  wrong3\n");
		return;
		}
		ret = snprintf(isp_clk_qos_request[i].owner,
		  sizeof(isp_clk_qos_request[i].owner) - 1, "isp_clk_%d", i);
		if (ret < 0 || ret >= (sizeof(isp_clk_qos_request[i].owner) - 1)) {
		CMDQ_ERR("[ERR] write owner  wrong4\n");
		return;
		}
...
{
	return cmdq_virtual_module_from_hw_engine(handle->engineFlag);
#if 0
...
void cmdq_mdp_virtual_function_setting(void)
{
	...
	#else
	pFunc->beginISPTask = NULL;
	pFunc->endISPTask = NULL;
	#endif
}
