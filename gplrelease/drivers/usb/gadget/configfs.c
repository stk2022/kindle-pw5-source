...
static ssize_t gadget_dev_desc_connected_show(struct config_item *item, char *page)
{
	return sprintf(page, "%d\n", connect_state);
}
static ssize_t gadget_dev_desc_connected_store(struct config_item *item,
		const char *page, size_t len)
{
	pr_err("%s \n", __func__);
	return 0;
...
CONFIGFS_ATTR(gadget_dev_desc_, connected);
...
static struct configfs_attribute *gadget_root_attrs[] = {
	...
	&gadget_dev_desc_attr_connected,
	NULL,
};
