...
#ifdef CONFIG_USB_MU3D_DRV
	.MaxPower	= 192,
	/* Only consume 192ma for passing USB30CV Descriptor Test
	 * [USB3.0 devices]
	 */
#else
	.MaxPower	= 500, /* 500ma */
		...
		struct usb_configuration *c)
{
	...
	for (i = 0; i < MAX_ACM_INSTANCES; i++) {
		if (config->port_index[i] != 0 || (quick_vcom & (0x1 << i))) {
					...
					struct usb_composite_dev *cdev)
{
	...
	/* note this is important for sysfs manipulation and this will be
	 * override when fsg_main_thread be created
	 */
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		 ...
		 * function bind will fail because we cannot allocate new string
		 * ids.
		...
		android_enable(dev);
		dev->enabled = true;
		pr_notice("[USB]%s: enable 0->1 , VID = 0x%x, PID = 0x%x\n",
		...
		pr_notice("[USB]%s: enable 1->0 , VID = 0x%x, PID = 0x%x\n",
			__func__, device_desc.idVendor, device_desc.idProduct);
		android_disable(dev);
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			pr_notice("[USB]disable function '%s'\n", f->name);
			if (f->disable)
				f->disable(f);
		}
		dev->enabled = false;
...
const struct usb_ctrlrequest *ctrl)
{
	...
	pr_notice("%s bRequestType=%x, bRequest=%x, recip=%x\n", __func__,
		ctrl->bRequestType, ctrl->bRequest, recip);
...
int acm_shortcut(void)
{
	...
	if (ptr) {
		...
		pr_notice("can't find \"androidboot.usbconfig=\" in cmdline\n");
	return 0;
}
...
static int __init meta_usb_init(void)
{
	...
	if (err) {
		pr_info("%s: failed to create device %d", __func__, err);
		goto err_create;
	}
