...
static void ssusb_set_mode(struct work_struct *work)
{
	...
	case MTU3_VBUS_OFF:
		mtu3_stop(mtu);
		if (device_can_wakeup(ssusb->dev))
			pm_relax(ssusb->dev);
		else if (ssusb->usb_lock.active)
			__pm_relax(&ssusb->usb_lock);
	...
	enum mtu3_vbus_id_state status)
{
	...
	switch (status) {
	case MTU3_ID_GROUND:
		otg_sx->desire_usb_mode = MTU3_ID_GROUND;
		break;
	case MTU3_ID_FLOAT:
		otg_sx->desire_usb_mode = MTU3_ID_FLOAT;
		break;
	case MTU3_VBUS_OFF:
		otg_sx->desire_usb_mode = MTU3_VBUS_OFF;
		break;
	case MTU3_VBUS_VALID:
		otg_sx->desire_usb_mode = MTU3_VBUS_VALID;
		break;
	default:
...
static int ssusb_extcon_register(struct otg_switch_mtk *otg_sx)
{
	...
	/*default disable vbus,especially for regulator*/
	ssusb_set_vbus(otg_sx, 0);
	...
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (kstrtobool(buf, &enable)) {
		dev_info(ssusb->dev, "wrong setting\n");
		return -EINVAL;
	}
...
static int mtu3_drp_init(struct mtu3 *mtu3)
{
	...
	if (IS_ERR(drp_inst)) {
		dev_info(mtu3->dev, "fail to register dual role instance\n");
...
int ssusb_otg_switch_init(struct ssusb_mtk *ssusb)
{
	...
	#endif
	if (!device_can_wakeup(ssusb->dev))
		wakeup_source_init(&ssusb->usb_lock, "ssusb.wakelock");
	return 0;
}
