...
static const unsigned long mt8512_pwm_register[PWM_NUM_MAX] = {
...
static struct mtk_com_pwm_data mt8168_pwm_data = {
	/* reuse 8167 pwm register's offsets */
	.pwm_register = &mt8167_pwm_register[0],
	.pwm_nums = 3,
};

static struct mtk_com_pwm_data mt8512_pwm_data = {
	.pwm_register = &mt8512_pwm_register[0],
	.pwm_nums = 7,
};
...
static const struct of_device_id pwm_of_match[] = {
	...
	{.compatible = "mediatek,mt8512-pwm", .data = &mt8512_pwm_data},
	{},
};
...
static inline void mtk_pwm_writel(struct mtk_com_pwm *pwm,
				...
				unsigned long val)
{
	void __iomem *reg = pwm->base + pwm->data->pwm_register[pwm_no]
				+ offset;
...
int mtk_pwm_set_thresh(struct pwm_device *pwm, int thresh)
{
	struct mtk_com_pwm *mt_pwm = to_mtk_pwm(pwm->chip);

	mtk_pwm_clk_enable(pwm->chip, pwm);

	mtk_pwm_writel(mt_pwm, pwm->hwpwm, PWMTHRES, thresh);

	mtk_pwm_clk_disable(pwm->chip, pwm);

	return 0;
}
EXPORT_SYMBOL_GPL(mtk_pwm_set_thresh);


int mtk_pwm_get_thresh(struct pwm_device *pwm)
{
	struct mtk_com_pwm *mt_pwm = to_mtk_pwm(pwm->chip);
	u32 thresh;
	...
	thresh = mtk_pwm_readl(mt_pwm, 1, PWMTHRES);

	mtk_pwm_clk_disable(pwm->chip, pwm);

	return thresh;
}
EXPORT_SYMBOL_GPL(mtk_pwm_get_thresh);
