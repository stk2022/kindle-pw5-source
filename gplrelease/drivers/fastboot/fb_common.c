...
/**
 * fastboot_info() - Write an INFO response of the form "INFO$reason".
 *
 * @reason: Pointer to returned reason string, or NULL to send a bare "INFO"
 * @response: Pointer to fastboot response buffer
 */
void fastboot_info(const char *reason, char *response)
{
	if (fastboot_progress_callback)
		fastboot_progress_callback(reason);
}
