...
#define FASTBOOT_MAX_BLK_WRITE (16384 * 1024)
...
struct fb_mmc_custom_partition {
	int hwpart;
	lbaint_t start;
	lbaint_t size;
	const char name[16];
...
const struct fb_mmc_custom_partition g_custom_partitions[] = {
	{
		.name   = "mmc0",
		.hwpart = 0,
		.start  = 0x0,
...
static int part_get_custom_partition_info(struct blk_desc *dev_desc,
		const char *name, disk_partition_t *info)
{
	int i;
	const struct fb_mmc_custom_partition *part;

	part = g_custom_partitions;
	for (i=0; i<sizeof(g_custom_partitions)/sizeof(g_custom_partitions[0]); i++) {
		if (strcmp(name, part->name) == 0) {
			/* match */
			if (blk_dselect_hwpart(dev_desc, part->hwpart)) {
				return -1;
			}
			...
			sprintf((char *)info->name, "%s", part->name);
		...
		const char *name, disk_partition_t *info)
{
	...
	ret = part_get_custom_partition_info(dev_desc, name, info);
	if (ret >= 0) {
		return ret;
	}
...
static int fb_mmc_erase_mmc_hwpart(struct blk_desc *dev_desc)
{
	...
	printf("Start Erasing mmc hwpart[%u]...\n", dev_desc->hwpart);
			     ...
			     u32 buff_sz, char *response)
{
	...
	if (buffer) { /* flash */
		...
		printf("Start Flashing Image to EMMC_BOOT1...\n");
		...
		printf("........ wrote %u bytes to EMMC_BOOT1\n",
		       blkcnt * blksz);
	} else { /* erase */
			       ...
			       disk_partition_t *part_info, char *response)
{
	...
	*dev_desc = blk_get_dev("mmc", CONFIG_FASTBOOT_FLASH_MMC_DEV);
	...
	r = part_get_info_by_name(*dev_desc, part_name, part_info);
	if (r < 0) {
		fastboot_fail("partition not found", response);
		return r;
	}
...
void fastboot_mmc_erase(const char *cmd, char *response)
{
	...
	lbaint_t start_rem, blkcnt_rem, total_blks = 0;
	...
	start_rem = blks_start - info.start;
	blkcnt_rem = info.start + info.size - blks_start - blks_size;
	if (start_rem || blkcnt_rem) {
		void *buf = malloc(info.blksz * grp_size);
		if (!buf)
			return;
		memset(buf, 0, info.blksz * grp_size);
		if (start_rem) {
			printf("Erasing blocks " LBAFU " to " LBAFU " before erase_group alignment\n",
					info.start, info.start + start_rem);
			total_blks += fb_mmc_blk_write(dev_desc, info.start, start_rem, buf);
		}
		if (blkcnt_rem) {
			printf("Erasing blocks " LBAFU " to " LBAFU " after erase_group alignment\n",
					blks_start + blks_size, blks_start + blks_size + blkcnt_rem);
			total_blks += fb_mmc_blk_write(dev_desc, blks_start + blks_size, blkcnt_rem, buf);
		}
		free(buf);
	}
