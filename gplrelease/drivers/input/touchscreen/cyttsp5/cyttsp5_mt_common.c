...
static void send_user_event(struct cyttsp5_mt_data *md, u8 prev, u8 cur)
{
...
static void cyttsp5_mt_lift_all(struct cyttsp5_mt_data *md)
{
	int max;
	if (!md->si) {
		dev_vdbg(md->dev, "%s: sysinfo not available\n", __func__);
		return;
	}

	max = md->si->tch_abs[CY_TCH_T].max;
	...
	if (md->num_prv_rec != 0) {
		...
		{
			send_user_event(md, md->num_prv_rec, 0);

			input_report_key(md->input, BTN_TOUCH, 0);
			input_report_key(md->input, BTN_TOOL_FINGER, 0);
			input_report_key(md->input, BTN_TOOL_DOUBLETAP, 0);
			input_report_key(md->input, BTN_TOOL_TRIPLETAP, 0);
		}
		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		 ...
		 */
		if ((md->num_prv_rec == 0 && tch->abs[CY_TCH_O] != CY_OBJ_HOVER)
		...
		if (si->desc.tch_record_size > TOUCH_REPORT_SIZE) {
			/*
...
cyttsp5_get_mt_touches_pr_tch:
	...
	send_user_event(md, md->num_prv_rec, num_cur_tch);

	input_report_key(md->input, BTN_TOUCH, num_cur_tch > 0);
	input_report_key(md->input, BTN_TOOL_FINGER, num_cur_tch >= 1);
	/*
	 * if the current touch record is 2, there could be two cases:
	 *		two finger down: send button 2 down
	 *		one finger down: send button 2 down anyway
	 * basically we are terminating two finger tap 1 frame later because we might have not sent that before.
	 * if we don't do that, we might have missed button 2 down.
	 * */
	input_report_key(md->input, BTN_TOOL_DOUBLETAP, num_cur_tch >= 2);
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	__set_bit(BTN_TOOL_DOUBLETAP, md->input->keybit);
	__set_bit(BTN_TOOL_TRIPLETAP, md->input->keybit);
	...
	for (i = 0; i < NUM_SIGNALS(md->pdata->frmwrk); i++) {
		...
		if (signal != CY_IGNORE_VALUE) {
			...
			if ((i == CY_ABS_ID_OST) &&
				(md->si->desc.tch_record_size <= TOUCH_REPORT_SIZE))
				break;
		}
	}
...
/***** Allow force lift-off on resume if finger was present on suspend *****/
void cyttsp5_core_lift_all(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_mt_data *md = &cd->md;
