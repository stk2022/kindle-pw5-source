...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	/* Platform Power */
	vcc_vdd = regulator_get(adap_dev, "vcc_vdd");
	if (IS_ERR(vcc_vdd)) {
		vcc_vdd = NULL;
		pr_err("regulator get of vcc_vdd failed\n");
	}

	vcc_ana = regulator_get(adap_dev, "vcc_ana");
	if (IS_ERR(vcc_ana)) {
		vcc_ana = NULL;
		pr_err("regulator get of vcc_ana failed\n");
	}
...
#ifdef CONFIG_LAB126
		rc = of_property_read_string(core_node, "cy,fw_board_prefix",
					&pdata->core_pdata->fw_board_prefix);
		if (!rc)
			pr_debug("%s: fw_board_prefix:%s\n", __func__, pdata->core_pdata->fw_board_prefix);
		else
			pdata->core_pdata->fw_board_prefix = NULL;	/* No fw_board_prefix in device tree */
#endif

		pdata->core_pdata->vcc_vdd = vcc_vdd;
		pdata->core_pdata->vcc_ana = vcc_ana;
