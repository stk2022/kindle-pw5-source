...
static struct cyttsp5_core_data * g_core_data = NULL;
...
static ssize_t cyttsp5_touch_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t cyttsp5_touch_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *cyttsp5_touch_proc_entry = NULL;
static const struct file_operations cyttsp5_touch_fops = {
	.owner = THIS_MODULE,
	.read = cyttsp5_touch_read_proc,
	.write = cyttsp5_touch_write_proc,
};

static ssize_t cyttsp5_misc_write_not_implemented(struct file *file, const char __user *buf,
			size_t count, loff_t *pos);
static ssize_t cyttsp5_misc_read_not_implemented(struct file *file, char __user *buf,
			size_t count, loff_t *pos);
static long cyttsp5_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg);

static const struct file_operations cyttsp5_misc_fops =
{
	.owner = THIS_MODULE,
	.read  = cyttsp5_misc_read_not_implemented,
	.write = cyttsp5_misc_write_not_implemented,
	.unlocked_ioctl = cyttsp5_core_ioctl,
};

static struct miscdevice cyttsp5_misc_device =
{
	.minor = 0,
	.name  = TOUCH_MISC_DEV_NAME,
	.fops  = &cyttsp5_misc_fops,
};
		...
		u16 row_number, u16 length, u8 ebid, u8 *read_buf, u16 *crc)
{
	...
	if (length < read_length) {
		dev_err(cd->dev, "%s: output length does not match\n", __func__);
		return -EINVAL;
	}
...
static void cyttsp5_start_wd_timer(struct cyttsp5_core_data *cd)
{
	...
	cd->watchdog_enabled = 1;			// WDT status variable
}
...
static void cyttsp5_stop_wd_timer(struct cyttsp5_core_data *cd)
{
	...
	del_timer_sync(&cd->watchdog_timer);
	cd->watchdog_enabled = 0;					// WDT status variable
}
...
static int cyttsp5_grip_get_param(struct cyttsp5_core_data* cd, struct cyttsp5_grip_suppression_data* data)
{
	u32 value;

	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_XA, &value);
	data->xa = value;
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_XB, &value);
	data->xb = value;
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_XEXA, &value);
	data->xexa = value;
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_XEXB, &value);
	data->xexb = value;

	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_YA, &value);
	data->ya = value;
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_YB, &value);
	data->yb = value;
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_YEXA, &value);
	data->yexa = value;
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_YEXB, &value);
	data->yexb = value;

	return 0;
}

static int cyttsp5_grip_set_param(struct cyttsp5_core_data* cd, struct cyttsp5_grip_suppression_data* data)
{
	int ret = 0;

	printk(KERN_DEBUG "cyttsp5:setting grip suppression in pixels: %d %d %d %d %d %d %d %d",
		data->xa, data->xb, data->xexa, data->xexb, data->ya, data->yb, data->yexa, data->yexb);

	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_XA, data->xa, 2);
	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_XB, data->xb, 2);
	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_XEXA, data->xexa, 2);
	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_XEXB, data->xexb, 2);

	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_YA, data->ya, 2);
	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_YB, data->yb, 2);
	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_YEXA, data->yexa, 2);
	ret += cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_YEXB, data->yexb, 2);

	return ret;
}

static ssize_t cyttsp5_grip_suppression_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	...
	for ( ;cmd <= CY_OPT_PARAM_GRP_EXC_EDG_ORG; cmd++) {
		cyttsp5_hid_output_get_param(cd, cmd, &data);
...
static ssize_t cyttsp5_grip_suppression_store(struct device *dev,
			struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	struct cyttsp5_grip_suppression_data gsd;
	int ret;

	if(9 != sscanf(buf, " %hd, %hd, %hd, %hd, %hd, %hd, %hd, %hd",
	...
	pm_runtime_get_sync(dev);
	ret = cyttsp5_grip_set_param(cd, &gsd);
	pm_runtime_put(dev);

	return ret == 0 ? size : ret;
}

static ssize_t cyttsp5_enable_grip_suppression_store(struct device *dev,
			struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	u16 state;
	...
	pm_runtime_get_sync(dev);
	ret = cyttsp5_hid_output_set_param(cd, CY_OPT_PARAM_GRP_SUP_EN, state, 1);
	if (ret == 0)
...
static ssize_t cyttsp5_enable_grip_suppression_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	u32 data = 0;
	...
	pm_runtime_get_sync(dev);
	cyttsp5_hid_output_get_param(cd, CY_OPT_PARAM_GRP_SUP_EN, &data);
	pm_runtime_put(dev);
...
reset:
	dev_info(cd->dev, "%s: Beginning _fast_startup retry loop\n", __func__);
	msleep (100); // Cohiba: wait for power to be good
...
#ifdef FORCE_LIFT_OFF_ON_WAKE
	cyttsp5_core_lift_all(cd);
#endif /* FORCE_LIFT_OFF_ON_WAKE */
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case CY_DBG_SUSPEND:
		...
		rc = cyttsp5_core_sleep(cd);
		/*mpu: disabling WDT after suspend */
		wd_disabled = 1;
		if (rc)
...
static struct device_attribute attributes[] = {
	...
	__ATTR(grip_suppression, S_IRUSR | S_IWUSR,
		cyttsp5_grip_suppression_show, cyttsp5_grip_suppression_store),
	__ATTR(enable_grip_suppression, S_IRUSR | S_IWUSR,
		cyttsp5_enable_grip_suppression_show, cyttsp5_enable_grip_suppression_store),
};
...
static int add_sysfs_interfaces(struct device *dev)
{
	...
	cyttsp5_touch_proc_entry = proc_create("touch", 0660, NULL, &cyttsp5_touch_fops);
	if (cyttsp5_touch_proc_entry == NULL) {
		dev_err(dev, "Create proc entry /proc/touch FAILED!\n");
		goto undo;
	}
...
/**** TOUCH PROC ENTRY ****/
static ssize_t cyttsp5_touch_read_proc(struct file *file, char __user *buf,
			size_t count, loff_t *off)
{
	...
	if (g_core_data->sleep_state != SS_SLEEP_OFF)
		len = sprintf(buf, "Touch is locked\n");
	else
		len = sprintf(buf, "Touch is unlocked\n");
...
static ssize_t cyttsp5_touch_write_proc(struct file *filp, const char __user *buf,
			size_t len, loff_t *off)
{
	...
	if (g_core_data == NULL) {
		printk(KERN_ERR "cyttsp5_touch_write_proc:proc:not ready\n");
		return -EFAULT;
	}
	if (len >= 16){
		printk(KERN_ERR "cyttsp5_touch_write_proc:proc:command is too long!\n");
		return -ENOSPC;
	}

	if (copy_from_user(command, buf, len)) {
		printk(KERN_ERR "cyttsp5_touch_write_proc:proc::cannot copy from user!\n");
		return -EFAULT;
	}

	if ( !strncmp(command, "unlock", 6) ) {
		printk(KERN_INFO "cyttsp5: cyttsp5_touch_write_proc::command=unlock:\n");
		sts = cyttsp5_core_rt_resume(g_core_data->dev);
		if (sts < 0) {
				printk(KERN_ERR "cyttsp5_touch_write_proc:proc:command=%s:not succeed please retry\n", command);
				return -EBUSY;
		}
		g_core_data->exlock = false;
	} else if ( !strncmp(command, "lock", 4) ) {
		printk(KERN_INFO "cyttsp5: cyttsp5_touch_write_proc::command=lock:\n");
		sts = cyttsp5_core_rt_suspend(g_core_data->dev);
		if (sts < 0){
				printk(KERN_ERR "cyttsp5_touch_write_proc:proc:command=%s:not succeed please retry\n", command);
				return -EBUSY;
		}
		g_core_data->exlock = true;
	        ...
	        printk(KERN_ERR "cyttsp5_touch_write_proc:proc:command=%s:Unrecognized command\n", command);
	}
...
static long cyttsp5_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case TOUCH_IOCTL_GRIP_SUP_EN:
		{
			...
			printk(KERN_INFO "cyttsp5: Doing grip suppression enable : %d\n", grip_en);
			ret = cyttsp5_hid_output_set_param(g_core_data, CY_OPT_PARAM_GRP_SUP_EN, grip_en, 1);
			if (ret == 0)
				g_core_data->grip_sup_en = grip_en;
			pm_runtime_put(g_core_data->dev);
			break;
		}
		case CY5_IOCTL_GRIP_SET_DATA:
		{
			if(copy_from_user(&gsd, argp, sizeof(struct cyttsp5_grip_suppression_data)))
				return -EFAULT;
			pm_runtime_get_sync(g_core_data->dev);
			ret = cyttsp5_grip_set_param(g_core_data, &gsd);
			pm_runtime_put(g_core_data->dev);
			break;
		}
		case CY5_IOCTL_GRIP_GET_DATA:
		{
			struct cyttsp5_grip_suppression_data gdata;
			pm_runtime_get_sync(g_core_data->dev);
			ret = cyttsp5_grip_get_param(g_core_data, &gdata);
			pm_runtime_put(g_core_data->dev);
			if (copy_to_user(argp, &gdata, sizeof(struct cyttsp5_grip_suppression_data)))
				return -EFAULT;
			break;
		}
		...
		case TOUCH_IOCTL_SET_LOCK:
		{
			...
			if (locked) {
				int sts;
				printk(KERN_INFO "cyttsp5: cyttsp5_touch_write_ioctl::command=lock:\n");
				sts = cyttsp5_core_sleep(g_core_data);
				if (sts < 0){
					printk(KERN_ERR "cyttsp5_proc_write:ioctl:command=lock:not succeed please retry\n");
					return -EBUSY;
				}
				g_core_data->exlock = true;
			} else {
				int sts;
				printk(KERN_INFO "cyttsp5: cyttsp5_touch_write_ioctl::command=unlock:\n");
				sts = cyttsp5_core_wake(g_core_data);
				if (sts < 0) {
					printk(KERN_ERR "cyttsp5_proc_write:ioctl:command=unlock:not succeed please retry\n");
					return -EBUSY;
				}
				g_core_data->exlock = false;
			}
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
	...
	/* Initialize IRQ */
	if (gpio_is_valid(cd->cpdata->irq_gpio)) {
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->bus_ops = ops;
	cd->watchdog_enabled = 0;     //WDT state variable
	...
	rc = misc_register(&cyttsp5_misc_device);
	if (rc < 0) {
...
error_startup:
	...
	cyttsp5_stop_wd_timer(cd);
	cyttsp5_free_si_ptrs(cd);
	misc_deregister(&cyttsp5_misc_device);
error_misc_create:
