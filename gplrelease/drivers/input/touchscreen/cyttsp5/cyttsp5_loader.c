...
static int cyttsp5_check_firmware_version(struct device *dev,
		u32 fw_ver_new, u32 fw_revctrl_new, u32 fw_cfg_new)
{
	...
	fw_cfg_img = ld->si->cydata.fw_ver_conf;

	dev_dbg(dev, "%s: img config:0x%04X new config:0x%04X\n",
			__func__, fw_cfg_img, fw_cfg_new);

	if (fw_cfg_new > fw_cfg_img) {
		dev_dbg(dev, "%s: Image is newer, will upgrade\n",
				__func__);
		return 1;
	}
		...
		struct cyttsp5_touch_firmware *fw)
{
	...
	fw_cfg_new = get_unaligned_be16(fw->ver + 16);

	upgrade = cyttsp5_check_firmware_version(dev, fw_ver_new,
		fw_revctrl_new, fw_cfg_new);
		...
		const struct firmware *fw)
{
	...
	fw_cfg_new = get_unaligned_be16(fw->data + 17);
...
#define FILENAME_LEN_MAX 64
	...
	panel_id = cyttsp5_get_panel_id(dev);
#ifdef CONFIG_LAB126
	if (pdata->core_pdata->fw_board_prefix)
		if (panel_id == PANEL_ID_NOT_ENABLED)
			snprintf(filename, FILENAME_LEN_MAX, "%s%s_%s",
				CY_FW_FILE_DIR, pdata->core_pdata->fw_board_prefix, CY_FW_FILE_NAME);
		else
			snprintf(filename, FILENAME_LEN_MAX, "%s%s_%s_pid%02X%s",
				CY_FW_FILE_DIR, pdata->core_pdata->fw_board_prefix, CY_FW_FILE_PREFIX,
				panel_id, CY_FW_FILE_SUFFIX);
	else
#endif
	if (panel_id == PANEL_ID_NOT_ENABLED)
		snprintf(filename, FILENAME_LEN_MAX, "%s%s", CY_FW_FILE_DIR, CY_FW_FILE_NAME);
	else
		snprintf(filename, FILENAME_LEN_MAX, "%s%s_pid%02X%s",
			CY_FW_FILE_DIR, CY_FW_FILE_PREFIX, panel_id, CY_FW_FILE_SUFFIX);
