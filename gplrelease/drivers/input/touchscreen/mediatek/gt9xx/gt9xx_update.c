...
#define REQUEST_CONFIG_FILE_PATH "data/"
#define REQUEST_UPDATE_FILE_PATH "data/"

#define REQUEST_CONFIG_NAME REQUEST_CONFIG_FILE_PATH "_goodix_config_.cfg"
#define REQUEST_FIRMWARE_NAME REQUEST_UPDATE_FILE_PATH "_goodix_update_.bin"
...
/* parse config data*/
	...
	if (ret == SUCCESS) {
		GTP_DEBUG(
			...
			sensor_id, config[GTP_ADDR_LENGTH], opr_buf[0]);
...
//  3. the firmware version in update file is greater than the firmware version
//  in ic
...
// Get the correct nvram config data
// The correct conditions:
//  1. the config version in update file is greater than the config version in
//  ic
/* Update Conditions:
    1. File CFG Ver > IC CFG Ver
*/

static u8 gup_enter_update_config_judge(struct i2c_client *client,
					u8 *file_data)
{
	u8 rd_buf[5];
	s32 ret = 0;

	ret = gup_get_ic_msg(client, GTP_REG_CONFIG_DATA, rd_buf, 1);
	if (ret == FAIL) {
		GTP_ERROR("Read config message fail.");
		return ret;
	}

	GTP_INFO("FILE CFG Ver:%d", file_data[GTP_ADDR_LENGTH]);
	GTP_INFO("IC CFG Ver:%d", rd_buf[GTP_ADDR_LENGTH]);

#if GTP_AUTO_UPDATE_CFG_FORCE
	if (file_data[GTP_ADDR_LENGTH] != rd_buf[GTP_ADDR_LENGTH])
#else
	if (file_data[GTP_ADDR_LENGTH] > rd_buf[GTP_ADDR_LENGTH])
#endif
	{
		GTP_INFO("Need enter config update.");
		return SUCCESS;
	}

	GTP_INFO("File CFG Ver <= Ic CFG Ver, update config aborted!");
...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	if (!update_msg.cfg_fw || !update_msg.cfg_fw->data) {
		GTP_ERROR("[update_cfg]No need to upgrade config!");
		return FAIL;
	}

	file_len = update_msg.cfg_fw->size;
	...
	pre_buf = update_msg.cfg_fw->data;
	...
	ret = gup_enter_update_config_judge(client, file_config);
	if (ret <= 0) {
		if (ret < 0)
			GTP_ERROR("[update_cfg]Check cfg, UPDATE FAIL.");
		...
		if (search_type & AUTO_SEARCH_BIN) {
			...
			if (got_file_flag & BIN_FILE_READY) {
				...
				if (search_type & AUTO_SEARCH_CFG) {
					i = GUP_SEARCH_FILE_TIMES; // Bin & Cfg
								   // File
								   // required
								   // to be in
								   // the same
				...
				u8 *path)
{
	...
	if (path) {
		...
		if (sizeof(gtp_default_FW) <
			...
			GTP_ERROR(
				"INVALID gtp_default_FW, check your gt9xx_firmware.h file!");
			return FAIL;
		}
		...
		if (got_file_flag & CFG_FILE_READY) {
			...
			if (ret > 0)
				msleep(500); // waiting config to be stored in
					     // FLASH.
		}
#else
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	...
	{
		if (!update_msg.fw) {
			GTP_ERROR("firmware file haven't loaded");
			return -EFAULT;
		}
		...
		if (set_or_end == SEEK_SET) {
			if (update_msg.fw->size <
				...
				GTP_ERROR("No enough firmware data for read");
				return -EFAULT;
			}
			memcpy(buf,
			...
			memcpy(buf,
			       &update_msg.fw
					->data[update_msg.fw->size - offset],
			       length);
		}
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	u16 start_index = 4 * FW_SECTION_LENGTH + FW_DSP_LENGTH +
			  FW_BOOT_LENGTH + FW_BOOT_ISP_LENGTH +
			  FW_GLINK_LENGTH; // 32 + 4 + 2 + 4 = 42K
	// u16 start_index;
...
static s32 gup_check_fw_file(st_fw_head *fw_head)
{
	int i;
	u16 fw_checksum;
	...
	fw_checksum = 0;
	for (i = FW_HEAD_LENGTH; i < update_msg.fw->size; i += 2) {
		fw_checksum += (update_msg.fw->data[i] << 8) +
			       update_msg.fw->data[i + 1];
	}
	if (fw_checksum & 0xFFFF) {
		GTP_ERROR("Firmware check sum error");
		return -EFAULT;
	}

	memcpy(fw_head, update_msg.fw->data, FW_HEAD_LENGTH);
	return 0;
}
...
s32 gup_update_proc(void *dir)
{
	...
	if (searching_file) {
		...
		while ((show_len != 200) && (show_len != 100) &&
		       (timeout++ <
			100)) // wait for auto update quitted completely
		{
			msleep(100);
		}
	}
	...
	if (0) {
		...
		ret = gup_check_update_file(i2c_connect_client, &fw_head,
					    (u8 *)dir); // 20121211
		if (ret == FAIL) {
	...
	update_msg.fw = NULL;
	update_msg.cfg_fw = NULL;

#if GTP_AUTO_UPDATE_CFG
	ret = request_firmware(&update_msg.cfg_fw, REQUEST_CONFIG_NAME,
			       &i2c_connect_client->dev);
	if (ret < 0) {
		GTP_ERROR("[update_proc] failed request config %s",
			  REQUEST_CONFIG_NAME);
		update_msg.cfg_fw = NULL;
	}
	ret = gup_update_config(i2c_connect_client);
	if (ret < 0)
		...
		msleep(1000); // waiting config to be stored in FLASH.
#endif

	ret = request_firmware(&update_msg.fw, REQUEST_FIRMWARE_NAME,
			       &i2c_connect_client->dev);
	if (ret < 0) {
		GTP_ERROR("[update_proc] failed request firmware %s",
			  REQUEST_FIRMWARE_NAME);
		update_msg.fw = NULL;
		goto request_fail;
	}
	ret = gup_check_fw_file(&fw_head);
	if (ret) {
		GTP_ERROR("[update_proc] illgal firmware %s",
			  REQUEST_FIRMWARE_NAME);
		goto request_fail;
	}
...
request_fail:
	...
	if (update_msg.cfg_fw)
		release_firmware(update_msg.cfg_fw);
...
s32 gup_clk_calibration(void)
{
	...
	for (i = INIT_CLK_DAC; i < MAX_CLK_DAC; i++) {
		...
		if (ts->gtp_is_suspend) {
			i = 72; // 80;    // if sleeping while calibrating main
				// clock, set it default 72
			break;
		}
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
	...
	if (dwn_mode == GTP_FL_FW_BURN) {
		...
		ret = gup_burn_fw_proc(client, 0xC000, 2 * FW_DOWNLOAD_LENGTH,
				       FW_DSP_LENGTH); // write the second ban
		if (ret == FAIL) {
		...
		ret = gup_check_and_repair(
			client, 0xC000, 2 * FW_DOWNLOAD_LENGTH, FW_DSP_LENGTH);
		if (ret == FAIL) {
			GTP_ERROR("check fw dsp failed!");
			return FAIL;
		}
	} else if (dwn_mode == GTP_FL_ESD_RECOVERY) {
		GTP_INFO("[download_fw_dsp]Begin esd check dsp fw---->>");
		// GTP_INFO("esd recovery: check fw dsp");
		// ret = gup_check_and_repair(client, 0xC000, 2 *
		// FW_DOWNLOAD_LENGTH, FW_DSP_LENGTH);
		...
		for (i = 0; i < cmp_len; ++i) {
			if (chk_cmp_buf[i] !=
				...
				i2c_write_bytes(
					...
					cmp_len - i);
				GTP_ERROR(
					"Check failed index: %d(%d != %d), redownload chuck",
					i, chk_cmp_buf[i],
					gtp_default_FW_fl[FW_HEAD_LENGTH +
							  start_index + i]);
				break;
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	for (section = 1; section <= 4; section += 2) {
		...
		if (dwn_mode == GTP_FL_FW_BURN) {
			GTP_DEBUG("download firmware ss51 section%d & %d",
				  section, section + 1);
			...
			GTP_DEBUG("check firmware ss51 section%d & %d", section,
				  section + 1);
		...
		} else if (dwn_mode == GTP_FL_ESD_RECOVERY) // esd recovery mode
		{
			...
			// if (ret == FAIL)
			{
				// GTP_ERROR("check ss51 section%d failed,
				// redownload section%d", section, section);
				GTP_DEBUG(
					"esd recovery redownload ss51 section%d & %d",
					section, section + 1);
				...
				if (ret == FAIL) {
					GTP_ERROR(
						"download fw ss51 section%d failed!",
						section);
					return FAIL;
				}
			}
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
	...
	update_msg.file->f_op->llseek(update_msg.file, 0, SEEK_SET);
	ret = update_msg.file->f_op->read(
		update_msg.file, (char *)gtp_default_FW_fl,
		update_msg.fw_total_len + FW_HEAD_LENGTH,
		&update_msg.file->f_pos);
	update_msg.fw_total_len += FW_HEAD_LENGTH;
	set_fs(update_msg.old_fs);
...
s32 gup_fw_download_proc(void *dir, u8 dwn_mode)
{
	...
	ts = i2c_get_clientdata(i2c_connect_client);
	if (dir == NULL) {
		if (dwn_mode == GTP_FL_FW_BURN) // GT9XXF firmware burn mode
		{
			GTP_INFO("[fw_download_proc]Begin fw download ......");
		} else if (GTP_FL_ESD_RECOVERY ==
			   dwn_mode) // GTP_FL_ESD_RECOVERY: GT9XXF esd recovery
				     // mode
		{
			GTP_INFO(
	...
	ret = gup_check_update_file_fl(i2c_connect_client, &fw_head,
				       (char *)dir);
	show_len = 10;
