...
#if GTP_DEBUG_ON
static const int key_codes[] = {KEY_HOME, KEY_BACK, KEY_MENU, KEY_HOMEPAGE,
				KEY_F1,   KEY_F2,   KEY_F3};
static const char *key_names[] = {"Key_Home",     "Key_Back", "Key_Menu",
				  "Key_Homepage", "KEY_F1",   "KEY_F2",
				  "KEY_F3"};
#endif
...
static ssize_t gtp_touch_read_proc(struct file *, char __user *, size_t,
				   loff_t *);
static ssize_t gtp_touch_write_proc(struct file *, const char __user *, size_t,
				    loff_t *);

static struct proc_dir_entry *gtp_touch_proc_entry;
static const struct file_operations gtp_touch_fops = {
	.owner = THIS_MODULE,
	.read = gtp_touch_read_proc,
	.write = gtp_touch_write_proc,
};

static ssize_t gtp_misc_write_not_implemented(struct file *file,
					      const char __user *buf,
					      size_t count, loff_t *pos);
static ssize_t gtp_misc_read_not_implemented(struct file *file,
					     char __user *buf, size_t count,
					     loff_t *pos);
static long gtp_core_ioctl(struct file *file, unsigned int cmd,
			   unsigned long arg);

static const struct file_operations gtp_misc_fops = {
	.owner = THIS_MODULE,
	.read = gtp_misc_read_not_implemented,
	.write = gtp_misc_write_not_implemented,
	.unlocked_ioctl = gtp_core_ioctl,
};

static struct miscdevice gtp_misc_device = {
	.minor = 0, .name = TOUCH_MISC_DEV_NAME, .fops = &gtp_misc_fops,
};
...
Input:
    ts: goodix i2c_client private data
    pre: previoud touches
    cur: current touches
Output:
    None.
*********************************************************/
static void send_user_event(struct goodix_ts_data *ts, u8 prev, u8 cur)
{
	struct device *dev = &ts->input_dev->dev;
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	// GT9XXF reques event
	if ((finger == 0x00) &&
	    (ts->chip_type == CHIP_TYPE_GT9F)) // request arrived
	{
		...
		for (i = 0; i < GTP_MAX_KEY_NUM; i++) {
			...
			for (ret = 0; ret < GTP_MAX_KEY_NUM; ++ret) {
				if (key_codes[ret] == touch_key_array[i]) {
					GTP_DEBUG("Key: %s %s", key_names[ret],
						  (key_value & (0x01 << i))
							  ? "Down"
							  : "Up");
					break;
	...
	if (pre_touch || touch_num) {
		...
		if (touch_num) {
			...
			id = coor_data[pos];
			input_x =
				coor_data[pos + 1] | (coor_data[pos + 2] << 8);
			input_y =
				coor_data[pos + 3] | (coor_data[pos + 4] << 8);
			input_w =
				coor_data[pos + 5] | (coor_data[pos + 6] << 8);
			if ((id & 0x80)) {
				if (!input_w) {
		...
		send_user_event(ts, pre_touch, touch_num);
	...
	if (touch_num) {
		for (i = 0; i < touch_num; i++) {
			...
			if ((id & 0x80) && !input_w) {
				GTP_DEBUG("Pen touch DOWN:(%d)(%d, %d)[%d]", id,
...
#if GTP_WITH_HOVER
		GTP_DEBUG("@@@@ touch UP ,pre_pen is %d,pre_finger is %d!",
			  pre_pen, pre_finger);
		if (pre_pen) {
			GTP_DEBUG("Pen touch UP!");
			gtp_pen_up(0);
...
/* parse config data*/
	...
	ret = gtp_parse_dt_cfg(&ts->client->dev, &config[GTP_ADDR_LENGTH],
				       ...
				       size_t size, loff_t *ppos)
{
	...
	/* Touch PID & VID */
	ptr += sprintf(ptr, "\n==== GT9XX Version Info ====\n");
	temp_data[0] = GTP_REG_VERSION >> 8;
	temp_data[1] = GTP_REG_VERSION & 0xff;
	ret = gtp_i2c_read(i2c_connect_client, temp_data, 8);
	if (ret < 0) {
		ptr += sprintf(ptr, "Chip PID: NG  VID: NG\n");
	} else {
		if (temp_data[5] == 0x00)
			ptr += sprintf(ptr, "PID: %c%c%c VID:%02x%02x\n",
				       temp_data[2], temp_data[3], temp_data[4],
				       temp_data[7], temp_data[6]);
		else
			ptr += sprintf(ptr, "PID: %c%c%c%c VID:%02x%02x\n",
				       temp_data[2], temp_data[3], temp_data[4],
				       temp_data[5], temp_data[7],
				       temp_data[6]);
	}

	temp_data[0] = GTP_REG_BOOT_STATUS >> 8;
	temp_data[1] = GTP_REG_BOOT_STATUS & 0xff;
	gtp_i2c_read(i2c_connect_client, temp_data, 3);
	ptr += sprintf(ptr, "Boot status 0x%X\n", temp_data[2]);

	/* sensor ID */
	temp_data[0] = GTP_REG_SENSOR_ID >> 8;
	temp_data[1] = GTP_REG_SENSOR_ID & 0xff;
	ret = gtp_i2c_read(i2c_connect_client, temp_data, 3);
	ptr += sprintf(ptr, "SensorID: %d\n", temp_data[2]);
					...
					loff_t *off)
{
	...
	if (copy_from_user(buf, buffer, count)) {
		GTP_ERROR("failed copy config from __user memory\n");
		return -EFAULT;
	}

	if (!memcmp(buf, "fwupdate", strlen("fwupdate"))) {
#if GTP_AUTO_UPDATE
		/*firmware update command*/
		ret = gup_init_update_proc(ts);
		if (ret < 0)
			GTP_ERROR("failed create update thread\n");
		else
			GTP_INFO("success create update thread\n");
#endif
	} else {
		if (count != GTP_CONFIG_MAX_LENGTH) {
			GTP_ERROR("size not match [%d:%d]\n",
...
/**** TOUCH PROC ENTRY ****/
static ssize_t gtp_touch_read_proc(struct file *file, char __user *buf,
				   size_t count, loff_t *off)
{
	...
	if (ts == NULL) {
		GTP_ERROR("gtp_proc_read:proc:not ready\n");
		return -EFAULT;
	}
...
static ssize_t gtp_touch_write_proc(struct file *filp, const char __user *buf,
				    size_t len, loff_t *off)
{
	...
	if (ts == NULL) {
		GTP_ERROR("gtp_touch_write_proc:proc:not ready\n");
		return -EFAULT;
	}
	if (len >= 16) {
		GTP_ERROR("gtp_touch_write_proc:proc:command is too long!\n");
		return -ENOSPC;
	}

	if (copy_from_user(command, buf, len)) {
		GTP_ERROR(
			"gtp_touch_write_proc:proc::cannot copy from user!\n");
		return -EFAULT;
	}

	if (!strncmp(command, "unlock", 6)) {
		pr_info(
			"gtp: gtp_touch_write_proc::command=unlock:\n");
		goodix_ts_resume(ts);
		ts->exlock = false;
	} else if (!strncmp(command, "lock", 4)) {
		pr_info("gtp: gtp_touch_write_proc::command=lock:\n");
		goodix_ts_suspend(ts);
		ts->exlock = true;
	} else {
		GTP_ERROR(
			"gtp_touch_write_proc:proc:command=%s:Unrecognized command\n",
			command);
	}
			   ...
			   unsigned long arg)
{
	...
	case TOUCH_IOCTL_SET_LOCK: {
		...
		if (locked) {
			pr_info(
				"gtp: gtp_touch_write_ioctl::command=lock:\n");
			goodix_ts_suspend(ts);
			ts->exlock = true;
		} else {
			pr_info(
				"gtp: gtp_touch_write_ioctl::command=unlock:\n");
			goodix_ts_resume(ts);
			ts->exlock = false;
		}
		break;
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TOOL_TYPE, 0, MT_TOOL_MAX, 0,
			     0);
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
	...
	if (IS_ERR(clk_filp)) {
		...
		if (ret == FAIL) {
			// recalculate main clock & rewrite main clock data to
			// file
			GTP_ERROR(
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (on == 1) {
		...
		udelay(2);
		//if (!regulator_is_enabled(vcc_i2c))
		...
		if (on == -1) {
			GTP_INFO("GTP Regulators release.");
			regulator_put(vcc_i2c);
			regulator_put(vdd_ana);
			GTP_INFO("GTP INT & RST GPIOs release.");
			GTP_GPIO_FREE(gtp_rst_gpio);
			GTP_GPIO_FREE(gtp_int_gpio);
		}
	}
...
// ts->esd_lock = SPIN_LOCK_UNLOCKED;
	...
	// Create touch proc file system
	gtp_touch_proc_entry =
		proc_create(GTP_TOUCH_PROC_FILE, 0644, NULL, &gtp_touch_fops);
	if (gtp_touch_proc_entry == NULL) {
		GTP_ERROR("create_proc_entry %s failed\n", GTP_TOUCH_PROC_FILE);
		return -ENODEV;
	} else {
		GTP_INFO("create proc entry %s success", GTP_TOUCH_PROC_FILE);
	}

	// Register misc dev file system
	ret = misc_register(&gtp_misc_device);
	if (ret < 0) {
		GTP_ERROR("Error, register misc device\n");
...
error_misc_create:
	remove_proc_entry(GTP_TOUCH_PROC_FILE, NULL);
	gtp_touch_proc_entry = NULL;
error_io_request:
	kfree(ts);
#ifdef GTP_CONFIG_OF /* device tree support */
	GTP_INFO("GTP Releasing Power...");
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	misc_deregister(&gtp_misc_device);
...
static int goodix_resume_work(void *ts)
{
	gtp_wakeup_sleep((struct goodix_ts_data *)ts);
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
	...
	GTP_DEBUG_FUNC();
	if (!ts->gtp_is_suspend || ts->enter_update) {
		GTP_DEBUG("Already resumed!");
		return;
	}
	GTP_INFO("System resume.");

	thread = kthread_run(goodix_resume_work, ts, "goodix");
	if (IS_ERR(thread)) {
		ret = PTR_ERR(thread);
		pr_info(" failed to create kernel thread: %d\n", ret);
	}
...
static struct dev_pm_ops gtp_pm_ops = {
	.suspend = gtp_pm_suspend, .resume = gtp_pm_resume,
};
#endif
