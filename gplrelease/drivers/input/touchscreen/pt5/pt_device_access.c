...
enum print_buffer_format {
	...
	PT_PR_FORMAT_SELFTEST_U8    = 1,
	PT_PR_FORMAT_SELFTEST_U16   = 2,
...
#define TTHE_TUNER_MAX_BUF	(PT_MAX_PRBUF_SIZE * 4)
...
start_testing:
	...
	if (!dad->cmcp_force_calibrate) {
		...
		rc = _pt_calibrate_idacs_cmd(dev, 0, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Err calibrating idacs for mutual rc=%d\n",
				__func__, rc);
		}
		rc = _pt_calibrate_idacs_cmd(dev, 1, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Err calibrating idacs for btns rc=%d\n",
				__func__, rc);
		}
		rc = _pt_calibrate_idacs_cmd(dev, 2, &status);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR,
				"%s: Err calibrating idacs for self rc=%d\n",
				__func__, rc);
		}
	}
	/*resume_scanning */
	...
	self_test_id_supported = pt_get_cmcp_info(dad, cmcp_info);
	...
	while (retry--) {
		cmd->request_get_fw_mode(dev, PT_CORE_CMD_UNPROTECTED, &mode);
 ...
 *   out_format   - format of ouput data - 3 formats are provide,
 *    PT_PR_FORMAT_DEFAULT      : format all data as a column
 *    PT_PR_FORMAT_SELFTEST_U8  : sort status bytes and self test results,
 *      and format the results as a row, each element include 1 byte
 *    PT_PR_FORMAT_SELFTEST_U16 : sort status bytes and self test results,
		...
		u8 *out_buf, size_t out_buf_size, u8 out_format)
{
	...
	if (out_format == PT_PR_FORMAT_DEFAULT) {
		for (i = 0; i < length; i++)
			index += scnprintf(&out_buf[index],
				out_buf_size - index,
				"%02X\n", in_buf[i]);
	} else if (out_format == PT_PR_FORMAT_SELFTEST_U8) {
		index += scnprintf(&out_buf[index],
			out_buf_size - index,
	...
	} else if (out_format == PT_PR_FORMAT_SELFTEST_U16) {
		index += scnprintf(&out_buf[index],
			out_buf_size - index,
			"Response Status[1-%d]: ", MIN(length, 3));
		for (i = 0; i < MIN(length, 3); i++)
		...
		size_t count, loff_t *ppos)
{
	...
	if (!*ppos) {
		...
		if (dad->fw_self_test_param_len > 0) {
			cmd->request_get_fw_mode(dad->dev,
