...
#ifdef CONFIG_LAB126
/*******************************************************************************
 * FUNCTION: send_user_event
 *
 * SUMMARY: Send touch events to user-space.
 *
 * PARAMETERS:
 *     *md - pointer to touch data structure
 ******************************************************************************/
static void send_user_event(struct pt_mt_data *md, u8 prev, u8 cur)
{
...
static int pt_xy_worker(struct pt_mt_data *md)
{
	...
	if (tch.hdr[PT_TCH_LO]) {
		pt_debug(dev, DL_WARN, "%s: Large area detected with noise level=%d\n",
			__func__, tch.hdr[PT_TCH_NOISE]);
 ...
 *     *dev - pointer to device structure
 ******************************************************************************/
static int pt_sleep_attention(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	struct pt_mt_data *md = &cd->md;
...
static int pt_mt_open(struct input_dev *input)
{
	...
	/* set up sleep call back */
	_pt_subscribe_attention(dev, PT_ATTEN_SLEEP, PT_MT_NAME,
		pt_sleep_attention, 0);
...
static void pt_mt_close(struct input_dev *input)
{
	...
	_pt_unsubscribe_attention(dev, PT_ATTEN_SLEEP, PT_MT_NAME,
		pt_sleep_attention, 0);
