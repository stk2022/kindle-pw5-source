		...
		int on, struct device *dev)
{
	...
	if (on && rst_gpio) {
		...
		if (rc < 0) {
			 ...
			 * The start level is low because the DUT needs
	...
	if (on) {
		...
		if (!vcc_ana) {
			...
			if (regulator_is_enabled(vcc_ana)) {
				pt_debug(dev, DL_INFO, "%s: disabling vcc_ana\n", __func__);
			...
			if (!vcc_vdd) {
				...
				if (regulator_is_enabled(vcc_vdd)) {
					pt_debug(dev, DL_INFO, "%s: disabling vcc_vdd\n", __func__);
		...
		//platform power
		if (vcc_vdd) {
			pt_debug(dev, DL_INFO,  "%s put vcc_vdd\n", __func__);
			regulator_put(vcc_vdd);
		}
		if (vcc_ana) {
			pt_debug(dev, DL_INFO, "%s put vcc_ana\n", __func__);
		...
		struct device *dev)
{
	...
	if (on == PT_MT_POWER_ON) {
		...
		/*
		* For TDDI parts, force part out of RESET by holding DDI XRES
		...
		* - For Gen6 the RST line must be disabled (set high) when VDDI is being
		* applied for a min of 2ms, after which it must be enabled (set low)
		* for 10ms.
		* - For TT/TC parts it must be low while power is applied but the pwr
		* sequence for Gen6 is ok.
		*/
		if (pdata->rst_gpio) {
			/*
			* Set initial RST GPIO direction to input to allow it to
			* float up with VDDI.
			*/
			rc = gpio_direction_input(pdata->rst_gpio);
			if (rc < 0) {
				pt_debug(dev, DL_ERROR,
					"%s: Input Setup ERROR: RST GPIO %d\n",
					__func__, pdata->rst_gpio);
				/*
				* Not ideal but if setting the direction to input failed
				* ensure part is not in reset during first 2ms.
				*/
		 ...
		 * - TC device power up order: VCC, VDDI, AVDD, AVEE
		 * - TT device power up order: VCC, VDDI
		 * NOTE: VDDI must be stable for >10ms before XRES is released
		 */
		pt_debug(dev, DL_INFO,
		"%s: Enable defined pwr: VDD, VCC, AVDD, AVEE\n", __func__);

		/* Turn on VCC (3.3v) */
		if (vcc_ana) {
			if (!regulator_is_enabled(vcc_ana)) {
				pt_debug(dev, DL_INFO, "%s: enabling vcc_ana\n", __func__);
				rc = regulator_enable(vcc_ana);
			}
		}
		/* Turn on VDD [Digital Interface] (+1.8v) */
		if (vcc_vdd) {
			if (!regulator_is_enabled(vcc_vdd)) {
				pt_debug(dev, DL_INFO, "%s: enabling vcc_vdd\n", __func__);
				rc = regulator_enable(vcc_vdd);
			}
		}

		/*
		 * Ensure the power is stable.
		 * This must be a min of 2ms for Gen6 but as short as
		 * possible for TT/TC parts.
		 */
		usleep_range(2000, 3000);

		/*
		 * 2ms after enabling VDDI and VCC, change the direction of the
		 * RST GPIO to be an output so it will no longer float and then
		 * force the DUT into reset
		 */
		...
		/* Ensure it stays in reset for atleast 10ms before releasing it */
		...
		*/
		 ...
		 * TC device The power down order is: VDDI, AVEE, AVDD
		 * TT device The power down order is: VDDI, VCC
		 *
		...
		/* Turn off VDD [Digital Interface] (+1.8v) */
		if (vcc_vdd) {
			if (regulator_is_enabled(vcc_vdd)) {
				pt_debug(dev, DL_INFO, "%s: disabling vcc_vdd\n", __func__);
				regulator_disable(vcc_vdd);
