...
static struct pt_core_data * p_core_data = NULL;
...
static ssize_t pt_touch_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t pt_touch_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *pt_touch_proc_entry = NULL;
static const struct file_operations pt_touch_fops = {
	.owner = THIS_MODULE,
	.read = pt_touch_read_proc,
	.write = pt_touch_write_proc,
};

static ssize_t pt_misc_write_not_implemented(struct file *file, const char __user *buf,
			size_t count, loff_t *pos);
static ssize_t pt_misc_read_not_implemented(struct file *file, char __user *buf,
			size_t count, loff_t *pos);
static long pt_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg);

static const struct file_operations pt_misc_fops =
{
	.owner = THIS_MODULE,
	.read  = pt_misc_read_not_implemented,
	.write = pt_misc_write_not_implemented,
	.unlocked_ioctl = pt_core_ioctl,
};

static struct miscdevice pt_misc_device =
{
	.minor = 0,
		...
		u8 power_state)
{
	...
	struct pt_hid_cmd hid_cmd = {
		...
		.wait_interrupt = 1,
		.timeout_ms = HID_SET_POWER_TIMEOUT,
	};
...
#define PIP2_RESP_DEFAULT_TIME_MAX     (HID_OUTPUT_USER_TIMEOUT * 1000)
#define PIP2_RESP_FILE_WRITE_TIME_MIN  220
#define PIP2_RESP_FILE_IOCTL_TIME_MAX  (PIP2_CMD_FILE_ERASE_TIMEOUT * 1000)
#else
...
static const struct pip2_cmd_response_structure pip2_cmd_response[] = {
	...
	{.id = PIP2_CMD_ID_CTRL,
	 ...
	 .response_time_min = PIP2_RESP_DEFAULT_TIME_MIN,
	 .response_time_max = PIP2_CMD_FILE_ERASE_TIMEOUT},
...
static int _pt_request_pip2_send_cmd(struct device *dev,
	...
	u16 report_body_len, u8 *read_buf, u16 *actual_read_len)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	int i = 0;
...
static int pt_pip_start_bootloader_(struct pt_core_data *cd)
{
	...
	struct pt_hid_output hid_output = {
		...
		.timeout_ms = PIP1_START_BOOTLOADER_TIMEOUT,
		.reset_expected = 1,
	};
...
static void pt_pip2_ver_load_ttdata(struct pt_core_data *cd, u16 len)
{
	...
	if (len >= 0x1D) {
		...
		ttdata->chip_rev = get_unaligned_le16(&full_ver->chip_rev);
		ttdata->chip_id  = get_unaligned_le16(&full_ver->chip_id);
		ttdata->wafer_number  = full_ver->wafer_number;
		ttdata->x_coord  = get_unaligned_le32(&full_ver->x_coord);
		ttdata->y_coord  = get_unaligned_le16(&full_ver->y_coord);
		memcpy(ttdata->wafer_lot, full_ver->wafer_lot,
			PT_WAFER_LOT_SIZE);
...
static int pt_hid_output_get_sysinfo_(struct pt_core_data *cd)
{
	...
	struct pt_hid_output hid_output = {
		...
		.timeout_ms = PIP1_CMD_GET_SYSINFO_TIMEOUT,
	};
		...
		u8 *security_key, u16 *actual_write_len)
{
	...
	struct pt_hid_output hid_output = {
		...
		.timeout_ms = PIP1_CMD_WRITE_CONF_BLOCK_TIMEOUT,
	};
	...
	unsigned short *crc)
{
	...
	pt_debug(dev, DL_INFO, "%s: CAL Cache action=%d\n", __func__, action);
	switch (action) {
		...
		u8 *results_available)
{
	...
	struct pt_hid_output hid_output = {
		...
		.write_length = 2,
		.write_buf = write_buf,
		.timeout_ms = PIP1_CMD_RUN_SELF_TEST_TIMEOUT,
	};
		...
		struct pt_cal_ext_data *cal_data, u8 *status)
{
	...
	struct pt_hid_output hid_output = {
		...
		.write_length = write_length,
		.write_buf = write_buf,
		.timeout_ms = PIP1_CMD_CALIBRATE_EXT_TIMEOUT,
	};
		...
		u8 mode, u8 *status)
{
	...
	struct pt_hid_output hid_output = {
		...
		.write_length = write_length,
		.write_buf = write_buf,
		.timeout_ms = PIP1_CMD_CALIBRATE_IDAC_TIMEOUT,
	};
		...
		u16 key_size, u8 *key_buf, u16 row_size, u8 *metadata_row_buf)
{
	...
	struct pt_hid_output hid_output = {
		...
		.write_length = write_length,
		.timeout_ms = PIP1_CMD_INITIATE_BL_TIMEOUT,
	};
		...
		struct pt_core_data *cd, u16 data_len, u8 *data_buf)
{
	struct pt_hid_output hid_output = {
		...
		.write_buf = data_buf,
		.timeout_ms = PIP1_CMD_PROGRAM_AND_VERIFY_TIMEOUT,
	};
	...
	u8 *fw_sys_mode)
{
	...
	if (!rc) {
		...
		if (status == PIP2_RSP_ERR_NONE &&
			...
			*fw_sys_mode = read_buf[PIP2_RESP_BODY_OFFSET + 1];
	} else {
		pt_debug(dev, DL_WARN,
			"%s: Unable to retrieve FW Sys Mode rc=%d\n",
			__func__, rc);
	}
	return rc;
}
	...
	if (sys_mode == FW_SYS_MODE_BOOT) {
		while (!rc && loop <= max_loop &&
			...
			rc = _pt_request_pip2_get_fw_sys_mode(cd->dev,
				PT_CORE_CMD_UNPROTECTED, &sys_mode);
			pt_debug(cd->dev, DL_DEBUG,
 ...
 * PARAMETERS:
 *	*cd       - pointer to core data
 *	*sys_mode - pointer to fw mode
 ******************************************************************************/
static int pt_get_fw_sys_mode(struct pt_core_data *cd, u8 *sys_mode)
{
	int rc;
	...
	rc = _pt_get_fw_sys_mode(cd, sys_mode);
		...
		u8 *sys_mode)
{
	...
	if (protect)
		return pt_get_fw_sys_mode(cd, sys_mode);

	return _pt_get_fw_sys_mode(cd, sys_mode);
}
		...
		struct pt_hid_desc *desc)
{
	...
	t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0),
		msecs_to_jiffies(HID_GET_HID_DESCRIPTOR_TIMEOUT));
 ...
 *	*dev  - pointer to device structure
 ******************************************************************************/
static int pt_pip2_get_version(struct device *dev)
{
	int rc = 0;
	int status;
	u8 read_buf[64];
...
static int pt_dut_reset_and_wait(struct pt_core_data *cd)
{
	...
	t = wait_event_timeout(cd->wait_q,
		(cd->hid_reset_cmd_state == 0),
		msecs_to_jiffies(HID_RESET_TIMEOUT));
	if (IS_TMO(t)) {
#ifdef TTDL_DIAGNOSTICS
		cd->bus_transmit_error_count++;
		pt_toggle_err_gpio(cd, PT_ERR_GPIO_I2C_TRANS);
...
static int pt_core_sleep_(struct pt_core_data *cd)
{
	...
	if (!rc)
		call_atten_cb(cd, PT_ATTEN_SLEEP, 0);
	return rc;
}
...
static int pt_core_sleep(struct pt_core_data *cd)
{
	...
	pinctrl_pm_select_sleep_state(cd->dev);
	...
	if (cd->pip2_prot_active) {
		...
		if (cd->pip2_cmd_tag_seq != (cd->input_buf[2] & 0x0F)) {
			pt_debug(cd->dev, DL_ERROR,
				...
				(cd->input_buf[2] & 0x0F));
#ifdef TTDL_DIAGNOSTICS
			pt_pr_buf(cd->dev, DL_INFO, cd->input_buf, size,
				"PIP RSP:");
#endif /* TTDL_DIAGNOSTICS */
		}
...
static int pt_pip2_launch_app(struct device *dev, int protect)
{
	...
	if (rc) {
		/*
		 * If sending the EXECUTE command failed the only other option
		 * is to reset the part and see if the FW launches.
		 */
		pt_debug(dev, DL_ERROR,
	...
	int state)
{
	...
	t = wait_event_timeout(cd->wait_q,
		((cd->startup_status >= state) ||
		 (((cd->startup_status & 0x0100) >> 8) == 1)),
	...
	if (IS_TMO(t)) {
		...
		rc = -ETIME;
	} else if (t == 1 && cd->startup_status == state) {
		pt_debug(cd->dev, DL_WARN,
			"%s: Enum state reached: enum=0x%04X in %dms\n",
			__func__, cd->startup_status, timeout);

	} else if (t > 1) {
		pt_debug(cd->dev, DL_WARN,
			"%s: Enum state reached: enum=0x%04X in %dms\n",
			__func__, cd->startup_status,
			timeout - jiffies_to_msecs(t));
	}
	return rc;
}
			...
			u8 *mode, u8 *sys_mode)
{
	...
	rc = _pt_request_pip2_send_cmd(cd->dev, PT_CORE_CMD_UNPROTECTED,
		&pip2_cmd, PIP2_CMD_ID_STATUS, NULL, 0, read_buf,
		&actual_read_len);
 ...
 ******************************************************************************/
int _pt_pip2_exit_bl(struct pt_core_data *cd, u8 *status_str, int buf_size)
{
	int rc;
	int wait_time = 0;
	...
	if (mode == PT_MODE_BOOTLOADER) {
		if (cd->flashless_dut == 1) {
			pt_debug(cd->dev, DL_INFO,
				"%s: Skip launch app, attempt hw_reset\n",
				__func__);
			goto skip_launch_app;
		}

		rc = pt_pip2_launch_app(cd->dev, PT_CORE_CMD_UNPROTECTED);
		if (rc) {
			pt_debug(cd->dev, DL_ERROR,
...
skip_launch_app:
		...
		if (!rc) {
			 ...
			 * Reaching BOOT mode guarantees both. If, however, the
			 * enumeration does not reach this far the DUT may
			 * still be in APP mode so test for all conditions.
			 */
		...
		rc = _pt_wait_for_fw_exit_boot_mode(cd, 1500, &wait_time);
		if (load_status_str) {
			if (!rc)
				strcpy(status_str,
					"Already in APP mode");
...
int pt_pip2_exit_bl(struct pt_core_data *cd, u8 *status_str, int buf_size)
{
	...
	rc = _pt_pip2_exit_bl(cd, status_str, buf_size);
...
reset:
	...
	if (cd->active_dut_generation == DUT_PIP1_ONLY) {
		/* Delay added before issuing first command to PIP1 devices */
		msleep(30);
		...
		if (retry == PT_CORE_STARTUP_RETRY_COUNT) {
			...
			rc = _pt_request_wait_for_enum_state(
				cd->dev, 150, STARTUP_STATUS_BL_RESET_SENTINEL);
			if (rc)
				pt_debug(cd->dev, DL_ERROR,
	...
	rc = _pt_wait_for_fw_exit_boot_mode(cd, 500, &wait_time);
	if (!rc) {
		cd->startup_status |= STARTUP_STATUS_FW_OUT_OF_BOOT;
		pt_debug(cd->dev, DL_WARN,
...
static int pt_core_wake(struct pt_core_data *cd)
{
	...
	pinctrl_pm_select_default_state(cd->dev);
...
exit:
	...
	if (retry--) {
		_pt_get_fw_sys_mode(cd, &sys_mode);
...
reset:
	...
	if (cd->active_dut_generation == DUT_UNKNOWN) {
		if (cd->bl_pip_ver_ready ||
			...
			pt_debug(cd->dev, DL_INFO, "%s: dut generation is %d\n",
	...
	if (cd->active_dut_generation == DUT_PIP1_ONLY) {
		...
		*enum_status |= STARTUP_STATUS_FW_OUT_OF_BOOT;
		...
		case PT_MODE_BOOTLOADER:
			 ...
			 * _pt_pip2_exit_bl() because exit_bl will cause several
...
int _pt_pip2_file_erase(struct device *dev, u8 file_handle, int *status)
{
	...
	*status = PIP2_RSP_ERR_TIMEOUT;
	...
	mutex_lock(&cd->system_lock);
	cd->pip_cmd_timeout = PIP2_CMD_FILE_ERASE_TIMEOUT;
	mutex_unlock(&cd->system_lock);
	...
	mutex_lock(&cd->system_lock);
	cd->pip_cmd_timeout = HID_OUTPUT_USER_TIMEOUT;
	mutex_unlock(&cd->system_lock);
	if (ret) {
		pt_debug(dev, DL_ERROR,
		...
		unsigned long event, void *data)
{
	...
	//It's locked, ignore fb notifier call back
	if (cd->exlock)
		goto exit;
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (length >= 1) {
		...
		rc = _pt_request_pip2_send_cmd(dev, PT_CORE_CMD_PROTECTED,
			&pip2_cmd, pip2_cmd_id, pip2_cmd_data, data_len,
			read_buf, &actual_read_len);
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf,
		...
		PT_DRV_DBG_CAL_CACHE_IN_HOST, "[0|1]", "Calibration cache in host",
		PT_DRV_DBG_SET_HW_DETECT, "[0|1]", "HW_detect",
		PT_DRV_DBG_VIRTUAL_I2C_DUT, "[0|1]", "Virtual DUT mode"
#endif /* TTDL_DIAGNOSTICS */
	);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case PT_DRV_DBG_CAL_CACHE_IN_HOST:		/* 216 */
		mutex_lock(&cd->system_lock);
		if (input_data[1] == 0) {
			cd->cal_cache_in_host = PT_CAL_CACHE_DISABLE;
			pt_debug(dev, DL_INFO,
				"%s: Disable Calibration cache in host\n",
				__func__);
		} else if (input_data[1] == 1) {
			cd->cal_cache_in_host = PT_CAL_CACHE_ENABLE;
			pt_debug(dev, DL_INFO,
		...
		mutex_unlock(&(cd->system_lock));
		break;
	case PT_DRV_DBG_SET_HW_DETECT:			/* 217 */
		mutex_lock(&cd->system_lock);
		if (input_data[1])
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (cd->active_dut_generation == DUT_PIP1_ONLY) {
		/*
		 * The DUT should report the same panel ID from both the BL
		 * and the FW unless the PT_PANEL_ID_FROM_XY_PIN is set, in
		 * which case the BL is not able to retrieve the panel_id.
		 */
		if (cd->mode == PT_MODE_BOOTLOADER) {
#if PT_PANEL_ID_FROM_XY_PIN
			/* BL can't retrieve PID on an XY pin system */
			pid = cd->pid_for_loader;
#else
			rc = pt_hid_output_bl_get_panel_id_(cd, &pid);
			if (rc) {
			...
			rc = pt_hid_output_get_sysinfo(cd);
			if (!rc)
				pid = cd->sysinfo.sensing_conf_data.panel_id;
			pt_debug(dev, DL_WARN,
			...
			if (!rc) {
				if (status == 0) {
					...
					pt_debug(dev, DL_ERROR,
						 "%s: %s=%d\n",
						__func__,
						 "BL read gpio failed status",
						 status);
				}
			} else {
				pt_debug(dev, DL_ERROR,
					 "%s: BL read gpio failed rc=%d\n",
					 __func__, rc);
			}
		} else if (cd->mode == PT_MODE_OPERATIONAL) {
			rc = pt_hid_output_get_sysinfo(cd);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!rc) {
		return snprintf(buf, PT_MAX_PRBUF_SIZE,
			...
			ttdata->wafer_lot[4],  ttdata->wafer_lot[3],
			ttdata->wafer_lot[2],  ttdata->wafer_lot[1],
			ttdata->wafer_lot[0],
			ttdata->wafer_number,
			ttdata->x_coord,
			ttdata->y_coord);
		...
		struct device_attribute *attr, char *buf)
{
	...
	rc = _pt_pip2_exit_bl(cd, status_str, status_str_buf_len);
	ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "Status: %d\n%s\n",
		rc, status_str);
	return ret;
}
	...
	u8 *bus_toggled, u8 *irq_toggled, u8 *xres_toggled, u8 *err_str)
{
	...
	if (pt_check_irq_asserted(cd) && cd->route_bus_virt_dut) {
		...
		pt_adap_write_read_specific(cd, 3, release_irq, NULL);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* Retrieve FW system mode which can only be 0-4 */
	rc = pt_get_fw_sys_mode(cd, &sys_mode);
	if (sys_mode > FW_SYS_MODE_MAX)
		sys_mode = FW_SYS_MODE_UNDEFINED;

	if (rc || sys_mode != FW_SYS_MODE_TEST)
...
static int add_sysfs_interfaces(struct device *dev)
{
	...
	pt_touch_proc_entry = proc_create("touch", 0660, NULL, &pt_touch_fops);
	if (pt_touch_proc_entry == NULL) {
		pt_debug(dev, DL_ERROR, "Create proc entry /proc/touch FAILED!\n");
		goto undo;
	}
...
/**** TOUCH PROC ENTRY ****/
static ssize_t pt_touch_read_proc(struct file *file, char __user *buf,
			size_t count, loff_t *off)
{
	...
	if(p_core_data == NULL) {
		printk(KERN_ERR "pt_proc_read:proc:not ready\n");
		return -EFAULT;
	}
	...
	if (p_core_data->sleep_state != SS_SLEEP_OFF)
		len = sprintf(out,"Touch is locked\n");
	else
		len = sprintf(out,"Touch is unlocked\n");
	
	*off = len;

	if (copy_to_user(buf, out, len)) {
		printk(KERN_ERR "pt_touch_read_proc:proc::cannot copy to user!\n");
		len = -EFAULT;
	}
...
static ssize_t pt_touch_write_proc(struct file *filp, const char __user *buf,
			size_t len, loff_t *off)
{
	...
	if (p_core_data == NULL) {
		printk(KERN_ERR "pt_touch_write_proc:proc:not ready\n");
		return -EFAULT;
	}
	if (len >= 16){
		printk(KERN_ERR "pt_touch_write_proc:proc:command is too long!\n");
		return -ENOSPC;
	}

	if (copy_from_user(command, buf, len)) {
		printk(KERN_ERR "pt_touch_write_proc:proc::cannot copy from user!\n");
		return -EFAULT;
	}

	if ( !strncmp(command, "unlock", 6) ) {
		printk(KERN_INFO "pt: pt_touch_write_proc::command=unlock:\n");
		sts = pt_core_rt_resume(p_core_data->dev);
		if (sts < 0) {
				printk(KERN_ERR "pt_touch_write_proc:proc:command=%s:not succeed please retry\n", command);
				return -EBUSY;
		}
		p_core_data->exlock = false;
	} else if ( !strncmp(command, "lock", 4) ) {
		printk(KERN_INFO "pt: pt_touch_write_proc::command=lock:\n");
		sts = pt_core_rt_suspend(p_core_data->dev);
		if (sts < 0){
				printk(KERN_ERR "pt_touch_write_proc:proc:command=%s:not succeed please retry\n", command);
				return -EBUSY;
		}
		p_core_data->exlock = true;
	} else {
	        printk(KERN_ERR "pt_touch_write_proc:proc:command=%s:Unrecognized command\n", command);
	}
...
static long pt_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case TOUCH_IOCTL_SET_LOCK:
		{
			...
			if (p_core_data->exlock == !!locked)
				break;

			if (locked) {
				int sts;
				printk(KERN_INFO "pt: pt_touch_write_ioctl::command=lock:\n");
				sts = pt_core_sleep(p_core_data);
				if (sts < 0){
					printk(KERN_ERR "pt_proc_write:ioctl:command=lock:not succeed please retry\n");
					return -EBUSY;
				}
				p_core_data->exlock = true;
			} else {
				int sts;
				printk(KERN_INFO "pt: pt_touch_write_ioctl::command=unlock:\n");
				sts = pt_core_wake(p_core_data);
				if (sts < 0) {
					printk(KERN_ERR "pt_proc_write:ioctl:command=unlock:not succeed please retry\n");
					return -EBUSY;
				}
				p_core_data->exlock = false;
			}
...
static int pt_probe_complete(struct pt_core_data *cd)
{
	...
	if (cd->cpdata->setup_power) {
		pt_debug(cd->dev, DL_INFO, "%s: Device power on Regulators!\n", __func__);
		rc = cd->cpdata->setup_power(cd->cpdata,
			PT_MT_POWER_ON, cd->dev);
	} else {
		pt_debug(cd->dev, DL_WARN, "%s: No setup power regulators function\n",
			__func__);
		rc = 0;
	}
	if (rc < 0)
...
retry_hw_detect:
	...
	/* Retrieve the HW Version of the connected DUT and store in cd */
	_pt_request_hw_version(dev, cd->hw_version);
	pt_debug(dev, DL_WARN, "%s: HW Version: %s\n", __func__,
		cd->hw_version);
	...
	if (cd->hw_detected) {
		...
		pt_debug(dev, DL_INFO, "%s: register misc device\n",
			__func__);
		rc = misc_register(&pt_misc_device);
		if (rc < 0) {
			dev_err(dev, "%s: Error, register misc device\n", __func__);
			goto error_misc_create;
		}
	}
#endif /* !TTDL_KERNEL_SUBMISSION */
...
error_after_sysfs_create:
	...
	pt_free_si_ptrs(cd);
#ifndef TTDL_KERNEL_SUBMISSION
        misc_deregister(&pt_misc_device);
error_misc_create:
	remove_sysfs_interfaces(dev);
#endif /* !TTDL_KERNEL_SUBMISSION */
error_after_startup:
