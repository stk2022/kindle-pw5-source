...
typedef int (*PIP2_SEND_CMD)(struct device *, int,
		struct pip2_cmd_structure *, u8, u8 *, u16, u8 *, u16 *);
...
static void pt_calibrate_idacs(struct work_struct *calibration_work)
{
	...
	if (dut_gen == DUT_PIP1_ONLY) {
		...
		/*
		 * There will be an impovement for fw to calibrate
		 * automatically, then manual calibration is not need after
		 * fw 3.4.932110.
		 */
		if (cd->sysinfo.ready && cd->sysinfo.ttdata.revctrl < 932110) {
	...
	const struct firmware *fw)
{
	...
	u16 img_config_ver = fw->data[17]<<8 | fw->data[18];
	...
	if ((256 * img_app_major_ver + img_app_minor_ver) <
		...
		if (img_config_ver > hdr.config_ver){
			should_upgrade = 1;
		} else {
			pt_debug(dev, DL_WARN,
		...
		void *context)
{
	...
	if (ld->pip2_load_file_no == PIP2_FW_FILE) {
		ret = cmd->nonhid_cmd->pip2_file_get_state(dev,
	...
	if (cd->cal_cache_in_host == PT_CAL_CACHE_ENABLE)
...
static int pt_pip2_upgrade_firmware_from_builtin(struct device *dev)
{
	...
	int pip2_file_no[PIP2_MAX_FILE_NAMES];

	/*
	 * Load the supported filenames in the correct search order
	 * 0 - "tt_fw_ram.bin"
	 * 1 - "tt_fw.bin"
	 * 2 - "XXXX_tt_fw<_PIDX>.bin" where XXXX = Silicon ID
	 */
	filename[0] = kzalloc(sizeof(PT_FW_RAM_FILE_NAME), GFP_KERNEL);
	filename[1] = kzalloc(sizeof(PT_FW_FILE_NAME), GFP_KERNEL);
	filename[2] = generate_silicon_id_firmware_filename(dev);
	if (!filename[0] || !filename[1] || !filename[2])
		return -ENOMEM;

	memcpy(filename[0], PT_FW_RAM_FILE_NAME, sizeof(PT_FW_RAM_FILE_NAME));
	memcpy(filename[1], PT_FW_FILE_NAME, sizeof(PT_FW_FILE_NAME));

	/* Populate the PIP2 file number where each file should be BL to */
	pip2_file_no[0] = PIP2_RAM_FILE;
	if (cd->flashless_dut) {
		pip2_file_no[1] = PIP2_RAM_FILE;
		pip2_file_no[2] = PIP2_RAM_FILE;
	} else {
		pip2_file_no[1] = PIP2_FW_FILE;
		pip2_file_no[2] = PIP2_FW_FILE;
	}
	...
	mutex_lock(&cd->firmware_class_lock);
	index = 0;
	while (index < PIP2_MAX_FILE_NAMES) {
		pt_debug(dev, DL_INFO, "%s: Request FW class file: %s\n",
			__func__, filename[index]);
		ld->pip2_load_file_no = pip2_file_no[index];
#if (KERNEL_VERSION(3, 13, 0) > LINUX_VERSION_CODE)
		retval = request_firmware(&fw_entry, filename[index], dev);
#else
		retval = request_firmware_direct(&fw_entry,
		...
		char *buf, loff_t offset, size_t count)
{
	...
	if (offset == 0) {
		...
		rc = cmd->nonhid_cmd->pip2_file_get_state(dev, file_handle,
				&address, &file_size);
		if (rc) {
			pt_debug(dev, DL_ERROR,
