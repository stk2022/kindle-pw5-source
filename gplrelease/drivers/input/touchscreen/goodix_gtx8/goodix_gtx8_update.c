...
struct fw_name_map fw_name_map[] = {
	{0, "goodix_firmware_6861.bin"},
	{1, "goodix_firmware_6861P.bin"},
	{2, NULL},
	{3, NULL},
	{4, "goodix_firmware_6861.bin"},
};
...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	if (!memcmp(fw_info->fw_pid, "6861P", 5)) {
		/* change 6861P to 6862 */
		fw_info->fw_pid[3] = '2';
		fw_info->fw_pid[4] = 0;
	}
...
static int goodix_get_fw_name(struct fw_update_ctrl *fwu_ctrl)
{
	struct goodix_ts_core *core_data = fwu_ctrl->core_data;
	struct goodix_ts_version fw_ver = {0};
	struct goodix_ts_device *dev;
	u8 sensor_id = 0xFF, temp = 0xFF;
	int ret = -EINVAL, i;

	ts_info("get_fw_name_check1");
	if (!core_data || !core_data->ts_dev) {
		ts_err("core_data && ts_dev cann't be null");
		return -ENODEV;
	}

	dev = core_data->ts_dev;
	ret = dev->hw_ops->read_version(dev, &fw_ver);
	if (ret) {
		ts_err("failed get IC version, may coused by doze faile, %d", ret);
		ret = goodix_reg_read(dev, dev->reg.sensor_id, &sensor_id, 1);
		ret |= goodix_reg_read(dev, dev->reg.sensor_id, &temp, 1);
		if (ret || sensor_id != temp || sensor_id > 0x8) {
			ts_err("failed get sensor ID, %d, %d %d",
				ret, sensor_id, temp);
			return ret;
		}
	} else {
		sensor_id = fw_ver.sensor_id;
	}
	ts_info("get sensor ID %d", sensor_id);
	for (i = 0; i < ARRAY_SIZE(fw_name_map); i++) {
		if (fw_name_map[i].sensor_id == sensor_id &&
			fw_name_map[i].fw_name != NULL) {
			ts_info("get fw name: %s for sensor id %d",
				fw_name_map[i].fw_name, sensor_id);
			strlcpy(fwu_ctrl->fw_name,
				fw_name_map[i].fw_name,
				sizeof(fwu_ctrl->fw_name));
			return 0;
		}
	}
	ts_err("failed get fw name for sensor id %d", sensor_id);
	return -EINVAL;
}
...
static int goodix_fw_update_thread(void *data)
{
	...
	if (fwu_ctrl->mode & UPDATE_MODE_SRC_HEAD) {
		...
		if (goodix_get_fw_name(fwu_ctrl)) {
			ts_err("failed get firmware name");
			goto out;
		}
