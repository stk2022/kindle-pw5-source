...
static ssize_t goodix_touch_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t goodix_touch_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *goodix_touch_proc_entry = NULL;
static const struct file_operations goodix_touch_fops = {
    .owner = THIS_MODULE,
    .read = goodix_touch_read_proc,
    .write = goodix_touch_write_proc,
};

static ssize_t goodix_misc_write_not_implemented(struct file *file, const char __user *buf,
            size_t count, loff_t *pos);
static ssize_t goodix_misc_read_not_implemented(struct file *file, char __user *buf,
            size_t count, loff_t *pos);
static long goodix_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg);

static const struct file_operations goodix_misc_fops =
{
    .owner = THIS_MODULE,
    .read  = goodix_misc_read_not_implemented,
    .write = goodix_misc_write_not_implemented,
    .unlocked_ioctl = goodix_core_ioctl,
};

static struct miscdevice goodix_misc_device =
{
    .minor = 0,
    .name  = TOUCH_MISC_DEV_NAME,
    .fops  = &goodix_misc_fops,
};
...
Output:
    None.
*********************************************************/
static void send_user_event(struct goodix_ts_core *core_data, u8 prev, u8 cur)
{
    struct device *dev = &core_data->input_dev->dev;
		...
		struct goodix_touch_data *touch_data)
{
	...
	send_user_event(core_data,pre_fin,touch_num);
	pre_fin = touch_num;
	...
	if (fb_event && fb_event->data && core_data) {
		if (event == FB_EARLY_EVENT_BLANK) {
			...
			if (*blank == FB_BLANK_UNBLANK) {
				if (core_data->fb_state != FB_ON) {
					goodix_ts_resume(core_data);
					core_data->fb_state = FB_ON;
				}
			} else if (*blank == FB_BLANK_POWERDOWN) {
				if (core_data->fb_state != FB_OFF) {
					goodix_ts_suspend(core_data);
					core_data->fb_state = FB_OFF;
				}
			}
		}
	}
...
/**** TOUCH PROC ENTRY ****/
static ssize_t goodix_touch_read_proc(struct file *file, char __user *buf,
            size_t count, loff_t *off)
{
    ...
    if (core_data == NULL) {
        ts_err("goodix_proc_read:proc:not ready\n");
        return -EFAULT;
    }
	...
	*off = len;
	...
	if (copy_to_user(buf, out, len)) {
		printk(KERN_ERR "goodix_touch_read_proc:proc::cannot copy to user!\n");
		len = -EFAULT;
	}
...
static ssize_t goodix_touch_write_proc(struct file *filp, const char __user *buf,
            size_t len, loff_t *off)
{
    ...
    if (core_data == NULL) {
        ts_err("goodix_touch_write_proc:proc:not ready\n");
        return -EFAULT;
    }
    if (len >= 16){
        ts_err("goodix_touch_write_proc:proc:command is too long!\n");
        return -ENOSPC;
    }

    if (copy_from_user(command, buf, len)) {
        ts_err("goodix_touch_write_proc:proc::cannot copy from user!\n");
        return -EFAULT;
    }

    if ( !strncmp(command, "unlock", 6) ) {
        printk(KERN_INFO "goodix: goodix_touch_write_proc::command=unlock:\n");
        goodix_ts_resume(core_data);
        core_data->exlock = false;
    } else if ( !strncmp(command, "lock", 4) ) {
        printk(KERN_INFO "goodix: goodix_touch_write_proc::command=lock:\n");
        goodix_ts_suspend(core_data);
        core_data->exlock = true;
    } else {
            ts_err("goodix_touch_write_proc:proc:command=%s:Unrecognized command\n", command);
    }
...
static long goodix_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case TOUCH_IOCTL_SET_LOCK:
        {
            ...
            if (core_data->exlock == !!locked)
                break;

            if (locked) {
                printk(KERN_INFO "goodix: goodix_touch_write_ioctl::command=lock:\n");
                goodix_ts_suspend(core_data);
                core_data->exlock = true;
            } else {
                printk(KERN_INFO "goodix: goodix_touch_write_ioctl::command=unlock:\n");
                goodix_ts_resume(core_data);
                core_data->exlock = false;
            }
            break;
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (ts_device->board_data.pen_enable) {
	...
	goodix_touch_proc_entry = proc_create(GOODIX_TOUCH_PROC_FILE, 0666, NULL, &goodix_touch_fops);
	if (goodix_touch_proc_entry == NULL)
	{
		ts_err("create_proc_entry %s failed\n", GOODIX_TOUCH_PROC_FILE);
		r = -ENODEV;
		goto proc_err;
	}
	else
	{
		ts_info("create_proc_entry %s success", GOODIX_TOUCH_PROC_FILE);
	}

	// Register misc dev file system
	r = misc_register(&goodix_misc_device);
	if (r < 0) {
		ts_err("Error, register misc device\n");
		goto misc_create_err;
	}
...
misc_create_err:
	remove_proc_entry(GOODIX_TOUCH_PROC_FILE, NULL);
	goodix_touch_proc_entry = NULL;
proc_err:
...
static int goodix_ts_remove(struct platform_device *pdev)
{
	...
	if (goodix_touch_proc_entry) {
		remove_proc_entry(GOODIX_TOUCH_PROC_FILE, NULL);
		goodix_touch_proc_entry = NULL;
	}
	ts_info("GOODIX driver removing...");
	misc_deregister(&goodix_misc_device);
	goodix_debugfs_exit();
