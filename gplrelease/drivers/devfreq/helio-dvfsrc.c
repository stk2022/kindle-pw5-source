...
static void spm_dvfsfw_init(int spm_idx)
{
	unsigned long flags;

	spin_lock_irqsave(&spm_lock, flags);
	mt_secure_call(MTK_SIP_KERNEL_SPM_VCOREFS_ARGS,
			VCOREFS_SMC_CMD_0, 0, spm_idx, 0);
...
u32 spm_get_dvfs_final_level(void)
{
	...
	//pr_info("Final level: event_sta: %d, rsv9: %d\n", event_sta, rsv9);
	if (event_sta != 0)
...
void mtk_spmfw_init(int spm_idx, int skip_check)
{
	if (is_spm_enabled() != 0x0 && !skip_check)
		return;

	spm_dvfsfw_init(spm_idx);
	spm_go_to_vcorefs(dvfsrc->pwr_index);
}

int __weak dram_steps_freq(int dram_type, int step)
{
...
static int commit_data(int type, int data, int check_spmfw)
{
	...
	int vcore_uv = 0;
	#endif
	int spm_idx = spm_get_spmfw_idx();

	if (!is_dvfsrc_enabled())
		return ret;

	if ((spm_idx >= SPMFW_PSRAM_2133) &&
	   (type == PM_QOS_DDR_OPP))
		return ret;

	if (check_spmfw)
		mtk_spmfw_init(spm_idx, 0);
	...
	case PM_QOS_VCORE_OPP:
		...
		if (spm_idx >= SPMFW_PSRAM_2133) {
			if (data >= VCORE_OPP_1)
				data = VCORE_OPP_1;
		} else if (spm_idx != SPMFW_LP4X_2400) {
			if (data >= VCORE_OPP_2)
				data = VCORE_OPP_2;
		}
...
void helio_dvfsrc_enable(int dvfsrc_en)
{
	...
	mtk_spmfw_init(spm_get_spmfw_idx(), 1);
}
...
void dvfsrc_opp_table_init(void)
{
	...
	for (i = 0; i < VCORE_DVFS_OPP_NUM; i++) {
		...
		set_opp_table(i, get_vcore_uv_table(vcore_opp),
				dram_steps_freq(ddr_type, ddr_opp) * 1000);
	}
}
...
static int helio_dvfsrc_probe(struct platform_device *pdev)
{
	...
	dvfsrc->spm_map =
		syscon_regmap_lookup_by_compatible("mediatek,mt8512-scpsys");
	...
	ret = helio_dvfsrc_platform_init(dvfsrc);
	if (ret) {
		dev_err(dvfsrc->dev, "failed to init platform: %d\n",
			ret);
		return ret;
	}
...
static __maybe_unused int helio_dvfsrc_resume(struct device *dev)
{
	...
	if (in_falcon()) {
		...
		dvfsrc_write(DVFSRC_SW_REQ, 0x0);
		dvfsrc_restore();
	}
#endif
...
late_initcall(helio_dvfsrc_init)
