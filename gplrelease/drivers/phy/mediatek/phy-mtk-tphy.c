...
#define U3D_PHYD_MIX5		0x50
#define RXPLL_STBCYC_MSK		GENMASK(10, 0)
#define RXPLL_STBCYC_VAL		0x50
...
#define U3P_U3_PHYA_REG3	0x00c
#define P3A_RG_SYSPLL_FBDIV_MSK		GENMASK(31, 1)
#define P3A_RG_SYSPLL_FBDIV_VAL(x)	((x) << 1)
#define P3A_RG_SYS_PLL_HR_EN		BIT(0)
				 ...
				 struct mtk_phy_instance *instance)
{
	...
	/* 2712 E2, as PCIe EP by default, here back to USB3 */
	tmp = readl(u3_banks->phya + U3P_U3_PHYA_REG3);
	tmp &= ~P3A_RG_SYSPLL_FBDIV_MSK;
	tmp |= P3A_RG_SYSPLL_FBDIV_VAL(0x60276276);
	writel(tmp, u3_banks->phya + U3P_U3_PHYA_REG3);
				      ...
				      struct mtk_phy_instance *instance)
{
	...
	/*0x68[6] = 1 0x68[7] = 1 0x68[5:4] = 2b'01 0x68[2] = 1 0x68[13:10] = 4b'0000*/
	tmp = readl(com + U3P_U2PHYDTM0);
	tmp |= P2C_RG_DPPULLDOWN | P2C_RG_DMPULLDOWN | P2C_RG_TERMSEL;
	tmp &= ~(P2C_RG_XCVRSEL | P2C_RG_DATAIN);
	tmp |= P2C_RG_XCVRSEL_VAL(1);
	writel(tmp, com + U3P_U2PHYDTM0);

	/*0x68[20] = 1,0x68[21] = 1,0x68[19] = 1,0x68[17] = 1,0x68[23] = 1*/
	tmp = readl(com + U3P_U2PHYDTM0);
	tmp |= P2C_FORCE_TERMSEL | P2C_FORCE_DATAIN | P2C_FORCE_XCVRSEL
		| P2C_FORCE_DP_PULLDOWN | P2C_FORCE_DM_PULLDOWN;
	writel(tmp, com + U3P_U2PHYDTM0);
				       ...
				       struct mtk_phy_instance *instance)
{
	...
	/* 2712 E2, as PCIe EP by default, here back to RC */
	tmp = readl(bank->phya + U3P_U3_PHYA_REG3);
	tmp &= ~P3A_RG_SYSPLL_FBDIV_MSK;
	tmp |= P3A_RG_SYSPLL_FBDIV_VAL(0x60276276);
	writel(tmp, bank->phya + U3P_U3_PHYA_REG3);

	/* phyA need to be reset to make above change take effect for PCIe */
	...
	/*
	 * Change the CDR locking time to prevent unlocking,
	 * in case of system temperature changed from -40 C
	 * to 120 C, there maybe CDR locking issue by default.
	 */
	tmp = readl(bank->phyd + U3D_PHYD_MIX5);
	tmp &= ~RXPLL_STBCYC_MSK;
	tmp |= RXPLL_STBCYC_VAL;
	writel(tmp, bank->phyd + U3D_PHYD_MIX5);
}
...
static int mtk_phy_power_on(struct phy *phy)
{
	...
	ret = clk_prepare_enable(tphy->u3phya_ref);
	if (ret) {
		dev_info(tphy->dev, "failed to enable u3phya_ref\n");
		return ret;
	}
	ret = clk_prepare_enable(instance->ref_clk);
	if (ret) {
