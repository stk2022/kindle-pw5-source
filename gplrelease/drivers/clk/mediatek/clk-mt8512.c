...
static const int apll_fi2si1_parents[] = {
	CLK_TOP_AUD_1_SEL,
	CLK_TOP_AUD_2_SEL
};
...
#define GATE_MM(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &mm_cg_regs,			\
		.shift = _shift,			\
...
static const struct mtk_gate mm_clks[] = {
	GATE_MM(CLK_MM_PIPELINE0, CLK_TOP_MM_SEL, 0),
	GATE_MM(CLK_MM_PIPELINE1, CLK_TOP_MM_SEL, 1),
	GATE_MM(CLK_MM_PIPELINE2, CLK_TOP_MM_SEL, 2),
	GATE_MM(CLK_MM_PIPELINE3, CLK_TOP_MM_SEL, 3),
	GATE_MM(CLK_MM_PIPELINE4, CLK_TOP_MM_SEL, 4),
	GATE_MM(CLK_MM_PIPELINE5, CLK_TOP_MM_SEL, 5),
	GATE_MM(CLK_MM_PIPELINE7, CLK_TOP_MM_SEL, 7),
	GATE_MM(CLK_MM_DPI0_DPI_TMP0, CLK_TOP_DPI0_SEL, 19),
	GATE_MM(CLK_MM_DPI0_DPI_TMP1, CLK_TOP_DPI0_SEL, 20),
	GATE_MM(CLK_MM_DISP_FAKE, CLK_TOP_MM_SEL, 21),
	GATE_MM(CLK_MM_SMI_COMMON, CLK_TOP_MM_SEL, 22),
	GATE_MM(CLK_MM_SMI_LARB0, CLK_TOP_MM_SEL, 23),
	GATE_MM(CLK_MM_SMI_COMM0, CLK_TOP_MM_SEL, 24),
	GATE_MM(CLK_MM_SMI_COMM1, CLK_TOP_MM_SEL, 25),
};
...
#define GATE_IMG0(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &img0_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IMG1(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &img1_cg_regs,			\
		.shift = _shift,			\
...
static const struct mtk_gate img_clks[] = {
	/* IMG0 */
	GATE_IMG0(CLK_IMG_MDP_RDMA0, CLK_TOP_MM_SEL, 0),
	GATE_IMG0(CLK_IMG_MDP_RSZ0, CLK_TOP_MM_SEL, 2),
	GATE_IMG0(CLK_IMG_MDP_TDSHP0, CLK_TOP_MM_SEL, 4),
	GATE_IMG0(CLK_IMG_MDP_WROT0, CLK_TOP_MM_SEL, 5),
	GATE_IMG0(CLK_IMG_DISP_OVL0_21, CLK_TOP_MM_SEL, 8),
	GATE_IMG0(CLK_IMG_DISP_WDMA0, CLK_TOP_MM_SEL, 11),
	GATE_IMG0(CLK_IMG_DISP_GAMMA0, CLK_TOP_MM_SEL, 15),
	GATE_IMG0(CLK_IMG_DISP_DITHER0, CLK_TOP_MM_SEL, 16),
	GATE_IMG0(CLK_IMG_FAKE, CLK_TOP_MM_SEL, 21),
	GATE_IMG0(CLK_IMG_SMI_LARB1, CLK_TOP_MM_SEL, 23),
	/* IMG1 */
	GATE_IMG1(CLK_IMG_JPGDEC, CLK_TOP_MM_SEL, 1),
	GATE_IMG1(CLK_IMG_PNGDEC, CLK_TOP_MM_SEL, 2),
	GATE_IMG1(CLK_IMG_IMGRZ, CLK_TOP_MM_SEL, 3),
};
...
#define GATE_IP0(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip0_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP1(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip1_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP2(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip2_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP3(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip3_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP4(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip4_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP5(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip5_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP6(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip6_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP7(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip7_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP8(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip8_cg_regs,			\
		.shift = _shift,			\
...
#define GATE_IP9(_id, _parent, _shift) {	\
		.id = _id,				\
		.parent = _parent,			\
		.regs = &ip9_cg_regs,			\
		.shift = _shift,			\
...
static const struct mtk_gate ip_clks[] = {
	/* IP0 */
	GATE_IP0(CLK_IP_EMI_CK_GATE, CLK_TOP_CLK_NULL, 0),
	/* IP1 */
	GATE_IP1(CLK_IP_SRAM_OCC_GATE, CLK_TOP_SRAM_SEL, 0),
	/* IP2 */
	GATE_IP2(CLK_IP_HD_FAXI_CK, CLK_TOP_AXI_SEL, 0),
	/* IP3 */
	GATE_IP3(CLK_IP_NNA0_PWR_GATE, CLK_TOP_IP0_NNA_SEL, 0),
	/* IP4 */
	GATE_IP4(CLK_IP_NNA1_PWR_GATE, CLK_TOP_IP1_NNA_SEL, 0),
	/* IP5 */
	GATE_IP5(CLK_IP_WFST_PWR_GATE, CLK_TOP_IP2_WFST_SEL, 0),
	/* IP6 */
	GATE_IP6(CLK_IP_NNA0_OCC_GATE, CLK_TOP_IP0_NNA_SEL, 0),
	/* IP7 */
	GATE_IP7(CLK_IP_NNA1_OCC_GATE, CLK_TOP_IP1_NNA_SEL, 0),
	/* IP8 */
	GATE_IP8(CLK_IP_WFST_OCC_GATE, CLK_TOP_IP2_WFST_SEL, 0),
	/* IP9 */
	GATE_IP9(CLK_IP_TEST_26M, CLK_TOP_CLK26M, 0),
};
...
static int mt8512_mmsys_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt8512_clk_tree, mm_clks);
}

static int mt8512_imgsys_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt8512_clk_tree, img_clks);
}

static int mt8512_ipsys_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt8512_clk_tree, ip_clks);
}
...
static const struct udevice_id mt8512_mmsys_compat[] = {
	{ .compatible = "mediatek,mt8512-mmsys", },
	{ }
};

static const struct udevice_id mt8512_imgsys_compat[] = {
	{ .compatible = "mediatek,mt8512-imgsys", },
	{ }
};

static const struct udevice_id mt8512_ipsys_compat[] = {
...
U_BOOT_DRIVER(mtk_clk_mmsys) = {
	.name = "mt8512-mmsys",
	.id = UCLASS_CLK,
	.of_match = mt8512_mmsys_compat,
	.probe = mt8512_mmsys_probe,
	.priv_auto_alloc_size = sizeof(struct mtk_cg_priv),
	.ops = &mtk_clk_gate_ops,
	.flags = DM_FLAG_PRE_RELOC,
};

U_BOOT_DRIVER(mtk_clk_imgsys) = {
	.name = "mt8512-imgsys",
	.id = UCLASS_CLK,
	.of_match = mt8512_imgsys_compat,
	.probe = mt8512_imgsys_probe,
	.priv_auto_alloc_size = sizeof(struct mtk_cg_priv),
	.ops = &mtk_clk_gate_ops,
	.flags = DM_FLAG_PRE_RELOC,
};

U_BOOT_DRIVER(mtk_clk_ipsys) = {
	.name = "mt8512-ipsys",
	.id = UCLASS_CLK,
	.of_match = mt8512_ipsys_compat,
	.probe = mt8512_ipsys_probe,
	.priv_auto_alloc_size = sizeof(struct mtk_cg_priv),
	.ops = &mtk_clk_gate_ops,
	.flags = DM_FLAG_PRE_RELOC,
};
