...
static int rt9471_parse_dt(struct device *dev, struct rt9471_chip *chip)
{
...
#else
	ret = of_property_read_u32(np, "rt,intr_gpio_num", &chip->intr_gpio);
	if (ret < 0)
		return ret;
	if (strcmp(desc->chg_name, "secondary_chg") == 0) {
			...
			const struct i2c_device_id *id)
{
	...
	ret = rt9471_parse_dt(&client->dev, chip);
	if (ret < 0) {
