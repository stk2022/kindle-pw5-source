...
static int p922x_enable_charge_flow(struct p922x_dev *chip, bool en);
...
static int p922x_set_vout(struct p922x_dev *chip, int vout)
{
	...
	if ((vout >= SET_VOUT_MIN) && (vout <= SET_VOUT_MAX)) {
		dev_dbg(chip->dev, "%s: Set vout: %dmv\n", __func__, vout);
...
static int p922x_get_gp0_adc(struct p922x_dev *chip)
{
	u8 buf[2] = {0};
	int ret = 0;
	int adc = P922X_ADC_VOLTAGE_DEFAULT;

	ret = chip->bus.read_buf(chip, REG_ADC_GP0, buf, 2);
	if (ret) {
		dev_err(chip->dev, "%s: Failed to read gp0 adc: %d\n",
			__func__, ret);
		goto out;
	}

	/* adc = val * 2100mV / 4096 */
	adc = ((buf[0] | (buf[1] << 8)) * 2100) >> 12;

out:
	dev_dbg(chip->dev, "%s: gp0_adc:%d MV\n", __func__, adc);
...
{
	int adc;
	/* The default temp value is -229°C */
	int temp = P922X_DIE_TEMP_DEFAULT;

	if (p922x_get_pg_irq_status(chip))
		adc = p922x_get_gp0_adc(chip);
	else
		goto out;

	/* temp = -72.643 * adc^3 + 209.7 * adc^2 - 239.35 * x + 139.92 */
	temp = ((((-72643 * adc) / 1000) * adc / 1000) * adc / 1000 +
		((209700 * adc) / 1000) * adc / 1000 -
		(239350 * adc) / 1000 + 139920 ) / 100;
...
static int p922x_get_temp_die(struct p922x_dev *chip)
{
	u8 buf[2] = {0};
	int ret = 0;
	/* The default temp value is -229°C */
	...
	if (p922x_get_pg_irq_status(chip))
		ret = chip->bus.read_buf(chip, REG_ADC_DIE, buf, 2);
	else
		goto out;

	if (ret) {
		dev_err(chip->dev, "%s: Failed to read die temp: %d\n",
			__func__, ret);
		goto out;
	}

	/* temp = (val * 0.0898 ) - 229 */
	temp = (buf[0] | (buf[1] << 8)) * 898 - 2290000;
	temp /= 1000;
...
static int p922x_get_rx_negotiated_power(struct p922x_dev *chip)
{
	int ret = 0;
	u32 rx_np = 0;
	u8 np;

	ret = chip->bus.read(chip, REG_RX_NEGO_PWR, &np);
	if (ret) {
		dev_err(chip->dev, "%s: Failed to read rx negotiated power: %d\n",
			__func__, ret);
		goto out;
	}
	...
	dev_dbg(chip->dev, "%s: rx negotiated power:%d\n", __func__, rx_np);
out:
...
static ssize_t p922x_fwver(struct p922x_dev *chip)
{
	...
	chip->bus.read_buf(chip, REG_OTP_FW, ver, 4);

	pr_info("%s: ChipID: %04x, FWVer:%02x.%02x.%02x.%02x\n",
			__func__, id[0] | (id[1] << 8), ver[1], ver[0], ver[3], ver[2]);
...
{
	int ret;
	enum charger_type chg_type = WIRELESS_DEFAULT_CHARGER;
	struct power_supply *chg_psy;
	union power_supply_propval val;

	/* Get charger type from power supply */
	chg_psy = power_supply_get_by_name("Wireless");
	if (chg_psy) {
...
{
	pr_notice("[WPC]%s %d: weak function", __func__, __LINE__);

	return WIRELESS_DEFAULT_CHARGER;
}
#endif
...
static ssize_t p922x_temp_die_show(struct device *dev,
		struct device_attribute *attr,
		char *buffer)
{
	int temp;
...
static ssize_t p922x_gp0_adc_show(struct device *dev,
		struct device_attribute *attr,
		char *buffer)
{
	int adc;
...
static ssize_t p922x_rx_negotiated_power_show(struct device *dev,
		struct device_attribute *attr,
		char *buffer)
{
	u32 rx_np;
	...
	rx_np = p922x_get_rx_negotiated_power(chip);
...
static DEVICE_ATTR(temp_die, 0444, p922x_temp_die_show, NULL);
static DEVICE_ATTR(temp_adc, 0444, p922x_temp_adc_show, NULL);
static DEVICE_ATTR(gp0_adc, 0444, p922x_gp0_adc_show, NULL);
static DEVICE_ATTR(tx_signal_strength, 0444, p922x_tx_signal_strength_show, NULL);
static DEVICE_ATTR(rx_negotiated_power, 0444, p922x_rx_negotiated_power_show, NULL);
...
static struct attribute *p922x_sysfs_attrs[] = {
	...
	&dev_attr_temp_die.attr,
	&dev_attr_temp_adc.attr,
	&dev_attr_gp0_adc.attr,
	&dev_attr_tx_signal_strength.attr,
	&dev_attr_rx_negotiated_power.attr,
	NULL,
};
...
static void p922x_over_handle(struct p922x_dev *chip, u16 irq_src)
{
	...
	if (irq_src & P922X_INT_OV_TEMP)
		...
		if (chip->ovp_cnt > 3) {
			...
			dev_warn(chip->dev,
					"%s: Over Voltage INT Count: %d\n", __func__, chip->ovp_cnt);
			return;
		}
	} else
...
irqreturn_t p922x_int_thread(int irq, void *ptr)
{
	...
	ret = chip->bus.read_buf(chip, REG_INT, buf, 2);
	if (ret) {
		dev_err(chip->dev,
	...
	if (irq_src & P922X_INT_VRECT) {
		irq_status |= P922X_INT_TX_DATA_RECV;
		dev_info(chip->dev,
	...
	if (irq_src & P922X_INT_VOUT) {
		if (irq_status & P922X_INT_AC_MISS_DETECT) {
			dev_info(chip->dev,
				"%s: Received VOUT, offline\n", __func__);
			chip->ovp_cnt = 0;
#ifdef CONFIG_CHARGER_HAL
			atomic_set(&chip->online, false);
			clear_bit(CHARGER_VOUT_EN, &chip->chg_flag);
			clear_bit(CHARGER_EPP_EN, &chip->chg_flag);
			schedule_delayed_work(&chip->chg_update_status_work, 0);
#endif
		} else {
			dev_info(chip->dev,
				"%s: Received VOUT, online\n", __func__);
#ifdef CONFIG_CHARGER_HAL
			atomic_set(&chip->online, true);
			set_bit(CHARGER_VOUT_EN, &chip->chg_flag);
			schedule_delayed_work(&chip->chg_update_status_work, 0);
#endif
		}
	}

	if (irq_src & P922X_INT_EXT_MODE) {
		dev_info(chip->dev,
			"%s: Received EPP, online\n", __func__);
#ifdef CONFIG_CHARGER_HAL
		set_bit(CHARGER_EPP_EN, &chip->chg_flag);
		schedule_delayed_work(&chip->chg_update_status_work, 0);
#endif
	}
...
static int p922x_attached_vbus(struct p922x_dev *chip)
{
	...
	if (chip->use_buck) {
		...
		if (ret) {
			...
			p922x_enable_charge_flow(chip, false);
			return ret;
		}
	}

	p922x_enable_charge_flow(chip, true);
...
#ifdef CONFIG_CHARGER_HAL
	if (test_and_clear_bit(CHARGER_VOUT_EN, &chip->chg_flag)) {
		clear_bit(CHARGER_EPP_EN, &chip->chg_flag);
		schedule_delayed_work(&chip->chg_update_status_work, 0);
	}
#endif
...
static int p922x_support_wpc_en_parse_dt(struct p922x_dev *chip)
{
	...
	pinctrl_select_state(chip->wpc_en_pinctrl, chip->wpc_en_default);
...
static int p922x_support_sleep_en_parse_dt(struct p922x_dev *chip)
{
	...
	pinctrl_select_state(chip->sleep_en_pinctrl, chip->sleep_en_default);
...
static int p922x_set_wpc_en(struct charger_device *chg_dev, bool en)
{
	...
	if (!chip->support_wpc_en) {
		dev_dbg(chip->dev, "%s: not support_wpc_en\n", __func__);
		return 0;
	}
...
static int p922x_set_sleep_en(struct charger_device *chg_dev, bool en)
{
	...
	if (!chip->support_sleep_en) {
		dev_dbg(chip->dev, "%s: not support_sleep_en\n", __func__);
		return 0;
	}
...
static int p922x_force_enable_charge(struct charger_device *chg_dev, bool en)
{
	...
	dev_info(chip->dev, "%s\n", __func__);
	mutex_lock(&chip->irq_lock);
	p922x_enable_charge_flow(chip, en);
	mutex_unlock(&chip->irq_lock);
...
#ifdef CONFIG_CHARGER_HAL
static void p922x_chg_update_status_work(struct work_struct *work)
{
	struct p922x_dev *chip = container_of(work, struct p922x_dev,
						 chg_update_status_work.work);

	cancel_delayed_work(&chip->chg_update_status_work);

	if (test_bit(CHARGER_VOUT_EN, &chip->chg_flag)) {
		if (test_and_clear_bit(CHARGER_EPP_EN, &chip->chg_flag)) {
			chip->power_switch_target = WIRELESS_10W_CHARGER;
			charger_hal_charger_update_status(POWER_SUPPLY_TYPE_WIRELESS, CHARGER_IOUT_7W5, true);
		} else {
			chip->power_switch_target = WIRELESS_5W_CHARGER;
			charger_hal_charger_update_status(POWER_SUPPLY_TYPE_WIRELESS, CHARGER_IOUT_5W, true);
		}
		p922x_write_fod(chip, chip->power_switch_target);
	} else {
		chip->power_switch_target = WIRELESS_DEFAULT_CHARGER;
		charger_hal_charger_update_status(POWER_SUPPLY_TYPE_WIRELESS, CHARGER_IOUT_5W, false);
	}
...
static bool p922x_get_attach_status(struct p922x_dev *chip)
{
	...
	if (irq_status & P922X_INT_VOUT) {
		dev_info(chip->dev,
			"%s: VOUT, online\n", __func__);

		if (irq_status & P922X_INT_EXT_MODE) {
			dev_info(chip->dev,
				"%s: EPP, online\n", __func__);
#ifdef CONFIG_CHARGER_HAL
			set_bit(CHARGER_EPP_EN, &chip->chg_flag);
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGER_TYPE:
		val->intval = chip->power_switch_target;
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		val->intval = chip->wpc_en;
		break;
	default:
		return -EINVAL;
	}
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CHARGER_TYPE:
		chip->power_switch_target = val->intval;
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
#ifdef CONFIG_MTK_CHARGER_CLASS
		if (val->intval == 0)
			p922x_set_wpc_en(chip->chg_dev, false);
		else
			p922x_set_wpc_en(chip->chg_dev, true);
		break;
#endif
	default:
		return -EINVAL;
...
#ifdef CONFIG_CHARGER_HAL
	INIT_DELAYED_WORK(&chip->chg_update_status_work, p922x_chg_update_status_work);
#endif
