...
/* PATCH_BIT0 */
#define MSDC_CKGEN_MSDC_DLY_SEL_M	(0x1f << 10)
#define MSDC_CKGEN_MSDC_DLY_SEL_S	(10)
...
/* PAD_TUNE1 */
#define MSDC_PAD_TUNE_DATRRDLY2		(0x1f << 8)
#define MSDC_PAD_TUNE_CMDRDLY2		(0x1f << 16)
/* EMMC50_CFG0 */
...
#define PAD_DAT_RD_RXDLY_M		(0x1f << 7)
#define PAD_DAT_RD_RXDLY_S		(7)
#define PAD_DAT_RD_RXDLY2_S		(2)
...
static void msdc_set_mclk(struct msdc_host *host, enum bus_mode timing, u32 hz)
{
	...
	if (host->sclk <= 52000000) {
		...
		if (host->top_base) {
			writel(host->def_tune_para.top_control,
			       &host->top_base->emmc_top_control);
			writel(host->def_tune_para.top_cmd,
		...
		if (host->top_base) {
			writel(host->saved_tune_para.top_control,
			       &host->top_base->emmc_top_control);
			writel(host->saved_tune_para.top_cmd,
			       &host->top_base->emmc_top_cmd);
		} else {
...
static struct msdc_delay_phase get_best_delay(struct msdc_host *host, u64 delay)
{
	...
	if (delay == 0) {
		dev_err(dev, "phase error: [map:%llx]\n", delay);
	...
	printf("phase: [map:%llx] [maxlen:%d] [final:%d]\n",
				      ...
				      u8 dly_line)
{
	...
	if (host->top_base) {
		if (dly_line)
		clrsetbits_le32(&host->top_base->emmc_top_cmd,
					PAD_CMD_RXDLY2, value << PAD_CMD_RXDLY2_S);
		else
			clrsetbits_le32(&host->top_base->emmc_top_cmd,
					PAD_CMD_RXDLY_M, value << PAD_CMD_RXDLY_S);
	}
	else	{
		if (dly_line)
			clrsetbits_le32(tune_reg + 4, MSDC_PAD_TUNE_CMDRDLY2,
				       ...
				       u8 dly_line)
{
	...
	if (host->top_base) {
		...
		clrsetbits_le32(&host->top_base->emmc_top_control,
					PAD_DAT_RD_RXDLY2, value << PAD_DAT_RD_RXDLY2_S);
	else
			clrsetbits_le32(&host->top_base->emmc_top_control,
					PAD_DAT_RD_RXDLY_M, value << PAD_DAT_RD_RXDLY_S);
	}
	else {
		if (dly_line)
			clrsetbits_le32(tune_reg + 4, MSDC_PAD_TUNE_DATRRDLY2,
...
static void msdc_set_final_window(struct msdc_host *host, u8 delay, u8 edge)
{
	...
	setbits_le32(&host->base->msdc_iocon, edge << MSDC_IOCON_RSPL);
	setbits_le32(&host->base->msdc_iocon, edge << MSDC_IOCON_DSPL);
	setbits_le32(&host->base->msdc_iocon, edge << MSDC_IOCON_W_DSPL);
...
skip_fall:
	final_maxlen = max(final_rise_delay.maxlen, final_fall_delay.maxlen);
	if (final_fall_delay.maxlen >= 9 && final_fall_delay.start < 4)
...
static int msdc_execute_tuning(struct udevice *dev, uint opcode)
{
	...
	if (host->dev_comp->data_tune && host->dev_comp->async_fifo) {
		...
		if (mmc->selected_mode == MMC_HS_400) {
			...
			clrsetbits_le32(&host->base->pad_tune, MSDC_PAD_TUNE_DATRRDLY_M, 0);
...
tune_done:
	...
	if (host->top_base) {
		host->saved_tune_para.top_control = readl(&host->top_base->emmc_top_control);
		host->saved_tune_para.top_cmd = readl(&host->top_base->emmc_top_cmd);
	}
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	clrsetbits_le32(&host->base->patch_bit0,
					MSDC_CKGEN_MSDC_DLY_SEL_M,
					1 << MSDC_CKGEN_MSDC_DLY_SEL_S);
	...
	host->saved_tune_para.iocon = readl(&host->base->msdc_iocon);
	if (host->top_base) {
		host->def_tune_para.top_control =
			readl(&host->top_base->emmc_top_control);
		host->def_tune_para.top_cmd =
			readl(&host->top_base->emmc_top_cmd);
		host->saved_tune_para.top_control =
			readl(&host->top_base->emmc_top_control);
		host->saved_tune_para.top_cmd =
			readl(&host->top_base->emmc_top_cmd);
	} else {
...
static int msdc_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (host->base == FDT_ADDR_T_NONE)
		return -EINVAL;

	host->top_base = (void *)dev_read_addr_index(dev, 1);
	if (host->top_base == FDT_ADDR_T_NONE)
		host->top_base = NULL;
