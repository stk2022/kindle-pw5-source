...
#define MSDC_PATCH_BIT2_DDR50_SEL (0x1 << 19)   /* RW */
...
#define EMMC50_CTL0_DCCSEL        (0x1 << 0)    /* RW */
#define EMMC50_CTL0_HLSEL         (0x1 << 1)    /* RW */
...
#define EMMC50_CFG_ENDBIT_CHKCNT  (0xf << 5)   /* RW */
#define EMMC50_CFG_CMD_RESP_SEL   (0x1 << 9)   /* RW */
#define EMMC50_CFG_ENDBIT_CNT     (0x3FF << 11) /* RW */
#define EMMC50_CFG_READ_DAT_CNT   (0x7 << 21)  /* RW */

#define EMMC50_CFG3_OUTS_WR       (0x1f << 0)  /* RW */

#define EMMC50_CFG1_DSCFG         (0x1  << 28) /* RW */
...
struct msdc_host {
	...
	void __iomem *top_base;		/* host top base address */
...
static const struct of_device_id msdc_of_ids[] = {
	...
	{ .compatible = "mediatek,mt8695-mmc", .data = &mt8695_compat},
	{ .compatible = "mediatek,mt8512-sdio", .data = &mt8512_compat},
	{ .compatible = "mediatek,mt8518-sdio", .data = &mt8518_compat},
	{}
};
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	host->sclk = sclk;
	host->mclk = hz;
	if ((timing != MMC_TIMING_MMC_HS400) || (!host->is_ddr208))
		host->timing = timing;
		...
		struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	if (cmd->data) {
		...
		if (data->flags & MMC_DATA_READ) {
			if ((data->blocks * data->blksz) > 256)
				sdr_set_field(host->base + EMMC50_CFG0,
						EMMC50_CFG_ENDBIT_CNT,
						(data->blksz + 17));
			else
				sdr_set_field(host->base + EMMC50_CFG0,
						EMMC50_CFG_ENDBIT_CNT, 273);
		} else {
			sdr_set_field(host->base + EMMC50_CFG0,
					EMMC50_CFG_ENDBIT_CNT, 273);
		}
...
static void __msdc_enable_sdio_irq(struct mmc_host *mmc, int enb)
{
	...
	if (enb) {
		...
		/* clear interrupts */
		writel(events & MSDC_INT_SDIOIRQ, host->base + MSDC_INT);
...
static inline void msdc_set_cmd_delay(struct msdc_host *host, u32 value)
{
	...
	if (host->top_base)
		sdr_set_field(host->top_base + SD_TOP_CMD, PAD_CMD_RXDLY,
			      value);
	else
...
skip_fall:
	...
	dev_dbg(host->dev, "Final cmd/data pad delay: %x\n", final_delay);
...
static void sdio_ddr208_init(struct msdc_host *host)
{
	void __iomem *base = host->base;

	writel(host->hs400_ds_delay, base + PAD_DS_TUNE);
	sdr_set_field(base + MSDC_PATCH_BIT1, 0x3 << 19, 3);
	/* DDR50 byte swap issue design fix feature enable */
	sdr_set_bits(base + MSDC_PATCH_BIT2, MSDC_PATCH_BIT2_DDR50_SEL);
	sdr_set_field(base + MSDC_PATCH_BIT2, MSDC_PB2_POPENCNT, 8);
	/* hs400 mode must set it to 0 */
	sdr_clr_bits(base + MSDC_PATCH_BIT2, MSDC_PATCH_BIT2_CFGCRCSTS);
	sdr_clr_bits(base + EMMC50_CTL0, EMMC50_CTL0_DCCSEL);
	sdr_clr_bits(base + EMMC50_CTL0, EMMC50_CTL0_HLSEL);
	sdr_clr_bits(base + EMMC50_CFG0, EMMC50_CFG_CMD_RESP_SEL);
	sdr_clr_bits(base + EMMC50_CFG0, EMMC50_CFG_PADCMD_LATCHCK);
	sdr_set_bits(base + EMMC50_CFG1, EMMC50_CFG1_DSCFG);
	/* to improve read performance, set outstanding to 2 */
	sdr_set_field(base + EMMC50_CFG3, EMMC50_CFG3_OUTS_WR, 2);
}

static int sdio_plus_set_device_ddr208(struct msdc_host *host)
{
	...
	if (err) {
		dev_info(host->dev, "Get CCCR DDR208 fail\n");
		goto end;
	}
	if ((data & SDIO_MTK_DDR208_SUPPORT) == 0) {
		dev_info(host->dev, "Device doesn't support DDR208\n");
		err = -1;
		goto end;
	}
	 ...
	 */
	data = sdio_f0_readb(func, SDIO_CCCR_SPEED, &err);
	if (err) {
		dev_info(host->dev, "Get CCCR SPEED fail\n");
		goto end;
	}

	data = (~SDIO_SPEED_BSS_MASK) | SDIO_SPEED_DDR50;
	sdio_writeb(func, data, SDIO_CCCR_SPEED, &err);
	if (err) {
		dev_info(host->dev, "Set SPEED to DDR50 fail\n");
		goto end;
	}

	sdio_f0_writeb(func, SDIO_MTK_DDR208,
			SDIO_CCCR_MTK_DDR208, &err);
	if (err) {
		dev_info(host->dev, "Set CCCR DDR208 fail\n");
		goto end;
	}

	/**
	 * It need switch host to DDR208 mode
	 * after device switches to DDR208
	 */
	sdio_ddr208_init(host);
	msdc_set_mclk(host, MMC_TIMING_MMC_HS400, host->mclk);

	/* re-tune cmd response. */
	msdc_tune_resp_data(mmc, 19);

	dev_info(host->dev, "Set DDR208 done.\n");
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if (host->dev_comp->tune_resp_data_together) {
		...
		if (host->hs400_mode || host->is_ddr208) {
			sdr_clr_bits(host->base + MSDC_IOCON, MSDC_IOCON_DSPL);
			sdr_clr_bits(host->base + MSDC_IOCON,
					MSDC_IOCON_W_DSPL);
			msdc_set_data_delay(host, 0);
			if (host->is_ddr208)
				sdio_plus_set_device_ddr208(host);
		}
	} else {
		if (host->hs400_mode &&
				host->dev_comp->hs400_tune)
...
static int msdc_prepare_hs400_tuning(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->top_base)
		writel(host->hs400_ds_delay,
		       host->top_base + SD50_PAD_DS_TUNE);
	else
...
{
	if (of_property_read_bool(pdev->dev.of_node, "sd-uhs-ddr208"))
		host->is_ddr208 = true;
	else
		host->is_ddr208 = false;
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	/* create /proc/sdio file for adding sdio dbg cmd. */
	sdio_proc_init(mmc);
