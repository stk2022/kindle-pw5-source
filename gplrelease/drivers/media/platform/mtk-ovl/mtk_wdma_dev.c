...
#define MTK_WDMA_CLK_CNT                1u
#define MTK_WDMA_REG_BASE_COUNT         1u
...
//	return 0;//FPGA
	...
	for (i = 0; i < MTK_WDMA_CLK_CNT; i++) {
		...
		if (ret != 0) {
			...
			ret = -ENXIO;
			for (; reduce > 0U; reduce--)
				clk_disable_unprepare(wdma_dev->clks[reduce-1]);
		} else
...
static int mtk_wdma_mmsys_path_connext(struct regmap *mmsys_regmap)
{
	int ret = RET_OK;
	unsigned int addr = 0;
	unsigned int val = 0;
...
static int mtk_wdma_mmsys_path_disconnext(struct regmap *mmsys_regmap)
{
	int ret = RET_OK;
	unsigned int addr = 0;
	unsigned int val = 0;
...
#define DISP_MUTEX_MOD       0x30U //0x2CU
#define DISP_MUTEX_SOF       0x2CU //0x30U
#define DISP_MUTEX_MOD2      0x34U
static int mtk_wdma_mutex_enable(struct regmap *mutex_regmap)
{
	int ret = RET_OK;
	unsigned int addr = 0;
	unsigned int val = 0;

	addr = (DISP_MUTEX_MOD + DISP_MUTEX_GAP * DISP_MUTEX_IDX);
	ret += regmap_read(mutex_regmap, addr, &val);
	log_dbg("[wdma] mutex org 0x%x = 0x%x", addr, val);
	val |= (unsigned int)((1u << 8) | (1u << 11));
	log_dbg("[wdma] mutex set 0x%x = 0x%x", addr, val);
	ret += regmap_write(mutex_regmap, addr, val);
	ret += regmap_read(mutex_regmap, addr, &val);
	log_dbg("[wdma] mutex new 0x%x = 0x%x", addr, val);

	addr = (DISP_MUTEX_SOF + DISP_MUTEX_GAP * DISP_MUTEX_IDX);
	ret += regmap_read(mutex_regmap, addr, &val);
	log_dbg("[wdma] mutex org 0x%x = 0x%x", addr, val);
	val &= (~(0u << 0));
	log_dbg("[wdma] mutex set 0x%x = 0x%x", addr, val);
	...
	addr = (DISP_MUTEX_EN + DISP_MUTEX_GAP * DISP_MUTEX_IDX);
	ret += regmap_read(mutex_regmap, addr, &val);
	log_dbg("[wdma] mutex org 0x%x = 0x%x", addr, val);
	val |= (unsigned int)(1u << 0);
	log_dbg("[wdma] mutex set 0x%x = 0x%x", addr, val);
	ret += regmap_write(mutex_regmap, addr, val);
	ret += regmap_read(mutex_regmap, addr, &val);
	log_dbg("[wdma] mutex new 0x%x = 0x%x", addr, val);
...
static int mtk_wdma_mutex_disable(struct regmap *mutex_regmap)
{
	int ret = RET_OK;
	unsigned int addr = 0;
	unsigned int val = 0;
...
static int mtk_wdma_param_store(struct mtk_wdma_ctx *ctx, struct vb2_buffer *vb)
{
	...
	log_dbg("[wdma] %s[%d] get curr_vb2_v4l2_buffer 0x%p 0x%p",
	__func__, __LINE__,
	...
	_mtk_wdma_hw_param.src_width = ctx->s_frame.input_size.width;
	_mtk_wdma_hw_param.src_height = ctx->s_frame.input_size.height;
	_mtk_wdma_hw_param.clip_x = (unsigned int)ctx->s_frame.crop.left;
	_mtk_wdma_hw_param.clip_y = (unsigned int)ctx->s_frame.crop.top;
	_mtk_wdma_hw_param.clip_width = ctx->s_frame.crop.width;
	_mtk_wdma_hw_param.clip_height = ctx->s_frame.crop.height;
...
static int mtk_wdma_prepare_hw(struct mtk_wdma_ctx *ctx)
{
	...
	ret = mtk_wdma_clock_on(ctx->wdma_dev);

	ret = mtk_wdma_hw_set(
		ctx->wdma_dev->reg_base[0], &_mtk_wdma_hw_param);

	ret = mtk_wdma_mmsys_path_connext(ctx->wdma_dev->mmsys_regmap);
	ret = mtk_wdma_mutex_enable(ctx->wdma_dev->mutex_regmap);
...
static int mtk_wdma_unprepare_hw(struct mtk_wdma_ctx *ctx)
{
	...
	ret = mtk_wdma_mmsys_path_disconnext(ctx->wdma_dev->mmsys_regmap);
	ret = mtk_wdma_mutex_disable(ctx->wdma_dev->mutex_regmap);

	ret = mtk_wdma_hw_unset(ctx->wdma_dev->reg_base[0]);
...
static const struct mtk_wdma_fmt *mtk_wdma_get_format(unsigned int index)
{
	if (index >= ARRAY_SIZE(mtk_wdma_formats))
		return NULL;

	return (&(mtk_wdma_formats[index]));
}
...
static void mtk_wdma_set_frame_size(
	struct mtk_wdma_frame *frame, unsigned int width, unsigned int height)
{
	frame->f_width = width;
	...
	struct device *alloc_devs[])
{
	...
	*num_planes = (unsigned int)(frame->fmt->num_planes);
...
static int mtk_wdma_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (!IS_ALIGNED(addr, 8UL)) {
		log_err("[wdma] addr[0x%lx] is not 8 align", addr);
		return RET_ERR_EXCEPTION;
	}

	ret = mtk_wdma_param_store(ctx, vb);
...
static void mtk_wdma_stop_streaming(struct vb2_queue *q)
{
	...
	ret = pm_runtime_put_sync(&ctx->wdma_dev->pdev->dev);
}
	...
	struct file *file, void *fh, struct v4l2_capability *cap)
{
	...
	size_ret = strlcpy(
		cap->driver, MTK_WDMA_MODULE_NAME, sizeof(cap->driver));
	size_ret = strlcpy(
		cap->card, ctx->wdma_dev->pdev->name, sizeof(cap->card));
	size_ret = strlcpy(
		cap->bus_info, "platform", sizeof(cap->bus_info));
	...
	struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	...
	size_ret = strlcpy(f->description, fmt->name, sizeof(f->description));
	...
	struct file *file, void *fh, struct v4l2_format *f)
{
	...
	pix_mp->colorspace  = (unsigned int)V4L2_COLORSPACE_REC709;
	pix_mp->num_planes  = frame->fmt->num_planes;

	for (i = 0; i < pix_mp->num_planes; ++i) {
		pix_mp->plane_fmt[i].bytesperline =
			(frame->f_width * frame->fmt->depth[i]) / 8U;
	...
	struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (pix_mp->width >= 1280U) /* HD */
		pix_mp->colorspace = (unsigned int)V4L2_COLORSPACE_REC709;
	else /* SD */
		pix_mp->colorspace = (unsigned int)V4L2_COLORSPACE_SMPTE170M;

	for (i = 0; i < pix_mp->num_planes; ++i) {
		unsigned int bpl = (pix_mp->width * fmt->depth[i]) >> 3;
		unsigned int sizeimage = bpl * pix_mp->height;
	...
	struct file *file, void *fh, struct v4l2_format *f)
{
	...
	log_dbg("[wdma] %s end, WH[%d, %d], col[%d], p[%d, %d][%d, %d]",
		__func__,
	...
	struct file *file, void *fh, struct v4l2_requestbuffers *reqbufs)
{
	...
	if (reqbufs->count != 0) {
		vfd->queue = &ctx->vb2_q;
		log_dbg("[wdma] save vb2_q [0x%p, 0x%p, 0x%p]",
			&(ctx->vb2_q),
			vfd,
			vfd->queue);
	}
	...
	struct file *file, void *fh, struct v4l2_selection *s)
{
	...
	case V4L2_SEL_TGT_COMPOSE_DEFAULT:
	case V4L2_SEL_TGT_COMPOSE:
		frame->input_size.width = s->r.width;
		frame->input_size.height = s->r.height;
		break;
	case V4L2_SEL_TGT_CROP_BOUNDS:
	case V4L2_SEL_TGT_CROP:
	case V4L2_SEL_TGT_CROP_DEFAULT:
	...
	log_dbg("[wdma] %s end, target[0x%x], [%d, %d, %d, %d]",
		__func__,
		s->target,
		s->r.left, s->r.top,
...
static int mtk_wdma_open(struct file *file)
{
	...
	if (mutex_lock_interruptible(&wdma_dev->lock) != 0)
		return RET_ERR_EXCEPTION;
	...
	temp_ret = memset(&(ctx->vb2_q), 0, sizeof(ctx->vb2_q));
	ctx->vb2_q.dev = &(wdma_dev->pdev->dev);
	ctx->vb2_q.type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
	...
	struct mtk_wdma_dev *wdma_dev, enum MTK_WDMA_PROBE_STATUS st)
{
	...
	if (st >= PROBE_ST_IRQ_REQ) {
		...
		if (res == NULL)
			...
			devm_free_irq(
				dev, (unsigned int)(res->start), wdma_dev);
	}
...
static int mtk_wdma_register_video_device(struct mtk_wdma_dev *wdma_dev)
{
	...
	memset(&wdma_dev->vdev, 0, sizeof(wdma_dev->vdev));
	...
	ret = snprintf(wdma_dev->vdev.name, sizeof(wdma_dev->vdev.name),
...
static int mtk_wdma_probe(struct platform_device *pdev)
{
	...
	enum MTK_WDMA_PROBE_STATUS probe_st = PROBE_ST_UNKNOWN;
	...
	log_dbg("ok to get larb device 0x%p",
		wdma_dev->larb_dev);
	...
	for (i = 0; i < MTK_WDMA_CLK_CNT; i++) {
		...
		if (IS_ERR(wdma_dev->clks[i])) {
			...
			ret = -ENXIO;
			for (; reduce > 0U; reduce--)
				devm_clk_put(dev, wdma_dev->clks[reduce-1]);
			goto err_handle;
		} else
			log_dbg("ok to get dev[0x%p] clk[%s] as 0x%p\n",
				dev,
				_ap_mtk_wdma_clk_name[i],
				wdma_dev->clks[i]);
	}
	...
	for (i = 0; i < MTK_WDMA_REG_BASE_COUNT; i++) {
		...
		if (IS_ERR((void *)(wdma_dev->reg_base[i]))) {
			...
			ret = -ENXIO;
			for (; reduce > 0U; reduce--)
				devm_iounmap(dev, (void __iomem *)
					(wdma_dev->reg_base[reduce-1]));
			goto err_handle;
		}

		log_dbg("ok to map reg[%d] base=0x%p",
	...
	if (node != NULL) {
		log_dbg("ok to get mmsys-regmap node 0x%p",
			node);
		wdma_dev->mmsys_regmap = syscon_node_to_regmap(node);
		if (IS_ERR(wdma_dev->mmsys_regmap)) {
			log_err("fail to get mmsys-regmap regmap 0x%p",
				wdma_dev->mmsys_regmap);
			ret = -EPROBE_DEFER;
			goto err_handle;
		}
		log_dbg("ok to get mmsys-regmap regmap 0x%p",
			wdma_dev->mmsys_regmap);
	} else {
		log_err("fail to get mmsys-regmap node 0x%p",
			node);
		dev_err(&pdev->dev,
	...
	if (node != NULL) {
		log_dbg("ok to get mutex-regmap node 0x%p",
			node);
		wdma_dev->mutex_regmap = syscon_node_to_regmap(node);
		if (IS_ERR(wdma_dev->mutex_regmap)) {
			log_err("fail to get mutex-regmap regmap 0x%p",
				wdma_dev->mutex_regmap);
			ret = -EPROBE_DEFER;
			goto err_handle;
		}
		log_dbg("ok to get mutex-regmap regmap 0x%p",
			wdma_dev->mutex_regmap);
	} else {
		log_err("fail to get mutex-regmap node 0x%p",
			node);
		dev_err(&pdev->dev,
