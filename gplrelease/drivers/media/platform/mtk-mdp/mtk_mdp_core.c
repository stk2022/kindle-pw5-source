...
static wait_queue_head_t mdp_latency_wq;
static atomic_t mdp_latency_work;
...
void mtk_mdp_clock_on(struct mtk_mdp_dev *mdp)
{
	...
	for (i = 0; i < ARRAY_SIZE(mdp->comp); i++)
		if (mdp->comp[i] != NULL)
			mtk_mdp_comp_clock_on(dev, mdp->comp[i], mdp);
}

void mtk_mdp_clock_off(struct mtk_mdp_dev *mdp)
{
	struct device *dev = &mdp->pdev->dev;
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(mdp->comp); i++)
		if (mdp->comp[i] != NULL)
			mtk_mdp_comp_clock_off(dev, mdp->comp[i], mdp);
}
...
static int mtk_mdp_probe(struct platform_device *pdev)
{
	...
	mdp->regulator_vcore = regulator_get(&pdev->dev, "vcore");
	if (mdp->regulator_vcore == NULL) {
		dev_err(dev, "get vcore regulator fail\n");
	}

	mdp->vcore_req = kzalloc(sizeof(struct pm_qos_request), GFP_KERNEL);
	/* request vcore voltage 0.65V */
	pm_qos_add_request(mdp->vcore_req,
		PM_QOS_VCORE_OPP,
	...
	mtk_mdp_fb_init(mdp);
	mtk_mdp_sfd_init(mdp);
	mdp_debugfs_init(dev);

	init_waitqueue_head(&mdp_latency_wq);
	atomic_set(&mdp_latency_work, 0);
	return 0;
...
void mdp_latency_work_inc(void)
{
	atomic_inc(&mdp_latency_work);
}

void mdp_latency_work_dec(void)
{
	atomic_dec(&mdp_latency_work);
}

void mdp_wakeup_latency_work(void)
{
	wake_up(&mdp_latency_wq);
}

int mdp_get_latency_work_nr(void)
{
	return atomic_read(&mdp_latency_work);
}

int mdp_wait_latency_work(void)
{
	int ret;

	mtk_mdp_dbg(1, "waiting latency work");

	ret = wait_event_interruptible_timeout(mdp_latency_wq,
					      (atomic_read(&mdp_latency_work) ==
					       0),
					      HZ);

	mtk_mdp_dbg(1, "wait latency work done");
...
static int mtk_mdp_remove(struct platform_device *pdev)
{
	...
	if (mdp->vcore_req) {
		pm_qos_remove_request(mdp->vcore_req);
		kfree(mdp->vcore_req);
		mdp->vcore_req = NULL;
	}

	if (mdp->regulator_vcore) {
		regulator_put(mdp->regulator_vcore);
		mdp->regulator_vcore = NULL;
	}
