...
#ifdef CONFIG_FALCON
		falcon_clr_preload_range(VCU_DMEM0_VIRT(vcu), VCU_DMEM0_LEN(vcu));
#endif
...
static int vcu_alloc_d_ext_mem(struct mtk_vcu *vcu, unsigned long len)
{
	...
	if (vcu->is_alloc == false) {
		...
		vcu->is_alloc = true;
#ifdef CONFIG_FALCON
		falcon_set_preload_range(VCU_DMEM0_VIRT(vcu), VCU_DMEM0_LEN(vcu));
#endif
	}
	mutex_unlock(&vcu->vcu_mutex[1]);
	mutex_unlock(&vcu->vcu_mutex[0]);
...
#ifdef CONFIG_FALCON
			falcon_set_preload(vma->vm_pgoff << PAGE_SHIFT, 1);
#endif
				start += PAGE_SIZE;
				pos += PAGE_SIZE;
				if (length > PAGE_SIZE)
