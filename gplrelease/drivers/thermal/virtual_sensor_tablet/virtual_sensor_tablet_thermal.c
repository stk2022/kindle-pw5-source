...
#define DRIVER_NAME "virtual_sensor_thermal"
#define THERMAL_NAME "virtual_sensor"
#define COMPATIBLE_NAME "amazon,virtual_sensor_thermal"
#define BUF_SIZE 128
#define DMF 1000
#define MASK (0x0FFF)
...
#define WPC_THERMAL_NAME "virtual_sensor1"
static struct wpc_algo *g_wpc;
...
static void wpc_online_work(struct work_struct *work)
{
	...
	thermal = thermal_zone_get_zone_by_name(WPC_THERMAL_NAME);
	if (IS_ERR(thermal) || !(thermal->devdata)) {
		pr_err("[WPC_INFO]: thermal:%p or thermal->devdata is NULL!\n", thermal);
		return ;
	}
	...
	pdata = tzone->pdata;
	if (!pdata) {
		pr_err("[WPC_INFO]: tzone->pdata is NULL!\n");
		return ;
	}

	mutex_lock(&therm_lock);
	if (g_wpc->wpc_online_status) {
		pdata->mode = THERMAL_DEVICE_ENABLED;
		mutex_lock(&tzone->tz->lock);
		if (!tzone->tz->polling_delay)
			tzone->tz->polling_delay = g_wpc->polling_interval;
		for (i = 0; i < thermal->tzp->num_tbps; i++) {
			if (thermal->tzp->tbp[i].cdev && !strcmp(thermal->tzp->tbp[i].cdev->type, "wpc_bcct1")) {
		...
		wpc_printk("wpc is online, polling_delay = %d, vs mode = %d\n",
				thermal->polling_delay, pdata->mode);
		schedule_work(&tzone->therm_work);
	} else {
		pdata->mode = THERMAL_DEVICE_DISABLED;
		if (!g_wpc->polling_interval)
			g_wpc->polling_interval = tzone->tz->polling_delay;
		wpc_printk("wpc is not online, polling_delay = %d, vs mode = %d\n",
				thermal->polling_delay, pdata->mode);
		mutex_lock(&tzone->tz->lock);
...
static int wpc_online_event(struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (event == PSY_EVENT_PROP_CHANGED && strcmp(psy->desc->name, "Wireless") == 0) {
		g_wpc->psy = psy;
		ret = power_supply_get_property(g_wpc->psy, POWER_SUPPLY_PROP_ONLINE, &propval);
		if (ret < 0) {
			pr_err("[WPC_INFO] %s, get psy online failed, ret = %d\n", __func__, ret);
			return NOTIFY_OK;
		} else {
			if (g_wpc->wpc_online_status != propval.intval) {
				g_wpc->wpc_online_status = propval.intval;
				queue_delayed_work(system_freezable_wq, &g_wpc->wpc_event_wq, 0);
				pr_info("[WPC_INFO] %s, online statue = %d\n", __func__, g_wpc->wpc_online_status);
			}
		}
	}
					   ...
					   int *t)
{
...
#if  defined (THERMAL_WPC) && !defined (CONFIG_THERMAL_EMULATION)
	if (!strncmp(thermal->type, WPC_THERMAL_NAME, strlen(WPC_THERMAL_NAME))) {
	...
	list_for_each_entry(tdev, &pdata->ts_list, node) {
...
#ifdef CONFIG_LAB126
			kernel_minerva_metric("ax0m4rye", "x3cb/2/043304d1", 9,
					MINERVA_TYPE_STRING,  "operation", "ThermalEvent",
					MINERVA_TYPE_STRING,  "metric", tdev->name,
					MINERVA_TYPE_LONG,    "counter", temp,
					MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_DEVICE_ID,
					MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_DEVICE_TYPE,
					MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_MARKETPLACE_ID,
					MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_HARDWARE,
					MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_PLATFORM,
					MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_SOFTWARE_VERSION);
#endif
		}
#endif
		...
		if (!strncmp(thermal->type, WPC_THERMAL_NAME, strlen(WPC_THERMAL_NAME))
				&& tdev->tdp->weight) {
			wpc_printk("[%s, %s_off_temp, VS_temp] = [%ld, %ld, %ld]\n",
					tdev->name, tdev->name, temp, tdev->off_temp, tempv);
		}
#endif
	}
	...
	if (!(atomic_read(&tzone->query_count) & tzone->mask)) {
		...
		kernel_minerva_metric("ax0m4rye", "x3cb/2/043304d1", 9,
				MINERVA_TYPE_STRING,  "operation", "ThermalEvent",
				MINERVA_TYPE_STRING,  "metric", thermal->type,
				MINERVA_TYPE_LONG,    "counter", tempv,
				MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_DEVICE_ID,
				MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_DEVICE_TYPE,
				MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_MARKETPLACE_ID,
				MINERVA_TYPE_PREDEFINED, MINERVA_PREDEFINED_HARDWARE,
...
#ifdef THERMAL_WPC
	if (!strncmp(thermal->type, WPC_THERMAL_NAME, strlen(WPC_THERMAL_NAME))) {
		g_wpc->polling_interval = polling_delay;
							 ...
							 *dev)
{
	...
	ret = virtual_sensor_thermal_init_tbp(&p_virtual_sensor_thermal_data->tzp,
					    dev);
...
#ifdef THERMAL_WPC
	if (!pdata->polling_delay)
		g_wpc->polling_interval = pdata->polling_delay;
#endif

	if (pdata->mode == THERMAL_DEVICE_DISABLED)
		pdata->polling_delay = 0;
...
#ifdef THERMAL_WPC
static int virtual_sensor_wpc_init(void)
{
	int retval = 0;
	struct wpc_algo *wpc_algo = NULL;

	wpc_algo = kzalloc(sizeof(struct wpc_algo), GFP_KERNEL);
	if (!wpc_algo) {
		pr_err("%s Failed to allocate wpc_algo memory\n",
			__func__);
		return -ENOMEM;
	}

	wpc_algo->polling_interval = TASK_DURATION;
	wpc_algo->wpc_online_status = 0;

	wpc_algo->psy_nb.notifier_call = wpc_online_event;
	retval = power_supply_reg_notifier(&wpc_algo->psy_nb);
	if (retval < 0) {
	...
	g_wpc = wpc_algo;
	INIT_DELAYED_WORK(&wpc_algo->wpc_event_wq, wpc_online_work);
...
#ifdef THERMAL_WPC
	tz = thermal_zone_get_zone_by_name(WPC_THERMAL_NAME);
	if (IS_ERR(tz)) {
		pr_err("[WPC_INFO]: thermal:%p	is NULL!\n", tz);
		tz = NULL;
	} else {
		err = virtual_sensor_wpc_init();
		if (err)
			pr_err("%s Failed to allocate vs_temp_data memory\n",
				__func__);
		wireless_charger_dev_get_online(get_charger_by_name("wireless_chg"), &wpc_online);
		if (wpc_online) {
			g_wpc->wpc_online_status = wpc_online;
			queue_delayed_work(system_freezable_wq, &g_wpc->wpc_event_wq, 0);
		}
	}
#endif
