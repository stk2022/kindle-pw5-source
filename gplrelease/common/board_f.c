...
#ifdef CONFIG_FALCON
	if (gd->relocaddr > KLOWMEM_TO_PHYS(CONFIG_FALCON_STORAGE_BIOS_ADDR)) {
		gd->relocaddr = KLOWMEM_TO_PHYS(CONFIG_FALCON_STORAGE_BIOS_ADDR);
		gd->relocaddr &= 0xfff00000;
		gd->relocaddr -= 0x00100000;
	}
#endif /* CONFIG_FALCON */
	debug("Ram top: %08lX\n", (ulong)gd->ram_top);
...
static int reserve_uboot(void)
{
	...
	#endif
#ifdef CONFIG_FALCON
	gd->relocaddr &= ~(MMU_SECTION_SIZE - 1);
	gd->relocaddr -= 65536;
