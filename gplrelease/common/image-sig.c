...
#ifdef CONFIG_UFBL
		const char *key_hint = fdt_getprop(sig_blob, noffset, FIT_KEY_HINT, NULL);

		/* Only verify production key on locked production device */
		if (strcmp("fireos-production", key_hint) && ufbl_is_locked_production_device()) {
			continue;
		}
		printf("%s:", key_hint);
#endif
...
#ifdef CONFIG_UFBL
	/* return not verified if not able to verify with all keys */
	return -EPERM;
#else
	return 0;
#endif
