...
/* board-specific MMC post initializations. */
__weak void board_mmc_post_init(void)
{
}
...
static int initr_env(void)
{
...
#if defined(CONFIG_FALCON)
	{
		...
		addr = env_get_ulong("bios_addr", 16, 0);
		if (addr) {
			sprintf(paddr, "0x%08x", KLOWMEM_TO_PHYS(addr));
			env_set("bios_paddr", paddr);
		}

		addr = env_get_ulong("sbios_addr", 16, 0);
		if (addr) {
			sprintf(paddr, "0x%08x", KLOWMEM_TO_PHYS(addr));
			env_set("sbios_paddr", paddr);
		}
	}
#endif /* CONFIG_FALCON */
